...
partial class Creature
{
    ...
    {
        var clothing = GetInventoryItemsOfTypeWeenieType(WeenieType.Clothing)
            .Where(c =>
                ((uint)(c.ClothingPriority ?? 0) & (uint)CoverageMaskHelper.Underwear) != 0
                || ((uint)(c.ValidLocations ?? 0) & (uint)EquipMask.Cloak) != 0
            )
            .ToList();
        ...
        var shirts = clothing
            .Where(c => ((uint)(c.ClothingPriority ?? 0) & (uint)CoverageMaskHelper.UnderwearShirt) != 0)
            .ToList();
        var pants = clothing
            .Where(c => ((uint)(c.ClothingPriority ?? 0) & (uint)CoverageMaskHelper.UnderwearPants) != 0)
            .ToList();
    ...
    public List<WorldObject> SelectWieldedArmor()
    {
        ...
        var armor = GetInventoryItemsOfTypeWeenieType(WeenieType.Clothing)
            .Where(a => ((uint)(a.ClothingPriority ?? 0) & (uint)CoverageMaskHelper.Outerwear) != 0)
            .ToList();
        ...
        var chest = armor
            .Where(c => ((uint)(c.ValidLocations ?? 0) & (uint)(EquipMask.ChestArmor | EquipMask.ChestWear)) != 0)
            .ToList();
        var upperArms = armor
            .Where(c => ((uint)(c.ValidLocations ?? 0) & (uint)(EquipMask.UpperArmArmor | EquipMask.UpperArmWear)) != 0)
            .ToList(); // this will also grab chest pieces that also cover upper arms etc.
        var lowerArms = armor
            .Where(c => ((uint)(c.ValidLocations ?? 0) & (uint)(EquipMask.LowerArmArmor | EquipMask.LowerArmWear)) != 0)
            .ToList();
        ...
        var upperLegs = armor
            .Where(c => ((uint)(c.ValidLocations ?? 0) & (uint)(EquipMask.UpperLegArmor | EquipMask.UpperLegWear)) != 0)
            .ToList();
        var lowerLegs = armor
            .Where(c => ((uint)(c.ValidLocations ?? 0) & (uint)(EquipMask.LowerLegArmor | EquipMask.LowerLegWear)) != 0)
            .ToList();
    ...
    public void GetMonsterInventory(
        List<WorldObject> allWeapons,
        List<WorldObject> ammo,
        bool allowMelee = true,
        bool allowRanged = true,
        bool allowCaster = true
    )
    ...
    {
        var items = weaponsOnly
            ? SelectWieldedWeapons(allowMelee, allowRanged, allowCaster)
            : SelectWieldedTreasure(allowMelee, allowRanged, allowCaster);
