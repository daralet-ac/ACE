...
partial class Creature
{
    ...
    public override ACE.Entity.ObjDesc CalculateObjDesc()
    {
        ...
        var armorItems = EquippedObjects
            .Values.Where(x =>
                (
                    x.ItemType == ItemType.Armor
                    || (x.CurrentWieldedLocation & (EquipMask.Armor | EquipMask.Extremity)) != 0
                )
            )
            .ToList();
        ...
        var clothesAndCloaks = EquippedObjects
            .Values.Where(x =>
                (x.ItemType == ItemType.Clothing)
                && (x.CurrentWieldedLocation & (EquipMask.Armor | EquipMask.Extremity)) == 0
        ...
        foreach (var w in eo)
        {
            ...
            if ((w.CurrentWieldedLocation & (EquipMask.Clothing | EquipMask.Armor | EquipMask.Cloak)) != 0)
            {
                ...
                // Check if the player model has data. Gear Knights, this is usually you.
                {
                    ...
                    var clothingBaseEffect = item.ClothingBaseEffects[thisSetupId];
                    foreach (var t in clothingBaseEffect.CloObjectEffects)
                    {
                        var partNum = (byte)t.Index;
                        coverage.Add(partNum);

                        objDesc.AddAnimPartChange(
                            new PropertiesAnimPart { Index = (byte)t.Index, AnimationId = t.ModelId }
                        );

                        foreach (var t1 in t.CloTextureEffects)
                        {
                            objDesc.AddTextureChange(
                    ...
                    {
                        var size = item.ClothingSubPalEffects.Count;
                        var palCount = size;
        ...
        for (var i = 0; i < wo.CSetup.Parts.Count; i++)
        {
            if (wo.CSetup.Parts[i] != 0x010001EC || i != 16) // This is essentially a "null" part, so do not add it for the head
            {
                objDesc.AnimPartChanges.Add(
                    new PropertiesAnimPart { Index = (byte)i, AnimationId = wo.CSetup.Parts[i] }
                );
            }
        }
