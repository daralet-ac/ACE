...
public partial class Corpse : Container
{
    ...
    /// </summary>
    public Corpse(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    {
        if (
            Biota.PropertiesAnimPart.GetCount(BiotaDatabaseLock) == 0
    ...
    public bool HasPermission(Player player)
    {
        ...
        if (
            TimeToRot != null
            && TimeToRot < HalfLife
            && !new ObjectGuid(VictimId.Value).IsPlayer()
            && !CorpseGeneratedRare
        )
        {
            return true;
        ...
        if (player.Fellowship != null && player.Fellowship.ShareLoot)
        {
            ...
            if (
                onlinePlayer != null
                && onlinePlayer.Fellowship != null
                && player.Fellowship == onlinePlayer.Fellowship
                && !CorpseGeneratedRare
                && PkLevel != PKLevel.PK
            )
            {
    ...
    public bool CorpseGeneratedRare
    {
        ...
        set
        {
            if (!value)
            {
                RemoveProperty(PropertyBool.CorpseGeneratedRare);
            }
            else
            {
                SetProperty(PropertyBool.CorpseGeneratedRare, value);
            }
        }
    }

    public bool IsOnNoDropLandblock =>
        Location != null ? NoDrop_Landblocks.Contains(Location.LandblockId.Landblock) : false;
                ...
                if (Location != null && CorpseGeneratedRare)
                {
                    EnqueueBroadcast(
                        new GameMessageSystemChat(
                            $"{killerName} has discovered the {rareGenerated.Name}!",
    ...
    public void TryGenerateRare(DamageHistoryInfo killer)
    {
        ...
        if (realTimeRares && killerPlayer != null)
        {
            ...
            {
                killerPlayer.RaresLoginTimestamp = (int)
                    Time.GetFutureUnixTime(
                        ThreadSafeRandom.Next(1, (int)PropertyManager.GetLong("rares_max_seconds_between").Item)
                    );
            }
        }
        else if (realTimeRaresAlt && killerPlayer != null)
        {
            if (killerPlayer.RaresLoginTimestamp.HasValue)
            {
                ...
                var chancesModifier = Math.Round(
                    daysSinceRareSighting / maxDaysSinceLastRareFound,
                    2,
        ...
        _log.Debug(
            $"[LOOT][RARE] {Name} ({Guid}) generated rare {wo.Name} ({wo.Guid}) for {killer.Name} ({killer.Guid})"
        );
