...
partial class Creature
{
    ...
    public double? PowerupTime
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyFloat.PowerupTime);
            }
            else
            {
                SetProperty(PropertyFloat.PowerupTime, value.Value);
            }
        }
    }
    ...
    public float MeleeAttack()
    {
        ...
        DoSwingMotion(AttackTarget, motionCommand.Value, out var animLength, out var attackFrames);
        ...
        for (var i = 0; i < numStrikes; i++)
        {
            ...
            actionChain.AddAction(
                ...
                () =>
                {
                    ...
                    var damageEvent = DamageEvent.CalculateDamage(
                        this,
                        target,
                        weapon,
                        motionCommand,
                    ...
                    target.OnAttackReceived(
                        ...
                        damageEvent.Evaded || damageEvent.Blocked
                    );
    ...
    public MotionCommand? GetCombatManeuver()
    {
        ...
        {
            _log.Error(
                $"{Name} ({Guid}).GetCombatManeuver() - couldn't find stance {CurrentMotionState.Stance} in MotionTable {MotionTableId:X8}"
            );
            return null;
        }
        ...
        {
            _log.Error(
                $"{Name} ({Guid}).GetCombatManeuver() - couldn't find attack height {AttackHeight} for stance {CurrentMotionState.Stance} in CMT {CombatTableDID:X8}"
            );
            return null;
        }
        ...
        {
            if (
                AttackType == AttackType.Punch && AttackHeight == ACE.Entity.Enum.AttackHeight.Low
                || AttackType == AttackType.Kick
            )
            {
                ...
                {
                    _log.Error(
                        $"{Name} ({Guid}).GetCombatManeuver() - couldn't find attack type Kick or Punch for attack height {AttackHeight} and stance {CurrentMotionState.Stance} in CMT {CombatTableDID:X8}"
                    );
                    return null;
                }
            }
            else if (AttackType.IsMultiStrike())
            {
                ...
                {
                    _log.Error(
                        $"{Name} ({Guid}).GetCombatManeuver() - couldn't find attack type {reduced} for attack height {AttackHeight} and stance {CurrentMotionState.Stance} in CMT {CombatTableDID:X8}"
                    );
                    return null;
                }
                //else
            ...
            {
                _log.Error(
                    $"{Name} ({Guid}).GetCombatManeuver() - couldn't find attack type {AttackType} for attack height {AttackHeight} and stance {CurrentMotionState.Stance} in CMT {CombatTableDID:X8}"
                );
                return null;
            }
        }
        ...
        if (!motions.ContainsKey((uint)motionCommand))
        {
            ...
            _log.Error(
                $"{Name} ({Guid}).GetCombatManeuver() - couldn't find {motionCommand} in MotionTable {MotionTableId:X8}"
            );
            return null;
        }
    ...
    private static readonly List<(float time, AttackHook attackHook)> defaultAttackFrames = new List<(
        float time,
    ...
    private static readonly ConcurrentDictionary<AttackFrameParams, bool> missingAttackFrames =
        new ConcurrentDictionary<AttackFrameParams, bool>();
    ...
    /// </summary>
    public void DoSwingMotion(
        WorldObject target,
        MotionCommand motionCommand,
        out float animLength,
        out List<(float time, AttackHook attackHook)> attackFrames
    )
    {
        if (!moveBit)
        {
        ...
        if (attackFrames.Count == 0)
        {
            ...
            if (!missingAttackFrames.ContainsKey(attackFrameParams))
            {
                ...
                _log.Warning(
                    $"{Name} ({Guid}) - no attack frames for MotionTable {MotionTableId:X8}, {CurrentMotionState.Stance}, {motionCommand}, using defaults"
                );
    ...
    public List<WorldObject> GetArmorLayers(Player target, BodyPart bodyPart)
    {
        ...
        var equipped = target
            .EquippedObjects.Values.Where(e => e is Clothing && (e.ClothingPriority & coverageMask) != 0)
            .ToList();
    ...
    public float GetArmorMod(
        ...
        List<WorldObject> armors,
        WorldObject weapon,
        float armorRendingMod = 1.0f
    )
    {
    ...
    public float GetSkillModifiedArmorLevel(
        float armorLevel,
        ArmorWeightClass armorWeightClass,
        float resistanceLevel = 1.0f
    )
    {
    ...
    /// </summary>
    public KeyValuePair<CombatBodyPart, PropertiesBodyPart> GetAttackPart(
        MotionCommand motionCommand,
        AttackHook attackHook
    )
    {
        ...
        if (parts.Count == 0)
        {
            ...
            _log.Warning(
                $"CombatTable: {CombatTableDID:X8}, MotionTable: {MotionTableId:X8}, CurrentStance: {CurrentMotionState.Stance}, AttackHeight: {AttackHeight}, AttackType: {AttackType}"
            );
