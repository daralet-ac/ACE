...
partial class Player
{
    public void PlayerEnterWorld()
    {
        ...
        _log.Debug(
            "[LOGIN] Account {Account} entered the world with character {PlayerName} (0x{PlayerGuid}) at {Timestamp}.",
            Account.AccountName,
            Name,
            Guid,
            DateTime.Now
    ...
    public void JoinTurbineChatChannel(string channelName)
    {
        ...
        Session.Network.EnqueueSend(
            new GameEventWeenieErrorWithString(Session, WeenieErrorWithString.YouHaveEnteredThe_Channel, channelName)
        );
    ...
    public void LeaveTurbineChatChannel(string channelName, bool breakAllegiance = false)
    {
        ...
        Session.Network.EnqueueSend(
            new GameEventWeenieErrorWithString(Session, WeenieErrorWithString.YouHaveLeftThe_Channel, channelName)
        );
    ...
    public void SendPropertyUpdatesAndOverrides()
    {
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessagePrivateUpdatePropertyBool(this, PropertyBool.SpellComponentsRequired, false)
            );
        }
    }
    ...
    public void HandlePreOrderItems()
    {
        ...
        switch (subscriptionStatus)
        {
            default:
                status = "purchasing";
                success = TryCreatePreOrderItem(
                    PropertyBool.ActdReceivedItems,
                    ACE.Entity.Enum.WeenieClassName.W_GEMACTDPURCHASEREWARDARMOR_CLASS
                );
                break;
            case SubscriptionStatus.ThroneOfDestiny_Preordered:
                status = "pre-ordering";
                TryCreatePreOrderItem(
                    PropertyBool.ActdReceivedItems,
                    ACE.Entity.Enum.WeenieClassName.W_GEMACTDPURCHASEREWARDARMOR_CLASS
                ); // pcaps show this actually didn't occur on retail. odd
                success = TryCreatePreOrderItem(
                    PropertyBool.ActdPreorderReceivedItems,
                    ACE.Entity.Enum.WeenieClassName.W_GEMACTDPURCHASEREWARDHEALTH_CLASS
                );
                break;
        }

        var msg =
            $"Thank you for {status} the Throne of Destiny expansion! A special gift has been placed in your backpack.";
