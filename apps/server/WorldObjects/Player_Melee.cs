...
partial class Player
{
    ...
    public void HandleActionTargetedMeleeAttack(uint targetGuid, uint attackHeight, float powerLevel)
    {
        ...
        {
            _log.Error(
                $"{Name}.HandleActionTargetedMeleeAttack({targetGuid:X8}, {attackHeight}, {powerLevel}) - CombatMode mismatch {CombatMode}, LastCombatMode {LastCombatMode}"
            );
        ...
        {
            _log.Warning(
                $"{Name}.HandleActionTargetedMeleeAttack({targetGuid:X8}, {AttackHeight}, {powerLevel}) - target guid not creature"
            );
            OnAttackDone();
            return;
        }
    ...
    public MotionCommand GetSwingAnimation()
    {
        ...
        var motions = CombatTable.GetMotion(
            CurrentMotionState.Stance,
            AttackHeight.Value,
            AttackType,
            PrevMotionCommand
