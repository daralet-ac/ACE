...
partial class Jewel : WorldObject
{
    ...
    public static void HandlePlayerAttackerBonuses(
    ...
    )
    {
        ...
        if (playerAttacker.GetEquippedItemsRatingSum(PropertyInt.GearManasteal) > 0)
        {
            ...
            if (playerAttacker != defender && manaStealChance >= ThreadSafeRandom.Next(0.0f, 1.0f))
            {
                ...
                playerAttacker.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"Mana Leech! Your attack restores {healAmount} mana.",
        ...
        if (
            playerAttacker.GetEquippedItemsRatingSum(PropertyInt.GearFire) > 0
    ...
    public static void HandlePlayerDefenderBonuses(Player playerDefender, Creature attacker, float damage)
    {
        ...
        {
            var healthToStamChance = (
                (float)playerDefender.GetEquippedItemsRatingSum(PropertyInt.GearHealthToStamina) / 100
            );
            ...
            if (attacker != playerDefender && healthToStamChance >= ThreadSafeRandom.Next(0.0f, 1.0f))
            {
                ...
                playerDefender.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"Your Amber of the Masochist restores {stamAmount} points of Stamina!",
        ...
        {
            var healthToManaChance = (
                (float)playerDefender.GetEquippedItemsRatingSum(PropertyInt.GearHealthToMana) / 100
            );
    ...
    public static void HandleMeleeAttackerBonuses(
        Player playerAttacker,
        Creature defender,
        float damage,
    ...
    )
    {
        ...
        if (
            playerAttacker.GetEquippedItemsRatingSum(PropertyInt.GearPierce) > 0
            || playerAttacker.GetEquippedItemsRatingSum(PropertyInt.GearBludgeon) > 0
            || playerAttacker.GetEquippedItemsRatingSum(PropertyInt.GearFamiliarity) > 0
        )
        {
    ...
    public static void HandleCasterAttackerBonuses(
        Player sourcePlayer,
    ...
    public static void HandleCasterDefenderBonuses(
        Player targetPlayer,
    ...
    public static float HandleElementalBonuses(Player playerAttacker, DamageType damageType)
    {
        ...
        {
            jewelElemental +=
                ((float)playerAttacker.GetEquippedItemsRatingSum(PropertyInt.GearLightning) / 100) * 0.75f;
        }
