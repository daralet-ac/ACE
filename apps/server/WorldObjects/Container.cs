...
public partial class Container : WorldObject
{
    ...
    /// </summary>
    public Container(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        ...
        : base(biota)
    {
        ...
        {
            DatabaseManager.Shard.GetInventoryInParallel(
                biota.Id,
                false,
                biotas =>
                {
    ...
    public int? MerchandiseItemTypes
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyInt.MerchandiseItemTypes);
            }
            else
            {
    ...
    {
        var freeSlots = (ItemCapacity ?? 0) - CountPackItems();
    ...
    public List<WorldObject> GetInventoryItemsOfTypeWeenieType(WeenieType type)
    {
        ...
        var localInventory = Inventory
            .Values.Where(wo => wo.WeenieType == type)
    ...
    public List<WorldObject> GetInventoryItemsOfWCID(uint weenieClassId)
    {
        ...
        var localInventory = Inventory
            .Values.Where(i => i.WeenieClassId == weenieClassId)
            .OrderBy(i => i.PlacementPosition)
            .ToList();
    ...
    public List<WorldObject> GetInventoryItemsOfWeenieClass(string weenieClassName)
    {
        ...
        // search main pack / creature
        var localInventory = Inventory
            .Values.Where(i => i.WeenieClassName.Equals(weenieClassName, StringComparison.OrdinalIgnoreCase))
            .OrderBy(i => i.PlacementPosition)
            .ToList();
    ...
    public List<WorldObject> GetTradeNotes()
    {
        ...
        // search main pack / creature
        var localInventory = Inventory
            .Values.Where(i => i.WeenieClassName.StartsWith("tradenote"))
            .OrderBy(i => i.PlacementPosition)
            .ToList();
    ...
    public bool TryAddToInventory(
        ...
        bool limitToMainPackOnly = false,
        bool burdenCheck = true
    )
    {
        if (worldObject == null)
        {
    ...
    public bool CanAddToInventory(int totalContainerObjectsToAdd, int totalInventoryObjectsToAdd, int totalBurdenToAdd)
    {
        ...
        return (GetFreeContainerSlots() >= totalContainerObjectsToAdd)
            && (GetFreeInventorySlots() >= totalInventoryObjectsToAdd);
    }
    ...
    )
    {
        ...
        {
            containerItems
                .Where(i => !i.UseBackpackSlot && i.PlacementPosition >= placementPosition)
                .ToList()
                .ForEach(i => i.PlacementPosition++);
        }
        else
        {
            containerItems
        ...
        {
            worldObject.WieldSkillType = worldObject.WieldSkillType.HasValue
                ? (int)worldObject.ConvertToMoASkill((Skill)worldObject.WieldSkillType)
                : null;
            worldObject.WieldSkillType2 = worldObject.WieldSkillType2.HasValue
                ? (int)worldObject.ConvertToMoASkill((Skill)worldObject.WieldSkillType2)
                : null;
            worldObject.WieldSkillType3 = worldObject.WieldSkillType3.HasValue
                ? (int)worldObject.ConvertToMoASkill((Skill)worldObject.WieldSkillType3)
                : null;
            worldObject.WieldSkillType4 = worldObject.WieldSkillType4.HasValue
                ? (int)worldObject.ConvertToMoASkill((Skill)worldObject.WieldSkillType4)
                : null;
        }
    ...
    public bool TryRemoveFromInventory(ObjectGuid objectGuid, out WorldObject item, bool forceSave = false)
    {
        ...
        {
            var removedItemsPlacementPosition = item.PlacementPosition ?? 0;
            ...
            {
                Inventory
                    .Values.Where(i => !i.UseBackpackSlot && i.PlacementPosition > removedItemsPlacementPosition)
                    .ToList()
                    .ForEach(i => i.PlacementPosition--);
            }
            else
    ...
    public override void ActOnUse(WorldObject wo)
    {
        ...
        if (player.LastOpenedContainerId != ObjectGuid.Invalid && player.LastOpenedContainerId != Guid)
        {
            ...
            if (
                ...
                && lastOpenedContainer.IsOpen
                && lastOpenedContainer.Viewer == player.Guid.Full
            )
            {
    ...
    public void GenerateContainList()
    {
        ...
        foreach (
            var item in Biota.PropertiesCreateList.Where(x =>
                x.DestinationType == DestinationType.Contain || x.DestinationType == DestinationType.ContainTreasure
            )
        )
        {
    ...
    public void MergeAllStackables()
    {
        ...
        for (var i = inventory.Count - 1; i > 0; i--)
        {
            ...
            for (var j = 0; j < i; j++)
            {
                ...
                if (
                    destinationItem.WeenieClassId != sourceItem.WeenieClassId
                    || destinationItem.StackSize == destinationItem.MaxStackSize
                )
                {
                    continue;
                }
                ...
                var amount = Math.Min(
                    ...
                    (destinationItem.MaxStackSize - destinationItem.StackSize) ?? 0
                );
    ...
    public override bool IsAttunedOrContainsAttuned =>
        base.IsAttunedOrContainsAttuned || Inventory.Values.Any(i => i.IsAttunedOrContainsAttuned);

    public override bool IsStickyAttunedOrContainsStickyAttuned =>
        base.IsStickyAttunedOrContainsStickyAttuned
        || Inventory.Values.Any(i => i.IsStickyAttunedOrContainsStickyAttuned);

    public override bool IsUniqueOrContainsUnique =>
        base.IsUniqueOrContainsUnique || Inventory.Values.Any(i => i.IsUniqueOrContainsUnique);

    public override bool IsBeingTradedOrContainsItemBeingTraded(HashSet<ObjectGuid> guidList) =>
        base.IsBeingTradedOrContainsItemBeingTraded(guidList)
        || Inventory.Values.Any(i => i.IsBeingTradedOrContainsItemBeingTraded(guidList));
