...
public class ManaStone : WorldObject
{
    ...
    /// </summary>
    public ManaStone(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    private double? Efficiency
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyFloat.ItemEfficiency);
            }
            else
            {
                SetProperty(PropertyFloat.ItemEfficiency, value.Value);
            }
        }
    }
    ...
    private double? DestroyChance
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyFloat.ManaStoneDestroyChance);
            }
            else
            {
                SetProperty(PropertyFloat.ManaStoneDestroyChance, value.Value);
            }
        }
    }
    ...
    public override void HandleActionUseOnTarget(Player player, WorldObject target)
    {
        ...
        if (player != target)
        {
            ...
            if (invTarget == null)
            {
                ...
                player.Session.Network.EnqueueSend(
                    new GameEventUseDone(player.Session, WeenieError.YouDoNotOwnThatItem)
                );
                return;
            }
        ...
        if (!ItemCurMana.HasValue)
        {
            if (target == player)
            {
                ...
                target.ItemCurMana.HasValue
                && target.ItemCurMana.Value > 0
                && target.ItemMaxMana.HasValue
                && target.ItemMaxMana.Value > 0
            )
            {
                ...
                else
                {
                    if (
                        ...
                        && !player.TryDequipObjectWithNetworking(
                            target.Guid,
                            out _,
                            Player.DequipObjectAction.ConsumeItem
                        )
                    )
                    {
                        _log.Error($"Failed to remove {target.Name} from player inventory.");
                        player.Session.Network.EnqueueSend(
                            new GameEventUseDone(player.Session, WeenieError.ActionCancelled)
                        );
                        return;
                    }
                    ...
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat(
                            $"The Mana Stone drains {ItemCurMana:N0} points of mana from the {target.Name}.\nThe {target.Name} is destroyed.",
        ...
        else if (ItemCurMana.Value > 0)
        {
            if (target == player)
            {
                ...
                if (itemsGivenMana.Count < 1)
                {
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat(
                            "You have no items equipped that need mana.",
                            ChatMessageType.Broadcast
                        )
                    );
                    useResult = WeenieError.ActionCancelled;
                ...
                else
                {
                    ...
                    var additionalManaText =
                        (additionalManaNeeded > 0)
                            ...
                            : "\nYour items are fully charged.";
                    var msg =
                        $"The Mana Stone gives {itemsGivenMana.Values.Sum():N0} points of mana to the following items: {itemsGivenMana.Select(c => c.Key.Name).Aggregate((a, b) => a + ", " + b)}.{additionalManaText}";
            ...
            else if (target.ItemMaxMana.HasValue && target.ItemMaxMana.Value > 0)
            {
                ...
                if (targetItemCurMana >= target.ItemMaxMana)
                {
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat(
                            $"The {target.Name} is already full of mana.",
                            ChatMessageType.Broadcast
                        )
                    );
                }
                else
                {
                    ...
                    {
                        manaToPour = (int)
                            Math.Round(manaToPour * Creature.GetPositiveRatingMod(player.LumAugItemManaGain * 5));
    ...
    private bool DoDestroyDiceRoll(Player player)
    {
        ...
        if (dice < DestroyChance)
        {
            ...
            {
                player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat($"The Mana Stone is destroyed.", ChatMessageType.Broadcast)
                );
                return true;
            }
        }
