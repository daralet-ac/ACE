...
partial class WorldObject
{
    ...
    {
        get => GeneratorProfiles != null && GeneratorProfiles.Count > 0;
    }
    ...
    public void InitializeGenerator()
    {
        ...
        {
            _log.Warning(
                "[GENERATOR] 0x{GeneratorGuid} {Generator}.InitializeGenerator: {WeenieClassName} ({WeenieClassId}) MaxGeneratedObjects = {MaxGeneratedObjects} | InitGeneratedObjects = {InitialGeneratedObjects}. Setting MaxGeneratedObjects = InitGeneratedObjects",
                Guid.Full,
                Name,
                WeenieClassName,
                WeenieClassId,
    ...
    {
        get => GeneratorProfiles.Select(i => i.CurrentCreate).Sum();
    }
    ...
    public void SelectAProfile()
    {
        ...
        for (var i = 0; i < GeneratorProfiles.Count; i++)
        {
            ...
            if (profile.RegenLocationType.HasFlag(RegenLocationType.Treasure))
            {
                ...
                {
                    _log.Warning(
                        $"[GENERATOR] 0x{Guid} {Name}.SelectAProfile(): profile[{i}].RegenLocationType({profile.RegenLocationType}), profile.Biota.WCID({profile.Biota.WeenieClassId}), profile.Biota.InitCreate({profile.Biota.InitCreate}) > 1, set to 1. WCID: {WeenieClassId} - LOC: {Location.ToLOCString()}"
                    );
                ...
                {
                    _log.Warning(
                        $"[GENERATOR] 0x{Guid} {Name}.SelectAProfile(): profile[{i}].RegenLocationType({profile.RegenLocationType}), profile.Biota.WCID({profile.Biota.WeenieClassId}), profile.Biota.MaxCreate({profile.Biota.MaxCreate}) > 1, set to 1. WCID: {WeenieClassId} - LOC: {Location.ToLOCString()}"
                    );
    ...
    public int GetSpawnObjectsForProfile(GeneratorProfile profile)
    {
        ...
        {
            _log.Warning(
                $"[GENERATOR] 0x{Guid}:{WeenieClassId} {Name}.GetSpawnObjectsForProfile(profile[{profile.LinkId}]): profile.InitCreate = {profile.InitCreate} | profile.MaxCreate = {profile.MaxCreate} | profile.WeenieClassId = {profile.WeenieClassId} | Profile Init invalid, cannot spawn."
            );
        }
        else if (numObjects == 0)
        {
            _log.Warning(
                $"[GENERATOR] 0x{Guid}:{WeenieClassId} {Name}.GetSpawnObjectsForProfile(profile[{profile.LinkId}]): profile.InitCreate = {profile.InitCreate} | profile.MaxCreate = {profile.MaxCreate} | profile.WeenieClassId = {profile.WeenieClassId} | genSlotsAvailable = {genSlotsAvailable} | profileSlotsAvailable = {profileSlotsAvailable} | numObjects = {numObjects}, cannot spawn."
            );
        }
    ...
    public void StartGenerator()
    {
        ...
        {
            if (
                this is Container && this is not Creature
    ...
    /// </summary>
    public void ProcessGeneratorDestructionDirective(
        GeneratorDestruct generatorDestructType,
        bool fromLandblockUnload = false
    )
    {
    ...
    public void NotifyOfEvent(RegenerationType regenerationType)
    {
        ...
        {
            if (
                (!(Generator is Container) || Generator.GeneratorAutomaticDestruction)
                && Generator.InitCreate > 0
                && Generator.CurrentCreate == 0
    ...
    public void Generator_Update()
    {
        ...
        if (!GeneratorEnteredWorld)
        {
            ...
            {
                if (
                    PropertyManager.GetBool("increase_minimum_encounter_spawn_density").Item
                    && (GetProperty(ACE.Entity.Enum.Properties.PropertyBool.IsPseudoRandomGenerator) ?? false == true)
                )
                {
                    ...
                    if (!Name.ToLower().Contains("harmless")) // Harmless generators can stay enabled
                    {
                        ...
                        {
                            var pkStatus = (PlayerKillerStatus)(
                                obj.GetProperty(ACE.Entity.Enum.Properties.PropertyInt.PlayerKillerStatus) ?? 0
                            );
                            if (
                                (
                                    obj.ItemType == ItemType.LifeStone
                                    || obj.ItemType == ItemType.Portal
                                    || pkStatus == PlayerKillerStatus.Vendor
                                    || pkStatus == PlayerKillerStatus.RubberGlue
                                )
                                && PhysicsObj.get_distance_to_object(obj.PhysicsObj, true) < 50
                            )
                            {
        ...
        if (!GeneratorDisabled)
        {
            if (CurrentlyPoweringUp)
            {
                ...
                while (!GenStopSelectProfileConditions)
                {
                    ...
                    {
                        _log.Error(
                            $"[GENERATOR] 0x{Guid} {Name}.Generator_Generate(): genLoopCount > 1000, aborted init spawn. GenStopSelectProfileConditions: {GenStopSelectProfileConditions} | InitCreate: {InitCreate} | CurrentCreate: {CurrentCreate} | WCID: {WeenieClassId} - LOC: {Location.ToLOCString()}"
                        );
                        break;
                    }
                }
