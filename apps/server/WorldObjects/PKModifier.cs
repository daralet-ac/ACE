...
public class PKModifier : WorldObject
{
    ...
    /// </summary>
    public PKModifier(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    public override ActivationResult CheckUseRequirements(WorldObject activator)
    {
        ...
        )
        {
            if (!string.IsNullOrWhiteSpace(GetProperty(PropertyString.UsePkServerError)))
            {
                player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(GetProperty(PropertyString.UsePkServerError), ChatMessageType.Broadcast)
                );
            }
        ...
        if (player.IsAdvocate || player.AdvocateQuest || player.AdvocateState)
        {
            return new ActivationResult(
                new GameEventWeenieError(player.Session, WeenieError.AdvocatesCannotChangePKStatus)
            );
        }
        ...
        {
            return new ActivationResult(
                new GameEventWeenieError(player.Session, WeenieError.CannotChangePKStatusWhileRecovering)
            );
        }
        ...
        {
            return new ActivationResult(
                new GameEventWeenieErrorWithString(player.Session, WeenieErrorWithString.The_IsCurrentlyInUse, Name)
            );
        }
    ...
    public override void ActOnUse(WorldObject activator)
    {
        ...
        if (
            player.PkLevel == PKLevel.PK
            && IsNPKSwitch
            && (Time.GetUnixTime() - player.PkTimestamp) < MinimumTimeSincePk
        )
        {
            IsBusy = true;
            player.IsBusy = true;
            ...
            actionChain.AddAction(
                ...
                {
                    player.Session.Network.EnqueueSend(
                        new GameEventWeenieError(player.Session, WeenieError.YouFeelAHarshDissonance)
                    );
                    player.IsBusy = false;
                    Reset();
                }
        ...
        if ((player.PkLevel == PKLevel.NPK && IsPKSwitch) || (player.PkLevel == PKLevel.PK && IsNPKSwitch))
        {
            ...
            var useMotion =
                UseTargetSuccessAnimation != MotionCommand.Invalid ? UseTargetSuccessAnimation : MotionCommand.Twitch1;
            EnqueueBroadcastMotion(new Motion(this, useMotion));
            ...
            actionChain.AddAction(
                ...
                {
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat(GetProperty(PropertyString.UseMessage), ChatMessageType.Broadcast)
                    );
        ...
        {
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat(GetProperty(PropertyString.ActivationFailure), ChatMessageType.Broadcast)
            );
        }
    }
