...
partial class Player
{
    ...
    public void HandleActionUseWithTarget(uint sourceObjectGuid, uint targetObjectGuid)
    {
        ...
        var sourceItem = FindObject(
            ...
            out _,
            out _,
            out var sourceItemIsEquipped
        );

        if (sourceItem == null)
        {
            _log.Warning(
                $"{Name}.HandleActionUseWithTarget({sourceObjectGuid:X8}, {targetObjectGuid:X8}): couldn't find {sourceObjectGuid:X8}"
            );
            SendUseDoneEvent();
            return;
        }
        ...
        var target = FindObject(
            targetObjectGuid,
            SearchLocations.MyInventory | SearchLocations.MyEquippedItems | SearchLocations.Landblock
        );

        if (target == null)
        {
            _log.Warning(
                $"{Name}.HandleActionUseWithTarget({sourceObjectGuid:X8}, {targetObjectGuid:X8}): couldn't find {targetObjectGuid:X8}"
            );
            SendUseDoneEvent();
            return;
        }
    ...
    public void ApplyConsumable(MotionCommand useMotion, Action action, float animMod = 1.0f)
    {
        ...
        {
            animTime = EnqueueMotion_Force(
                actionChain,
                MotionStance.NonCombat,
                MotionCommand.Ready,
                (MotionCommand)prevStance
            );
        }
    ...
    public void HandleMotionDone_UseConsumable(uint motionID, bool success)
    {
        ...
        if (!FastTick || !FoodState.IsChugging)
        {
            return;
        }
