...
partial class Creature
{
    ...
    public virtual void MoveTo(WorldObject target, float runRate = 1.0f)
    {
        ...
        {
            Console.WriteLine(
                $"{Name}.MoveTo({target.Name}, {runRate}) - CurPos: {Location.ToLOCString()} - DestPos: {AttackTarget.Location.ToLOCString()} - TargetDist: {Vector3.Distance(Location.ToGlobal(), AttackTarget.Location.ToGlobal())}"
            );
        }
    ...
    public void FakeTeleport(Position _newPosition)
    {
        ...
        setPosition.Flags =
            Physics.Common.SetPositionFlags.SendPositionEvent
            | Physics.Common.SetPositionFlags.Slide
            | Physics.Common.SetPositionFlags.Placement
            | Physics.Common.SetPositionFlags.Teleport;
