...
public class AttributeTransferDevice : WorldObject
{
    public PropertyAttribute TransferFromAttribute
    {
        ...
        set
        {
            if (value == 0)
            {
                RemoveProperty(PropertyInt.TransferFromAttribute);
            }
            else
            {
                SetProperty(PropertyInt.TransferFromAttribute, (int)value);
            }
        }
    }
    ...
    public PropertyAttribute TransferToAttribute
    {
        ...
        set
        {
            if (value == 0)
            {
                RemoveProperty(PropertyInt.TransferToAttribute);
            }
            else
            {
                SetProperty(PropertyInt.TransferToAttribute, (int)value);
            }
        }
    }
    ...
    /// </summary>
    public AttributeTransferDevice(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    /// </summary>
    public AttributeTransferDevice(Biota biota)
        : base(biota)
    {
        SetEphemeralValues();
    }
    ...
    public void ActOnUse(WorldObject activator, bool confirmed)
    {
        ...
        if (fromAttr.StartingValue <= 10)
        {
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"Your innate {TransferFromAttribute} must be above 10 to use the {Name}.",
        ...
        if (toAttr.StartingValue >= 100)
        {
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"Your innate {TransferToAttribute} must be below 100 to use the {Name}.",
                    ChatMessageType.Broadcast
                )
            );
            return;
        ...
        if (!confirmed)
        {
            if (
                !player.ConfirmationManager.EnqueueSend(
                    new Confirmation_AlterAttribute(player.Guid, Guid),
        ...
        var msgFrom = new GameMessageSystemChat(
            $"Your base {TransferFromAttribute} is now {fromAttr.Base}!",
            ChatMessageType.Broadcast
        );
        var msgTo = new GameMessageSystemChat(
            $"Your base {TransferToAttribute} is now {toAttr.Base}!",
            ChatMessageType.Broadcast
        );
