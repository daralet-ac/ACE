...
public partial class Portal : WorldObject
{
    ...
    /// </summary>
    public Portal(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    public override bool EnterWorld()
    {
        ...
        if (RelativeDestination != null && Location != null && Destination == null)
        {
            ...
            relativeDestination.Pos += new Vector3(
                RelativeDestination.PositionX,
                RelativeDestination.PositionY,
                RelativeDestination.PositionZ
            );
            relativeDestination.Rotation = new Quaternion(
                RelativeDestination.RotationX,
                relativeDestination.RotationY,
    ...
    public override ActivationResult CheckUseRequirements(WorldObject activator)
    {
        ...
        if (Destination == null)
        {
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"Portal destination for portal ID {WeenieClassId} not yet implemented!",
        ...
        if (player.LastPortalTeleportTimestamp != null)
        {
            ...
            if (timeSinceLastPortal < minTimeSinceLastPortal)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.YouHaveBeenTeleportedTooRecently)
                );
            }
        }
        ...
        if (!player.IgnorePortalRestrictions)
        {
            if (player.Level < MinLevel)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.YouAreNotPowerfulEnoughToUsePortal)
                );
            }
            ...
            if (player.Level > MaxLevel && MaxLevel != 0 && !player.EnchantmentManager.HasSpell(5379))
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.YouAreTooPowerfulToUsePortal)
                );
            }
            ...
            if (PortalRestrictions == PortalBitmask.Undef)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.PlayersMayNotUsePortal)
                );
            }
            ...
            if (
                PortalRestrictions.HasFlag(PortalBitmask.NoPKLite)
                && player.PlayerKillerStatus == PlayerKillerStatus.PKLite
            )
            {
                // Lite Player Killers may not interact with that portal!
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.PKLiteMayNotUsePortal)
                );
            }
            ...
            if (PortalRestrictions.HasFlag(PortalBitmask.NoNPK) && player.PlayerKillerStatus == PlayerKillerStatus.NPK)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.NonPKsMayNotUsePortal)
                );
            }
            ...
            if (PortalRestrictions.HasFlag(PortalBitmask.OnlyOlthoiPCs) && !player.IsOlthoiPlayer)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.OnlyOlthoiMayUsePortal)
                );
            }
            ...
            if ((PortalRestrictions.HasFlag(PortalBitmask.NoOlthoiPCs) || IsGateway) && player.IsOlthoiPlayer)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.OlthoiMayNotUsePortal)
                );
            }
            ...
            if (PortalRestrictions.HasFlag(PortalBitmask.NoVitae) && player.HasVitae)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.YouMayNotUsePortalWithVitae)
                );
            }
            ...
            if (PortalRestrictions.HasFlag(PortalBitmask.NoNewAccounts) && !player.Account15Days)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.YouMustBeTwoWeeksOldToUsePortal)
                );
            }
            ...
            if (player.AccountRequirements < AccountRequirements)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.MustPurchaseThroneOfDestinyToUsePortal)
                );
            }
            ...
            if ((AdvocateQuest ?? false) && !player.IsAdvocate)
            {
                ...
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.YouMustBeAnAdvocateToUsePortal)
                );
            }
        }
