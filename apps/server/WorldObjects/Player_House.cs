...
partial class Player
{
    ...
    public void HandleActionBuyHouse(uint slumlord_id, List<uint> item_ids)
    {
        ...
        if (houseInstance != null)
        {
            ...
            Session.Network.EnqueueSend(
                new GameMessageSystemChat("You already own a house!", ChatMessageType.Broadcast)
            );
            _log.Information(
                $"[HOUSE] {Name}.HandleActionBuyHouse(): Failed pre-purchase requirement - Already owns another house"
            );
            return;
        }
        ...
        if (slumlord == null)
        {
            _log.Error($"[HOUSE] {Name}.HandleActionBuyHouse: Couldn't find slumlord 0x{slumlord_id:X8}!");
            return;
        }
            ...
            if (playerLevel < slumlord.MinLevel)
            {
                Session.Network.EnqueueSend(
                    new GameEventWeenieErrorWithString(
                        ...
                        slumlord.MinLevel.ToString()
                    )
                );
                _log.Information($"[HOUSE] {Name}.HandleActionBuyHouse(): Failed pre-purchase requirement - MinLevel");
                return;
            }
        }
        ...
        if (slumlord.HouseRequiresMonarch)
        {
            ...
            {
                Session.Network.EnqueueSend(
                    new GameEventWeenieError(Session, WeenieError.YouMustBeMonarchToPurchaseDwelling)
                );
                _log.Information(
                    $"[HOUSE] {Name}.HandleActionBuyHouse(): Failed pre-purchase requirement - HouseRequiresMonarch"
                );
                return;
            }
        }
            ...
            if (allegianceMinLevel > 0 && (Allegiance == null || AllegianceNode.Rank < allegianceMinLevel))
            {
                Session.Network.EnqueueSend(
                    new GameEventWeenieErrorWithString(
                        ...
                        allegianceMinLevel.ToString()
                    )
                );
                _log.Information(
                    $"[HOUSE] {Name}.HandleActionBuyHouse(): Failed pre-purchase requirement - AllegianceMinLevel"
                );
                return;
            }
        }
        ...
        if (slumlord.House.HouseType != HouseType.Apartment)
        {
            if (PropertyManager.GetBool("house_15day_account").Item && !Account15Days)
            {
                ...
                {
                    var msg =
                        "Your account must be at least 15 days old to purchase this dwelling. This applies to all housing except apartments.";
                    Session.Network.EnqueueSend(
                        new GameEventCommunicationTransientString(Session, msg),
                        new GameMessageSystemChat(msg, ChatMessageType.Broadcast)
                    );
                    _log.Information(
                        $"[HOUSE] {Name}.HandleActionBuyHouse(): Failed pre-purchase requirement - house_15day_account"
                    );
                    return;
                }
            }
            ...
            if (PropertyManager.GetBool("house_30day_cooldown").Item)
            {
                ...
                {
                    Session.Network.EnqueueSend(
                        new GameEventWeenieError(Session, WeenieError.YouMustWaitToPurchaseHouse)
                    );
                    _log.Information(
                        $"[HOUSE] {Name}.HandleActionBuyHouse(): Failed pre-purchase requirement - house_30day_cooldown"
                    );
                    return;
                }
            }
        ...
        if (!verified)
        {
            _log.Warning($"[HOUSE] {Name} tried to purchase house {slumlord.Guid} without the required items!");
            return;
        }

        //Console.WriteLine("\nInventory check passed!");
        _log.Information($"[HOUSE] {Name}.HandleActionBuyHouse(): Inventory check passed!");
        ...
        if (!TryConsumePurchaseItems(consumeItems))
        {
            ...
            _log.Error(
                "[HOUSE] {Name}.HandleActionBuyHouse({SlumLordId:X8}, {ItemIds}) - TryConsumePurchaseItems failed with {ConsumeItemsList}",
                Name,
                slumlord_id,
                item_id_list,
                consumeItemsList
    ...
    public void GiveDeed(SlumLord slumLord)
    {
        ...
        if (location == null)
        {
            ...
            {
                _log.Error(
                    "{Name}.GiveDeed() - couldn't find location {SlumLordLocation}",
                    Name,
                    slumLord.Location.ToLOCString()
                );
            }
        }
    ...
    public void RemoveDeed()
    {
        ...
        if (deeds == null)
        {
            _log.Warning($"[HOUSE] {Name}.RemoveDeed(): couldn't find inventory deed");
            return;
        }
    ...
    public void HandleActionRentHouse(uint slumlord_id, List<uint> item_ids)
    {
        ...
        {
            _log.Error(
                "[HOUSE] {Name}.HandleActionRentHouse({SlumLordId:X8}): couldn't find house owner {SlumLordHouseOwner}",
                Name,
                slumlord_id,
                slumlord.HouseOwner
            );
        ...
        foreach (var item_id in item_ids)
        {
            ...
            if (IsTrading && item.IsBeingTradedOrContainsItemBeingTraded(ItemsInTradeWindow))
            {
                ...
                logLine +=
                    $"{stackStr}{item.Name} ({item.Guid}) is currently being traded, skipping." + Environment.NewLine;
                continue;
            }
            sentItems.Add(item);
        }
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat(
                $"Maintenance {(slumlord.IsRentPaid() ? "" : "partially ")}paid.",
                ChatMessageType.Broadcast
            )
        );
    }
    ...
    public bool TryConsumeItemForRent(SlumLord slumlord, WorldObjectInfo<int> itemInfo)
    {
        ...
        if (item == null)
        {
            _log.Error($"[HOUSE] {Name}.TryConsumeItemForRent({itemInfo.Guid:X8}) - couldn't get item");
            return false;
        }
        ...
        {
            _log.Error(
                $"[HOUSE] {Name}.TryConsumeItemForRent({item.Name} ({item.Guid}) - amount {amount} > stacksize {stackSize}"
            );
            return false;
        }
    ...
    public bool TryMoveItemForRent(SlumLord slumlord, WorldObject item)
    {
        ...
        {
            _log.Error(
                $"[HOUSE] {Name}.TryMoveItemForRent({slumlord.Name} ({slumlord.Guid}), {item.Name} ({item.Guid}) ) - CanAddToInventory failed!"
            );
            return false;
        }
        ...
        {
            _log.Error(
                $"[HOUSE] {Name}.TryMoveItemForRent({slumlord.Name} ({slumlord.Guid}), {item.Name} ({item.Guid}) ) - TryRemoveFromInventoryWithNetworking failed!"
            );
            return false;
        }
        ...
        {
            _log.Error(
                $"[HOUSE] {Name}.TryMoveItemForRent({slumlord.Name} ({slumlord.Guid}), {item.Name} ({item.Guid}) ) - TryAddToInventory failed!"
            );
            return false;
        }

        _log.Debug(
            "[HOUSE] {Name}.TryMoveItemForRent({SlumLordName} ({SlumLordGuid}), {StackSize}{ItemName} ({ItemGuid})) - Successfully moved to Slumlord.",
            Name,
            slumlord.Name,
            slumlord.Guid,
            (item.StackSize ?? 1) > 1 ? $"{item.StackSize}x " : "",
            item.Name,
            item.Guid
        );
        return true;
    ...
    public bool TrySplitItemForRent(SlumLord slumlord, WorldObject item, int amount)
    {
        ...
        {
            _log.Error(
                $"[HOUSE] {Name}.TrySplitItemForRent({slumlord.Name} ({slumlord.Guid}), {item.Name} ({item.Guid}), {amount}) - CanAddToInventory failed for split item {newItem.Name} ({newItem.Guid})!"
            );
            newItem.Destroy();
            return false;
        }
        ...
        {
            _log.Error(
                $"[HOUSE] {Name}.TrySplitItemForRent({slumlord.Name} ({slumlord.Guid}), {item.Name} ({item.Guid}), {amount}) - GetInventoryItem failed!"
            );
            newItem.Destroy();
            return false;
        }
        ...
        {
            _log.Error(
                $"[HOUSE] {Name}.TrySplitItemForRent({slumlord.Name} ({slumlord.Guid}), {item.Name} ({item.Guid}), {amount}) - failed to adjust stack!"
            );
            newItem.Destroy();
            return false;
        }
        ...
        {
            _log.Error(
                $"[HOUSE] {Name}.TrySplitItemForRent({slumlord.Name} ({slumlord.Guid}), {item.Name} ({item.Guid}), {amount}) - TryAddToInventory failed for split item {newItem.Name} ({newItem.Guid})!"
            );
            newItem.Destroy();
            return false;
        }
        ...
        _log.Debug(
            $"[HOUSE] {Name}.TrySplitItemForRent({slumlord.Name} ({slumlord.Guid}), {item.Name} ({item.Guid}), {amount}) - Created new item {((newItem.StackSize ?? 1) > 1 ? $"{newItem.StackSize}x " : "")}{newItem.Name} ({newItem.Guid}) and moved to Slumlord."
        );
    ...
    public void HandleActionAbandonHouse()
    {
        ...
        _log.Information($"[HOUSE] {Name}.HandleActionAbandonHouse()");
        ...
        if (HousePurchaseTimestamp != houseOwner.HousePurchaseTimestamp)
        {
            ...
            Session.Network.EnqueueSend(
                new GameMessagePrivateUpdatePropertyInt(
                    ...
                    HousePurchaseTimestamp ?? 0
                ),
                new GameMessageSystemChat("Updating housing information...", ChatMessageType.Broadcast),
                new GameEventHouseStatus(Session, WeenieError.HouseEvicted)
            );
        }
        ...
        actionChain.AddAction(
            ...
            () =>
            {
                ...
                if (
                    ...
                    && PropertyManager.GetBool("house_rent_enabled", true).Item
                )
                {
                    Session.Network.EnqueueSend(
                        new GameMessageSystemChat(
                ...
                if (
                    ...
                    && !House.SlumLord.HasRequirements(this)
                    && PropertyManager.GetBool("house_purchase_requirements").Item
                )
                {
                    var rankStr = AllegianceNode != null ? $"{AllegianceNode.Rank}" : "";
                    Session.Network.EnqueueSend(
                        new GameMessageSystemChat(
                            $"Warning!  Your allegiance rank {rankStr} is now below the requirements for owning a mansion.  Please raise your allegiance rank to {House.SlumLord.GetAllegianceMinLevel()} before the end of the maintenance period or you will lose your mansion, and all your items within it.",
                            ChatMessageType.System
                        )
                    );
                }
    ...
    public void SetHouseOwner(SlumLord slumlord)
    {
        ...
        _log.Information($"[HOUSE] Setting {Name} (0x{Guid}) as owner of {house.Name} (0x{house.Guid:X8})");
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat("Congratulations!  You now own this dwelling.", ChatMessageType.Broadcast)
        );
    ...
    public bool TryConsumePurchaseItems(List<WorldObjectInfo<int>> purchaseItems)
    {
        foreach (var purchaseItem in purchaseItems)
        {
            ...
            if (item == null)
            {
                ...
                _log.Error($"[HOUSE] {Name}.ConsumeItemsForHousePurchase(): couldn't find {purchaseItem.Guid}!");
                return false;
            }
            ...
            if (amount > stackSize)
            {
                ...
                _log.Error(
                    $"[HOUSE] {Name}.ConsumeItemsForHousePurchase(): {item.Name} ({item.Guid}) amount({amount}) > stackSize({stackSize})!"
                );
                return false;
            }
            ...
            if (!TryConsumeFromInventoryWithNetworking(item, amount))
            {
                ...
                _log.Error(
                    $"[HOUSE] {Name}.ConsumeItemsForHousePurchase(): TryConsumeFromInventoryWithNetworking({item.Name} ({item.Guid}), {amount}) failed!"
                );
                return false;
            }
    ...
    public bool HasItem(List<WorldObject> sentItems, WorldObject buyItem)
    {
        ...
        _log.Information($"[HOUSE] Checking for item: {stackStr}{buyItem.Name}");
        ...
        if (itemMatches.Count == 0)
        {
            ...
            _log.Information($"[HOUSE] No matching items found.");
            return false;
        }
        ...
        if (totalStack < required)
        {
            ...
            _log.Information($"[HOUSE] Found {totalStack} items, requires {required}.");
            return false;
        }
        return true;
    }
    ...
    public bool HasCurrency(List<WorldObject> sentItems, uint amount, bool useTradeNotes = true)
    {
        ...
        _log.Information($"[HOUSE] Checking for currency: {amount}");
    ...
    public uint GetTotalCurrency(List<WorldObject> sentItems, bool useTradeNotes = true)
    {
        ...
        _log.Information($"[HOUSE] Total pyreals: {totalPyreals}");
        if (!useTradeNotes)
        {
            return totalPyreals;
        }
        ...
        _log.Information($"[HOUSE] Total trade notes: {totalTradeNotes}");
    ...
    public void HandleActionAddGuest(string guestName)
    {
        ...
        var house = GetHouse();
        var guest = PlayerManager.FindByName(guestName, out var isOnline);
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat("You already have access to your house.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{guest.Name} is already on your guest list.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        if (Guests.Count == House.MaxGuests)
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"Your guest list has already reached the maximum limit ({House.MaxGuests})",
                    ChatMessageType.Broadcast
                )
            );
            return;
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat($"{guest.Name} has been added to your guest list.", ChatMessageType.Broadcast)
        );
        ...
        if (isOnline)
        {
            ...
            onlineGuest.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{Name} has added you to their house guest list.", ChatMessageType.Broadcast)
            );
        }
    }
    ...
    public void HandleActionRemoveGuest(string guestName, bool sendMsgToOwner = true)
    {
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat("You have permanent access to your house.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{guest.Name} is not on your guest list.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{guest.Name} removed from your guest list.", ChatMessageType.Broadcast)
            );
        }
        ...
        if (isOnline)
        {
            ...
            onlineGuest.Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"{Name} has removed you from their house guest list.",
                    ChatMessageType.Broadcast
                )
            );
    ...
    public void HandleActionRemoveAllGuests()
    {
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"Your house guest list is empty.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat($"You have removed all the guests from your house.", ChatMessageType.Broadcast)
        );
    }
    ...
    public void HandleActionSetOpenStatus(bool openStatus)
    {
        ...
        if (openStatus == house.OpenStatus)
        {
            ...
            {
                Session.Network.EnqueueSend(
                    new GameMessageSystemChat("You already have an open house.", ChatMessageType.Broadcast)
                );
            }
            else
            {
                Session.Network.EnqueueSend(
                    new GameMessageSystemChat("Your house is already closed to the public.", ChatMessageType.Broadcast)
                );
            }
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat("Your house is now open to the public.", ChatMessageType.Broadcast)
            );
        }
        else
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat("Your house is now closed to the public.", ChatMessageType.Broadcast)
            );
    ...
    public void HandleActionSetHooksVisible(bool visible)
    {
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat($"Your hooks are set to {visibleStr}.", ChatMessageType.Broadcast)
        );
    ...
    public void HandleActionModifyStorage(string guestName, bool hasPermission, bool sendMsgToOwner = true)
    {
        ...
        var house = GetHouse();
        var storage = PlayerManager.FindByName(guestName, out var isOnline);
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat("You have permanent access to your house storage.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        if (hasPermission)
        {
            ...
            else
            {
                if (storageAccess)
                {
                    Session.Network.EnqueueSend(
                        new GameMessageSystemChat(
                            $"{storage.Name} already has access to your home's storage.",
                            ChatMessageType.Broadcast
                        )
                    );
                    return;
            ...
            if (sendMsgToOwner)
            {
                Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"You have granted {storage.Name} access to your home's storage.  This is denoted by  the asterisk next to their name in the guest list.",
                        ChatMessageType.Broadcast
                    )
                ); // spacing from PCAP
            }
            ...
            if (isOnline)
            {
                ...
                onlineGuest.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"{Name} has granted you access to their house {andStr}storage.",
                        ChatMessageType.Broadcast
                    )
                );
            }
        ...
        else
        {
            if (!existing || !storageAccess)
            {
                ...
                Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"{storage.Name} doesn't have access to your house{storageStr}.",
                        ChatMessageType.Broadcast
                    )
                );
                return;
            ...
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"{storage.Name} no longer has access to your house storage.",
                    ChatMessageType.Broadcast
                )
            );
            ...
            if (isOnline)
            {
                ...
                onlineGuest.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"{Name} has revoked access to their house storage.",
                        ChatMessageType.Broadcast
                    )
                );
    ...
    public void HandleActionRemoveAllStorage()
    {
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"Your house guests don't have storage access.", ChatMessageType.Broadcast)
            );
            return;
        }
    ...
    public void HandleActionBoot(string playerName, bool allegianceHouse = false)
    {
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{playerName} is not online.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{player.Name} is not on {owner} property.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat($"Booted {player.Name} from {owner} house.", ChatMessageType.Broadcast)
        );

        owner = allegianceHouse ? "the allegiance" : "their";
        player.Session.Network.EnqueueSend(
            new GameMessageSystemChat($"{Name} has booted you from {owner} house.", ChatMessageType.Broadcast)
        );
    }
    ...
    public void HandleActionBootAll(bool guests = true)
    {
        ...
        if (guests && booted == 0)
        {
            ...
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"There is no one {elseStr}on your property.", ChatMessageType.Broadcast)
            );
        }
    }
    ...
    public void HandleActionModifyAllegianceStoragePermission(bool add)
    {
        ...
        if (add)
        {
            if (
                house.MonarchId != null
                && Guests.TryGetValue(new ObjectGuid(house.MonarchId.Value), out var storage)
                && storage
            )
            {
                Session.Network.EnqueueSend(
        ...
        else
        {
            if (
                house.MonarchId == null
                || Guests.TryGetValue(new ObjectGuid(house.MonarchId.Value), out var storage) && !storage
            )
            {
                Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
    ...
    public void HandleActionDoAllegianceHouseAction(AllegianceHouseAction action)
    {
        ...
        {
            Session.Network.EnqueueSend(
                new GameEventWeenieError(Session, WeenieError.YouDoNotHaveAuthorityInAllegiance)
            );
            return;
        }
        ...
        {
            Session.Network.EnqueueSend(
                new GameEventWeenieError(Session, WeenieError.YourMonarchDoesNotOwnAMansionOrVilla)
            );
            return;
        }
        ...
        if (action == AllegianceHouseAction.Help)
        {
            ...
            {
                allegianceHouse.Guests.TryGetValue(Allegiance.Monarch.PlayerGuid, out var storage);
                if (!storage)
                {
    ...
    public void HandleActionDoAllegianceHouseAction_GuestOpen(House allegianceHouse)
    {
        ...
        if (allegianceHouse.Guests.Count == House.MaxGuests)
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"The allegiance house guest list has already reached the maximum limit ({House.MaxGuests})",
                    ChatMessageType.Broadcast
                )
            );
            return;
    ...
    public void HandleActionDoAllegianceHouseAction_StorageOpen(House allegianceHouse)
    {
        if (
            allegianceHouse.MonarchId != null
            && allegianceHouse.Guests.TryGetValue(new ObjectGuid(allegianceHouse.MonarchId.Value), out var storage)
            && storage
        )
        {
            Session.Network.EnqueueSend(
        ...
        if (allegianceHouse.MonarchId == null && allegianceHouse.Guests.Count == House.MaxGuests)
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"Your allegiance house guest list has already reached the maximum limit ({House.MaxGuests})",
                    ChatMessageType.Broadcast
                )
            );
            return;
    ...
    public void ShowMultiHouseWarning(List<House> houses, string type)
    {
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat(
                $"Warning! You currently own {houses.Count} different houses on this {type}.",
                ChatMessageType.Broadcast
            )
        );
        Session.Network.EnqueueSend(
            new GameMessageSystemChat(
                $"Each {type} is only allowed to own 1 house, so you will have to choose which house you want to keep.",
                ChatMessageType.Broadcast
            )
        );
        Session.Network.EnqueueSend(
            new GameMessageSystemChat($"You currently own houses at:", ChatMessageType.Broadcast)
        );
        ...
        for (var i = 0; i < houses.Count; i++)
        {
            ...
            if (slumlord == null)
            {
                _log.Error($"{Name}.IsMultiHouseOwner(): {house.Guid} slumlord is null!");
                continue;
            }
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat(
                $"Please choose the house you want to keep with /house-select # , where # is 1-{houses.Count}",
                ChatMessageType.Broadcast
            )
        );
    }
