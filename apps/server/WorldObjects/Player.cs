...
public partial class Player : Creature, IPlayer
{
    ...
    /// </summary>
    public Player(Weenie weenie, ObjectGuid guid, uint accountId)
        : base(weenie, guid)
    {
    ...
    public Player(
        Biota biota,
        IEnumerable<ACE.Database.Models.Shard.Biota> inventory,
        IEnumerable<ACE.Database.Models.Shard.Biota> wieldedItems,
        Character character,
        Session session
    )
        : base(biota)
    ...
    private void SetEphemeralValues()
    {
        ...
        IsGearKnightPlayer =
            PropertyManager.GetBool("gearknight_core_plating").Item && HeritageGroup == HeritageGroup.Gearknight;
    ...
    public void Examine(WorldObject obj)
    {
        ...
        if (creature != null)
        {
            ...
            if (
                difficulty == 0
                || player == this
                || player != null && !player.GetCharacterOption(CharacterOption.AttemptToDeceiveOtherPlayers)
            )
            {
                chance = 1.0f;
            }
    ...
    public void OnAppraisal(WorldObject obj, bool success)
    {
        if (
            ...
            && !player.SquelchManager.Squelches.Contains(this, ChatMessageType.Appraisal)
        )
        {
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{Name} tried and failed to assess you!", ChatMessageType.Appraisal)
            );
        }
    ...
    public bool LogOut(bool clientSessionTerminatedAbruptly = false, bool forceImmediate = false)
    {
        if (PKLogoutActive && !forceImmediate)
        {
            ...
            Session.Network.EnqueueSend(
                new GameMessageSystemChat("Beginning delayed player killer logoff...", ChatMessageType.Broadcast)
            );
    ...
    public void HandleMRT()
    {
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat(
                $"Bypass Housing Barriers now set to: {IgnoreHouseBarriers}",
    ...
    public void HandleActionTalk(string message)
    {
        ...
        {
            EnqueueBroadcast(
                new GameMessageHearSpeech(message, GetNameWithSuffix(), Guid.Full, ChatMessageType.Speech),
                LocalBroadcastRange,
    ...
    public void SendGagError()
    {
        ...
        Session.Network.EnqueueSend(
            ...
            new GameMessageSystemChat(msg, ChatMessageType.WorldBroadcast)
        );
    }
    ...
    public bool HandleRunRateUpdate()
    {
        ...
        var changed =
            currentState.ForwardSpeed != prevState.ForwardSpeed
    ...
    public void HandleAdminvisionToggle(int choice)
    {
        ...
        {
            var adminObjs = PhysicsObj.ObjMaint.GetKnownObjectsValuesWhere(o =>
                o.WeenieObj.WorldObject != null && o.WeenieObj.WorldObject.Visibility
            );
            PhysicsObj.enqueue_objs(adminObjs);

            var nodrawObjs = PhysicsObj.ObjMaint.GetKnownObjectsValuesWhere(o =>
            ...
            {
                Session.Network.EnqueueSend(
                    new GameMessageUpdateObject(wo.WeenieObj.WorldObject, Adminvision, Adminvision ? true : false)
                );
            }
        ...
        var state = Adminvision ? "enabled" : "disabled";
    ...
    public void HandleActionEnterPkLite()
    {
        ...
        actionChain.AddAction(
            ...
            () =>
            {
                ...
                EnqueueBroadcast(
                    new GameMessageSystemChat($"{Name} is looking for a fight!", ChatMessageType.Broadcast),
                    LocalBroadcastRange
                );
                ...
                animTime = DatManager
                    ...
                    .GetAnimationLength(MotionCommand.EnterPKLite);
                innerChain.AddDelaySeconds(animTime);
                innerChain.AddAction(
                    this,
                    () =>
                    {
                        IsBusy = false;
