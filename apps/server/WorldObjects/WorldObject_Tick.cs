...
partial class WorldObject
{
    ...
    private void InitializeHeartbeats()
    {
        ...
        {
            _log.Warning(
                $"{Name} ({Guid}).InitializeHeartBeats() - RegenerationInterval {RegenerationInterval}, setting to 0"
            );
        ...
        if (CurrentLandblock == null)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        _log.Warning(
                            "Item 0x{ItemGuid:X8}:{Item} has enqueued an action but is not attached to a landblock.",
                            Guid.Full,
                            Name
                        );
                    }
    ...
    public virtual bool UpdateObjectPhysics()
    {
        ...
        if (this is Creature creature)
        {
            ...
            var runUpdate =
                PhysicsObj.IsAnimating && (!creature.IsMonster || !creature.IsAwake)
                || isDying
        ...
        try
        {
            ...
            if (this is SpellProjectile spellProjectile)
            {
                if (spellProjectile.Velocity == Vector3.Zero && spellProjectile.DebugVelocity < 30)
                {
                    ...
                    {
                        _log.Error(
                            $"Spell projectile w/ zero velocity detected @ {spellProjectile.Location.ToLOCString()}, launched by {spellProjectile.ProjectileSource?.Name} ({spellProjectile.ProjectileSource?.Guid}), spell ID {spellProjectile.Spell?.Id} - {spellProjectile.Spell?.Name}"
                        );
                    }
                }
        ...
        finally
        {
            ...
            ServerPerformanceMonitor.AddToCumulativeEvent(
                ServerPerformanceMonitor.CumulativeEventHistoryType.WorldObject_Tick_UpdateObjectPhysics,
                elapsedSeconds
            );
            ...
            else if (elapsedSeconds >= 0.010)
            {
                ...
                if (slowUpdateObjectPhysicsHits >= 5 && this is SpellProjectile spellProjectile)
                {
                    ...
                    _log.Warning(
                        $"[PERFORMANCE][PHYSICS] {Guid}:{Name} took {(elapsedSeconds * 1000):N1} ms to process UpdateObjectPhysics() at loc: {Location}. SpellProjectile destroyed."
                    );
                }
                else
                {
                    _log.Debug(
                        "[PERFORMANCE][PHYSICS] {Guid}:{Name} took {TimeElapsed:N1} ms to process UpdateObjectPhysics() at loc: {Location}",
                        Guid,
                        Name,
                        (elapsedSeconds * 1000),
                        Location
