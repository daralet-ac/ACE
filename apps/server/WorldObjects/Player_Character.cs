...
partial class Player
{
    ...
    {
        var newSetting = !GetCharacterOptions2(CharacterOptions2.NotUsed1);
    ...
    public void HandleActionAddFriend(string friendName)
    {
        ...
        Session.Network.EnqueueSend(
            new GameEventFriendsListUpdate(
                Session,
                GameEventFriendsListUpdate.FriendsUpdateTypeFlag.FriendAdded,
                newFriend
            )
        );
    ...
    public void HandleActionRemoveFriend(uint friendGuid)
    {
        ...
        Session.Network.EnqueueSend(
            new GameEventFriendsListUpdate(
                Session,
                GameEventFriendsListUpdate.FriendsUpdateTypeFlag.FriendRemoved,
                friendToRemove
            )
        );
    ...
    public void AddTitle(uint titleId, bool setAsDisplayTitle = false)
    {
        ...
        Character.AddTitleToRegistry(
            titleId,
            CharacterDatabaseLock,
            out var titleAlreadyExists,
            out var numCharacterTitles
        );

        var sendMsg = false;
        var notifyNewTitle = false;
        ...
        if (sendMsg && FirstEnterWorldDone)
        {
            ...
            {
                Session.Network.EnqueueSend(
                    new GameEventCommunicationTransientString(Session, "You have been granted a new title.")
                );
            }
        }
    }
    ...
    public void HandleActionFinishBarber(ClientMessage message)
    {
        ...
        var option_bound = message.Payload.ReadUInt32(); // Supress Levitation - Empyrean Only
        var option_unk = message.Payload.ReadUInt32(); // Unknown - Possibly set aside for future use?
        ...
        var validEyesTexture = ValidateEyeTexture(
            requestedEyesTextureDID,
            sex.EyeStripList,
            isBald: validatedHairStyle?.Bald ?? false
        );

        var validDefaultEyesTexture = ValidateEyeTexture(
            requestedDefaultEyesTextureDID,
            sex.EyeStripList,
            compareOldTexture: true,
            isBald: validatedHairStyle?.Bald ?? false
        );

        var validNoseTexture = ValidateFaceTexture(requestedNoseTextureDID, sex.NoseStripList);

        var validDefaultNoseTexture = ValidateFaceTexture(
            requestedDefaultNoseTextureDID,
            sex.NoseStripList,
            compareOldTexture: true
        );
        ...
        var validDefaultMouthTexture = ValidateFaceTexture(
            requestedDefaultMouthTextureDID,
            sex.MouthStripList,
            compareOldTexture: true
        );
        ...
        var validCharacterDefaultHairTexture = ValidateHairTexture(
            requestedCharacterDefaultHairTexture,
            validatedHairStyle,
            compareOldTexture: true
        );
        ...
        var validChangeRequested =
            validPaletteBase
            && validHeadObject
            && validEyesTexture
            && validDefaultEyesTexture
            && validEyesPalette
            && validNoseTexture
            && validDefaultNoseTexture
            && validMouthTexture
            && validDefaultMouthTexture
            && validCharacterHairTexture
            && validCharacterDefaultHairTexture
            && validHairPalette
            && validSkinPalette
            && validSetupTable;
        ...
        if (Heritage == (int)HeritageGroup.Empyrean)
        {
            ...
            if (Gender == (int)ACE.Entity.Enum.Gender.Male) // Male
            {
                if (option_bound == 1 && MotionTableId != EmpyreanMaleMotionDID)
                {
                    ...
                    Session.Network.EnqueueSend(
                        new GameMessagePrivateUpdateDataID(this, PropertyDataId.MotionTable, MotionTableId)
                    );
                }
    ...
    private bool ValidateSetupTable(
        uint requestedSetupTableId,
        HeritageGroupCG heritageGroup,
        SexCG sex,
        HairStyleCG validHairStyle
    )
    {
        ...
        if (
            ValidHeritageSetups.TryGetValue((HeritageGroup)Heritage, out var genders)
            && genders.TryGetValue((Gender)Gender, out var gender)
            && gender.Contains(requestedSetupTableId)
        )
        {
            return true;
        }
    ...
    /// </summary>
    private static readonly Dictionary<HeritageGroup, Dictionary<Gender, List<uint>>> ValidHeritageSetups =
        new()
        {
            {
                ...
                {
                    {
                        ACE.Entity.Enum.Gender.Male,
                        new() { (uint)SetupConst.UmbraenMaleCrown, (uint)SetupConst.UmbraenMaleNoCrown }
                    },
                    {
                        ACE.Entity.Enum.Gender.Female,
                        new() { (uint)SetupConst.UmbraenFemaleCrown, (uint)SetupConst.UmbraenFemaleNoCrown }
                    }
                }
            },
            {
                ...
                new()
                {
                    {
                        ACE.Entity.Enum.Gender.Male,
                        new() { (uint)SetupConst.PenumbraenMaleCrown, (uint)SetupConst.PenumbraenMaleNoCrown }
                    },
                    {
                        ACE.Entity.Enum.Gender.Female,
                        new() { (uint)SetupConst.PenumbraenFemaleCrown, (uint)SetupConst.PenumbraenFemaleNoCrown }
                    }
                }
            },
            {
                ...
                {
                    {
                        ...
                        new()
                        {
                            (uint)SetupConst.UndeadMaleSkeleton,
                            (uint)SetupConst.UndeadMaleSkeletonNoFlame,
                            (uint)SetupConst.UndeadMaleZombie,
                            (uint)SetupConst.UndeadMaleZombieNoFlame
                        }
                    },
                    {
                        ACE.Entity.Enum.Gender.Female,
                        new()
                        {
                            (uint)SetupConst.UndeadFemaleSkeleton,
                            (uint)SetupConst.UndeadFemaleSkeletonNoFlame,
                            (uint)SetupConst.UndeadFemaleZombie,
                            (uint)SetupConst.UndeadFemaleZombieNoFlame
                        }
                    }
                }
            }
    ...
    private bool ValidateHairStyle(
        uint requestedHeadObjectDID,
        List<HairStyleCG> hairStyleList,
        out HairStyleCG validHairStyle
    )
    {
        validHairStyle = null;
        ...
        {
            if (
                Heritage == (int)HeritageGroup.Gearknight
                || Heritage == (int)HeritageGroup.Olthoi
                || Heritage == (int)HeritageGroup.OlthoiAcid
            )
            {
                return true;
            }
    ...
    private bool ValidateEyeTexture(
        uint requestedEyesTextureDID,
        List<EyeStripCG> eyeStripList,
        bool compareOldTexture = false,
        bool isBald = false
    )
    {
    ...
    private bool ValidateFaceTexture(
        uint requestedFaceTextureDID,
        List<FaceStripCG> faceStripList,
        bool compareOldTexture = false
    )
    {
    ...
    private bool ValidateHairTexture(
        uint requestedHairTextureDID,
        HairStyleCG hairStyle,
        bool compareOldTexture = false
    )
    {
