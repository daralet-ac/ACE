...
partial class WorldObject
{
    ...
    private void SerializeCreateObject(
        BinaryWriter writer,
        bool gamedataonly,
        bool adminvision = false,
        bool changenodraw = false
    )
    {
        writer.WriteGuid(Guid);
        ...
        foreach (var property in properties)
        {
            switch (property.PropertyType)
            {
                case PropertyType.PropertyInt:
                    var value = GetProperty((PropertyInt)property.PropertyId);
                    if (value != null)
                    {
                ...
                default:
                    _log.Debug(
                        "Unsupported property in SendPartialUpdates: id {PropertyId}, type {PropertyType}.",
                        property.PropertyId,
    ...
    private void UpdateObjectDescriptionFlags()
    {
        ...
        )
        {
            ...
            if (
                WeenieType == WeenieType.Chest
                && !openable
                && PropertyManager.GetBool("fix_chest_missing_inventory_window").Item
            )
            {
                openable = true;
            }
    ...
    public bool? IgnoreCloIcons
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyBool.IgnoreCloIcons);
            }
            else
            {
                SetProperty(PropertyBool.IgnoreCloIcons, value.Value);
            }
        }
    }
    ...
    public bool? Dyable
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyBool.Dyable);
            }
            else
            {
                SetProperty(PropertyBool.Dyable, value.Value);
            }
        }
    }
    ...
    public virtual ACE.Entity.ObjDesc CalculateObjDesc()
    {
        ...
        // Check if the ClothingBase is applicable for this Setup. (Gear Knights, this is usually you.)
        {
            ...
            var clothingBaseEffect = item.ClothingBaseEffects[SetupTableId];
            foreach (var t in clothingBaseEffect.CloObjectEffects)
            {
                var partNum = (byte)t.Index;
                ...
                {
                    objDesc.TextureChanges.Add(
                        new PropertiesTextureMap
                        {
                            PartIndex = (byte)t.Index,
                            OldTexture = t1.OldTexture,
                            NewTexture = t1.NewTexture
                        }
                    );
                }
            }
            ...
            if (item.ClothingSubPalEffects.Count > 0 && (Shade.HasValue || PaletteTemplate.HasValue))
            {
                ...
                for (var i = 0; i < itemSubPal.CloSubPalettes.Count; i++)
                {
                    var itemPalSet = DatManager.PortalDat.ReadFromDat<PaletteSet>(
                        itemSubPal.CloSubPalettes[i].PaletteSet
                    );
                    var itemPal = (ushort)itemPalSet.GetPaletteID(shade);

                    for (var j = 0; j < itemSubPal.CloSubPalettes[i].Ranges.Count; j++)
                    {
                        var palOffset = (ushort)(itemSubPal.CloSubPalettes[i].Ranges[j].Offset / 8);
                        var numColors = (ushort)(itemSubPal.CloSubPalettes[i].Ranges[j].NumColors / 8);
                        objDesc.SubPalettes.Add(
                            new PropertiesPalette
                            {
                                SubPaletteId = itemPal,
                                Offset = palOffset,
                                Length = numColors
                            }
                        );
    ...
    protected void AddBaseModelData(ACE.Entity.ObjDesc objDesc)
    {
        ...
        else if (HairStyle.HasValue && Heritage.HasValue && Gender.HasValue)
        {
            ...
            var sex = cg.HeritageGroups[(uint)Heritage].Genders[(int)Gender];
            if (sex.HairStyleList.Count > (int)HairStyle) // just check for a valid entry...
            {
                var hairstyle = sex.HairStyleList[(int)HairStyle];
                ...
                {
                    objDesc.AnimPartChanges.Add(
                        new PropertiesAnimPart { Index = part.PartIndex, AnimationId = part.PartID }
                    );
                }
            }
        }
        ...
        if (this is Player player)
        {
            objDesc.TextureChanges.Add(
                ...
                {
                    PartIndex = 0x10,
                    OldTexture = player.Character.DefaultHairTexture,
                    NewTexture = player.Character.HairTexture
                }
            );
        }
        ...
        if (HairPaletteDID.HasValue)
        {
            objDesc.SubPalettes.Add(
                new PropertiesPalette
                {
                    SubPaletteId = HairPaletteDID.Value,
                    Offset = 0x18,
                    Length = 0x8
                }
            );
        ...
        if (SkinPaletteDID.HasValue)
        {
            objDesc.SubPalettes.Add(
                new PropertiesPalette
                {
                    SubPaletteId = SkinPaletteDID.Value,
                    Offset = 0x0,
                    Length = 0x18
                }
            );
        ...
        if (DefaultEyesTextureDID.HasValue && EyesTextureDID.HasValue)
        {
            objDesc.TextureChanges.Add(
                ...
                {
                    PartIndex = 0x10,
                    OldTexture = DefaultEyesTextureDID.Value,
                    NewTexture = EyesTextureDID.Value
                }
            );
        }
        ...
        if (EyesPaletteDID.HasValue)
        {
            objDesc.SubPalettes.Add(
                new PropertiesPalette
                {
                    SubPaletteId = EyesPaletteDID.Value,
                    Offset = 0x20,
                    Length = 0x8
                }
            );
        ...
        if (DefaultNoseTextureDID.HasValue && NoseTextureDID.HasValue)
        {
            objDesc.TextureChanges.Add(
                ...
                {
                    PartIndex = 0x10,
                    OldTexture = DefaultNoseTextureDID.Value,
                    NewTexture = NoseTextureDID.Value
                }
            );
        }
        ...
        if (DefaultMouthTextureDID.HasValue && MouthTextureDID.HasValue)
        {
            objDesc.TextureChanges.Add(
                ...
                {
                    PartIndex = 0x10,
                    OldTexture = DefaultMouthTextureDID.Value,
                    NewTexture = MouthTextureDID.Value
                }
            );
        }
    ...
    public float EnqueueMotion(
        ActionChain actionChain,
        MotionCommand motionCommand,
        float speed = 1.0f,
        bool useStance = true,
        MotionCommand? prevCommand = null,
        bool castGesture = false,
        bool half = false
    )
    {
        ...
        if (prevCommand != null)
        {
            animLength = Physics.Animation.MotionTable.GetAnimationLength(
                MotionTableId,
                stance,
                prevCommand.Value,
                motionCommand,
    ...
    )
    {
        ...
        actionChain.AddAction(
            ...
            () =>
            {
                ...
                if (this is Player player && player.CombatMode != CombatMode.Missile)
                {
                    return;
                }
    ...
    public float EnqueueMotionPersist(
        ActionChain actionChain,
        MotionStance stance,
        MotionCommand motionCommand,
        float speed = 1.0f
    ...
    public float EnqueueMotionAction(
        ActionChain actionChain,
        List<MotionCommand> motionCommands,
        float speed = 1.0f,
        MotionStance? useStance = null,
        bool usePrevCommand = false,
        bool checkCasting = false
    )
    {
    ...
    public float EnqueueMotion_Force(
        ActionChain actionChain,
        MotionStance stance,
        MotionCommand motionCommand,
        MotionCommand? prevCommand = null,
    ...
    public bool PlayersInRange(float range = 96.0f)
    {
        var isDungeon =
            ...
            && CurrentLandblock.PhysicsLandblock != null
            && CurrentLandblock.PhysicsLandblock.IsDungeon;
    ...
    {
        if (PhysicsObj == null || CurrentLandblock == null)
        {
            return;
        }
        ...
        foreach (var player in PhysicsObj.ObjMaint.GetKnownPlayersValuesAsPlayer())
        {
            if (
                self != null
                && squelchType != null
                && player.SquelchManager.Squelches.Contains(self, squelchType.Value)
            )
            {
                continue;
            }
