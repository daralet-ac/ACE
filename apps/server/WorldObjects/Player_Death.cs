...
partial class Player
{
    ...
    public void HandlePKDeathBroadcast(DamageHistoryInfo lastDamager, DamageHistoryInfo topDamager)
    {
        ...
        if (IsPKDeath(topDamager))
        {
            ...
            // Hotspots generated by player attack effects (Blazing Brand, etc) should credit the
            // owning player in this broadcast, rather than showing the hotspot's own name.
            var killerName = lastDamager.HotspotOwner != null
                ? lastDamager.TryGetHotspotOwner()?.Name ?? lastDamager.Name
                : lastDamager.Name;
            ...
            new ActionEventDelegate(() =>
            {
                ...
                teleportChain.AddAction(
                    ...
                    () =>
                    {
                        ...
                        var msgHealthUpdate = new GameMessagePrivateUpdateAttribute2ndLevel(
                            this,
                            Vital.Health,
                            newHealth
                        );
                        var msgStaminaUpdate = new GameMessagePrivateUpdateAttribute2ndLevel(
                            this,
                            Vital.Stamina,
                            newStamina
                        );
    ...
    private void HandleSuicide(int numDeaths, int step = 0)
    {
        ...
        {
            EnqueueBroadcast(
                new GameMessageHearSpeech(
                    SuicideMessages[step],
                    GetNameWithSuffix(),
                    Guid.Full,
    ...
    public List<WorldObject> CalculateDeathItems(Corpse corpse)
    {
        ...
        for (var i = 0; i < numItemsDropped && i < sorted.Inventory.Count; i++)
        {
            ...
            if ((deathItem.WorldObject.StackSize ?? 1) > 1)
            {
                ...
                {
                    _log.Warning(
                        "Couldn't find death item stack 0x{WorldObjectGuid:X8}:{WorldObject} for player {Player}",
                        deathItem.WorldObject.Guid.Full,
                        deathItem.WorldObject.Name,
                        Name
                    );
                }
            }
            else
            {
                if (
                    ...
                    || TryDequipObjectWithNetworking(
                        deathItem.WorldObject.Guid,
                        out _,
                        DequipObjectAction.ToCorpseOnDeath
                    )
                )
                {
                ...
                {
                    _log.Warning(
                        "Couldn't find death item 0x{WorldObjectGuid:X8}:{WorldObject} for player {Player}",
                        deathItem.WorldObject.Guid.Full,
                        deathItem.WorldObject.Name,
                        Name
                    );
        ...
        {
            if (
                TryRemoveFromInventoryWithNetworking(item.Guid, out _, RemoveFromInventoryAction.ToCorpseOnDeath)
                || TryDequipObjectWithNetworking(item.Guid, out _, DequipObjectAction.ToCorpseOnDeath)
            )
            {
                dropItems.Add(item);
            }
    ...
    public void DeathItemLog(List<WorldObject> dropItems, Corpse corpse)
    {
        ...
        {
            msg +=
                $"{(dropItem.StackSize.HasValue && dropItem.StackSize > 1 ? dropItem.StackSize.Value.ToString("N0") + " " + dropItem.GetPluralName() : dropItem.Name)} (0x{dropItem.Guid}){(dropItem.WeenieClassId == 273 && PropertyManager.GetBool("corpse_destroy_pyreals").Item ? $" which {(dropItem.StackSize.HasValue && dropItem.StackSize > 1 ? "were" : "was")} destroyed" : "")}, ";
        }
    ...
    public void HandleActionAddPlayerPermission(string playerName)
    {
        ...
        if (Name.Equals(player.Name))
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"You already have permission to loot your corpse.",
                    ChatMessageType.Broadcast
                )
            );
            return;
        ...
        if (!player.GetCharacterOption(CharacterOption.AcceptCorpseLootingPermissions))
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"{player.Name} is not accepting corpse looting permissions from other players.",
                    ChatMessageType.Broadcast
                )
            );
            return;
        ...
        if (player.HasLootPermission(Guid))
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"{player.Name} already has permission to loot your corpse.",
                    ChatMessageType.Broadcast
                )
            );
            return;
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat(
                $"You have given permission to {player.Name} to loot one of your corpses. This permission will last one hour.",
                ChatMessageType.Broadcast
            )
        );
    }
    ...
    public void HandleActionRemovePlayerPermission(string playerName)
    {
        ...
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"You always have permission to loot your corpse.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        if (player == null || !player.HasLootPermission(Guid))
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"{player.Name} doesn't have permission to loot your corpse.",
                    ChatMessageType.Broadcast
                )
            );
            return;
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat(
                $"{player.Name}'s permission to loot your corpse has been revoked.",
                ChatMessageType.Broadcast
            )
        );
    }
    ...
    public void HandleActionDisplayPlayerConsentList()
    {
        ...
        foreach (var playerGuid in LootPermission.Keys)
        {
            ...
            {
                Console.WriteLine(
                    $"{Name}.HandleActionDisplayPlayerConsentList(): couldn't find player guid {playerGuid}"
                );
                continue;
            }
    ...
    public void HandleActionRemoveFromPlayerConsentList(string playerName)
    {
        ...
        if (!HasLootPermission(player.Guid))
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"You don't have permission to loot {player.Name}'s corpse.",
                    ChatMessageType.Broadcast
                )
            );
            return;
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat(
                $"You have removed your permissions to loot {player.Name}'s corpse.",
                ChatMessageType.Broadcast
            )
        );
    }
    ...
    public bool UnderLifestoneProtection
    {
        ...
        set
        {
            if (!value)
            {
                RemoveProperty(PropertyBool.UnderLifestoneProtection);
            }
            else
            {
                SetProperty(PropertyBool.UnderLifestoneProtection, value);
            }
        }
    }
    ...
    public double? LifestoneProtectionTimestamp
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyFloat.LifestoneProtectionTimestamp);
            }
            else
            {
                SetProperty(PropertyFloat.LifestoneProtectionTimestamp, value.Value);
            }
        }
    }
    ...
    public void LifestoneProtectionTick()
    {
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat("You're no longer protected by the Lifestone's magic!", ChatMessageType.Magic)
        );
    }
    ...
    public void LifestoneProtectionDispel()
    {
        ...
        Session.Network.EnqueueSend(
            new GameMessageSystemChat("Your actions have dispelled the Lifestone's magic!", ChatMessageType.Magic)
        );
    }
    ...
    public double? MinimumTimeSincePk
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyFloat.MinimumTimeSincePk);
            }
            else
            {
                SetProperty(PropertyFloat.MinimumTimeSincePk, value.Value);
            }
        }
    }
    ...
    public void SetMinimumTimeSincePK()
    {
        ...
        {
            EnqueueBroadcast(
                new GameMessagePublicUpdatePropertyInt(this, PropertyInt.PlayerKillerStatus, (int)PlayerKillerStatus)
            );
    ...
    public void PK_DeathTick()
    {
        if (
            ...
            || (PropertyManager.GetBool("pk_server_safe_training_academy").Item && RecallsDisabled)
        )
        {
            return;
        }
        ...
        if (
            PkLevel == PKLevel.NPK
            && !PropertyManager.GetBool("pk_server").Item
            && !PropertyManager.GetBool("pkl_server").Item
        )
        {
            MinimumTimeSincePk = null;
            return;
    ...
    private static Database.Models.World.TreasureDeath OlthoiDeathTreasureType =>
        Database.DatabaseManager.World.GetCachedDeathTreasure(2222)
        ?? new()
        {
            TreasureType = 2222,
            Tier = 8,
    ...
    /// </summary>
    public List<WorldObject> CalculateDeathItems_Olthoi(
        Corpse corpse,
        bool hadVitae,
        bool killerIsOlthoiPlayer,
        bool killerIsPkPlayer
    )
    {
