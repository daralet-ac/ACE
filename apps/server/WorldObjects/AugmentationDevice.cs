...
public class AugmentationDevice : WorldObject
{
    ...
    /// </summary>
    public static bool AttributeAugmentationSafetyCapEnabled =>
    ...
    /// </summary>
    public AugmentationDevice(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    /// </summary>
    public AugmentationDevice(Biota biota)
        : base(biota)
    {
        SetEphemeralValues();
    }
        ...
        if (!confirmed)
        {
            if (
                !player.ConfirmationManager.EnqueueSend(
                    new Confirmation_Augmentation(player.Guid, Guid),
    ...
    public void DoAugmentation(Player player)
    {
        ...
        if (AugTypeHelper.IsAttribute(type))
        {
            ...
            playerAttr.StartingValue += AttributeAugmentationSafetyCapEnabled
                ? Math.Min(5, 100 - playerAttr.StartingValue)
                : 5;
        ...
        else if (type == AugmentationType.BurdenLimit)
        {
            ...
            player.Session.Network.EnqueueSend(
                new GameMessagePrivateUpdatePropertyInt(player, PropertyInt.EncumbranceCapacity, capacity)
            );
        }
        ...
        var updateXP = new GameMessagePrivateUpdatePropertyInt64(
            player,
        ...
        player.EnqueueBroadcast(
            new GameMessageSystemChat($"{player.Name} has acquired the {Name} augmentation!", ChatMessageType.Broadcast)
        );
    ...
    public bool VerifyRequirements(Player player)
    {
        ...
        {
            player.EnqueueBroadcast(
                new GameMessageSystemChat($"{Name} is missing AugmentationCost", ChatMessageType.System)
            );
            return false;
        }
        ...
        if (AugTypeHelper.IsAttribute(type))
        {
            ...
            {
                player.SendWeenieErrorWithString(
                    WeenieErrorWithString.AugmentationSkillNotTrained,
                    $"You are not able to purchase this augmentation because your {playerAttribute.Attribute.ToString()} is already at the maximum innate level!"
                );
                return false;
            }
        }
        else if (AugTypeHelper.IsSkill(type))
        {
            ...
            if (playerSkill.AdvancementClass != SkillAdvancementClass.Trained)
            {
                player.SendWeenieErrorWithString(
                    WeenieErrorWithString.AugmentationSkillNotTrained,
                    $"You are not able to purchase this augmentation because you are not trained in {playerSkill.Skill.ToSentence()}!"
                );
                return false;
            }
        }
