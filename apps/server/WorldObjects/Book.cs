...
public sealed class Book : WorldObject
{
    ...
    /// </summary>
    public Book(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
    ...
    )
    {
        ...
        {
            SetProperty(PropertyString.ShortDesc, shortDesc);
        }

        if (!string.IsNullOrEmpty(inscription))
        {
            SetProperty(PropertyString.Inscription, inscription);
        }

        if (!string.IsNullOrEmpty(scribeName))
        {
            SetProperty(PropertyString.ScribeName, scribeName);
        }

        if (!string.IsNullOrEmpty(scribeAccount))
        {
            SetProperty(PropertyString.ScribeAccount, scribeAccount);
        }
    }
    ...
    public override void ActOnUse(WorldObject worldObject)
    {
        ...
        var maxChars = Biota.PropertiesBook.MaxNumCharsPerPage;
        var maxPages = Biota.PropertiesBook.MaxNumPages;
        ...
        var authorID = (ScribeIID.HasValue) ? (uint)ScribeIID : 0xFFFFFFFF;

        var pages = Biota.PropertiesBookPageData.Clone(BiotaDatabaseLock);

        var ignoreAuthor = IgnoreAuthor ?? false;
        ...
        var bookDataResponse = new GameEventBookDataResponse(
            player.Session,
            Guid.Full,
            maxChars,
            maxPages,
    ...
    public PropertiesBookPageData AddPage(
        uint authorId,
        string authorName,
        string authorAccount,
        bool ignoreAuthor,
    ...
    public bool ModifyPage(int index, string pageText, Player player)
    {
        ...
        if (
            ...
            || (
                ...
                && player.Name == page.AuthorName
                && player.Account.AccountName == page.AuthorAccount
            )
            || player is Sentinel
            || player is Admin
        )
    ...
    public bool DeletePage(int index, Player player)
    {
        ...
        if (
            page == null
            || (!page.IgnoreAuthor && player.Guid.Full != page.AuthorId && !(player is Sentinel) && !(player is Admin))
        )
        {
            return false;
        }
