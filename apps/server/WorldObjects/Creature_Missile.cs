...
partial class Creature
{
    public float ReloadMissileAmmo(ActionChain actionChain = null)
    {
        ...
        var animLength2 = Physics.Animation.MotionTable.GetAnimationLength(
            MotionTableId,
    ...
    /// </summary>
    public WorldObject LaunchProjectile(
        WorldObject weapon,
        WorldObject ammo,
        WorldObject target,
        Vector3 origin,
    ...
    )
    {
            ...
            if (!proj.HitMsg)
            {
                if (player != null)
                {
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat("Your missile attack hit the environment.", ChatMessageType.Broadcast)
                    );
                }
            }
        ...
        proj.EnqueueBroadcast(
            new GameMessagePublicUpdatePropertyInt(proj, PropertyInt.PlayerKillerStatus, (int)pkStatus)
        );
    ...
    public Vector3 GetAimVelocity(WorldObject target, float projectileSpeed)
    {
        ...
        var velocity = GetProjectileVelocity(target, origin, dir, dest, projectileSpeed, out var time);
    ...
    )
    {
        ...
        var velocity = GetProjectileVelocity(target, startPos, dir, endPos, projectileSpeed, out var time);
    ...
    )
    {
        ...
        if (!targetVelocity.Equals(Vector3.Zero))
        {
            ...
            else
            {
                ...
                {
                    var numSolutions = Trajectory.solve_ballistic_arc(
                        origin,
                        speed,
                        dest,
                        targetVelocity,
