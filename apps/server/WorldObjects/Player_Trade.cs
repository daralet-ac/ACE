...
partial class Player
{
    ...
    public void HandleActionOpenTradeNegotiations(uint tradePartnerGuid, bool initiator = false)
    {
        if (IsOlthoiPlayer)
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"As a mindless engine of destruction an Olthoi cannot participate in trade negotiations!",
        ...
        if (initiator && tradePartner.IsOlthoiPlayer)
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"The Olthoi's hunger for destruction is too great to understand a request for trade negotiations!",
        ...
        if (initiator)
        {
            CreateMoveToChain(
                ...
                (success) =>
                {
                    ...
                    {
                        Session.Network.EnqueueSend(
                            new GameEventWeenieError(Session, WeenieError.TradeMaxDistanceExceeded)
                        );
                        return;
                    }
    ...
    public void HandleActionAddToTrade(uint itemGuid, uint tradeWindowSlotNumber)
    {
        ...
        var wo = GetInventoryItem(itemGuid);
        ...
        actionChain.AddAction(
            ...
            () =>
            {
                target.Session.Network.EnqueueSend(
                    new GameEventAddToTrade(target.Session, itemGuid, TradeSide.Partner)
                );
            }
        );
    ...
    public void HandleActionAcceptTrade()
    {
        ...
        target.Session.Network.EnqueueSend(
            new GameEventCommunicationTransientString(target.Session, $"{Name} has accepted the offer")
        );
    ...
    private void FinalizeTrade(Player target)
    {
        ...
        target.Session.Network.EnqueueSend(
            new GameEventCommunicationTransientString(target.Session, "The items are being traded")
        );
        ...
        foreach (var itemGuid in ItemsInTradeWindow)
        {
            if (
                TryRemoveFromInventoryWithNetworking(itemGuid, out var wo, RemoveFromInventoryAction.TradeItem)
                || TryDequipObjectWithNetworking(itemGuid, out wo, DequipObjectAction.TradeItem)
            )
            {
                targetEscrow.Add(wo);
        ...
        foreach (var itemGuid in target.ItemsInTradeWindow)
        {
            if (
                target.TryRemoveFromInventoryWithNetworking(itemGuid, out var wo, RemoveFromInventoryAction.TradeItem)
                || target.TryDequipObjectWithNetworking(itemGuid, out wo, DequipObjectAction.TradeItem)
            )
            {
                myEscrow.Add(wo);
    ...
    private bool GetItemsInTradeWindow(Player player, out List<WorldObject> itemsToBeTraded)
    {
        ...
        foreach (var itemGuid in player.ItemsInTradeWindow)
        {
    ...
    {
        if (session.Player.TradeTransferInProgress)
        {
            return;
        }
        ...
        if (target != null)
        {
            ...
            target.Session.Network.EnqueueSend(
                new GameEventCommunicationTransientString(target.Session, "Trade confirmation failed...")
            );
        }
    }
    ...
    public void HandleActionTradeSwitchToCombatMode(Session session)
    {
        if (session.Player.CombatMode != CombatMode.NonCombat && session.Player.IsTrading)
        {
            ...
            {
                target.Session.Network.EnqueueSend(
                    new GameEventWeenieError(target.Session, WeenieError.TradeNonCombatMode)
                );
    ...
    private bool VerifyTrade_Inventory(Player partner)
    {
        ...
        var playerBCanAddToInventory = partner.CanAddToInventory(
            self_items,
            out var partnerEncumbered,
            out var partnerPackSpace
        );
    ...
    public Dictionary<ObjectGuid, HashSet<ObjectGuid>> KnownTradeObjs =
        new Dictionary<ObjectGuid, HashSet<ObjectGuid>>();
        ...
        foreach (var knownTradeObj in KnownTradeObjs)
        {
            if (knownTradeObj.Value.Contains(itemGuid))
            {
                ...
                if (
                    ...
                    && Location.DistanceTo(player.Location) <= LocalBroadcastRange
                )
                {
                    return player;
                }
