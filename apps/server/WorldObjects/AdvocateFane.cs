...
public class AdvocateFane : WorldObject
{
    ...
    /// </summary>
    public AdvocateFane(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    /// </summary>
    public AdvocateFane(Biota biota)
        : base(biota)
    {
        SetEphemeralValues();
    }
    ...
    public override void ActOnUse(WorldObject activator)
    {
        ...
        if (
            player.AdvocateQuest
            || player.PkLevel != PKLevel.NPK
            || player is Admin
            || player.WeenieType == WeenieType.Admin
            || player is Sentinel
            || player.WeenieType == WeenieType.Sentinel
        ) // PlayerKillers, Admins and Sentinels can't be Advocates.
        {
            var actionChain = new ActionChain();

            var failMotion =
                UseTargetFailureAnimation != MotionCommand.Invalid ? UseTargetFailureAnimation : MotionCommand.Twitch2;
