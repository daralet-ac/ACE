...
public class PetDevice : WorldObject
{
    ...
    public int? PetClass
    {
        ...
        set
        {
            if (value.HasValue)
            {
                SetProperty(PropertyInt.PetClass, value.Value);
            }
            else
            {
                RemoveProperty(PropertyInt.PetClass);
            }
        }
    }
    ...
    public uint? Pet
    {
        ...
        set
        {
            if (value.HasValue)
            {
                SetProperty(PropertyInstanceId.Pet, value.Value);
            }
            else
            {
                RemoveProperty(PropertyInstanceId.Pet);
            }
        }
    }
    ...
    /// </summary>
    public PetDevice(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    ...
    public override void ActOnUse(WorldObject activator)
    {
        ...
        if (result == null || result.Value)
        {
            ...
            if (Structure != null)
            {
                ...
                player.Session.Network.EnqueueSend(
                    new GameMessagePublicUpdatePropertyInt(this, PropertyInt.Structure, Structure.Value)
                );
            }
        }
        else
    ...
    public bool? SummonCreature(Player player, uint wcid)
    {
        ...
        {
            _log.Error(
                $"{player.Name}.SummonCreature({wcid}) - couldn't find wcid for PetDevice {WeenieClassId} - {WeenieClassName}"
            );
            return false;
        }
        ...
        {
            _log.Error(
                $"{player.Name}.SummonCreature({wcid}) - PetDevice {WeenieClassId} - {WeenieClassName} tried to summon {wo.WeenieClassId} - {wo.WeenieClassName} of unknown type {wo.WeenieType}"
            );
            return false;
        }
    ...
    /// </summary>
    public static Dictionary<uint, Tuple<uint, DamageType>> PetDeviceToPetMapping = new Dictionary<
        uint,
        Tuple<uint, DamageType>
    >()
    {
        // ============================ Geomancer ============================
    ...
    public void Refill(Player player, CraftTool spirit)
    {
        ...
        actionChain.AddAction(
            ...
            () =>
            {
                ...
                {
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat("This essence is already full.", ChatMessageType.Broadcast)
                    );
                ...
                player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat("You add the spirit to the essence.", ChatMessageType.Broadcast)
                );
