...
partial class WorldObject
{
    public double? UseTimestamp
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyFloat.UseTimestamp);
            }
            else
            {
                SetProperty(PropertyFloat.UseTimestamp, value.Value);
            }
        }
    }
    ...
    protected double? ResetTimestamp
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyFloat.ResetTimestamp);
            }
            else
            {
                SetProperty(PropertyFloat.ResetTimestamp, value.Value);
            }
        }
    }
    ...
    protected double? ResetInterval
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyFloat.ResetInterval);
            }
            else
            {
                SetProperty(PropertyFloat.ResetInterval, value.Value);
            }
        }
    }
    ...
    protected bool DefaultLocked
    {
        ...
        set
        {
            if (!value)
            {
                RemoveProperty(PropertyBool.DefaultLocked);
            }
            else
            {
                SetProperty(PropertyBool.DefaultLocked, value);
            }
        }
    }
    ...
    protected bool DefaultOpen
    {
        ...
        set
        {
            if (!value)
            {
                RemoveProperty(PropertyBool.DefaultOpen);
            }
            else
            {
                SetProperty(PropertyBool.DefaultOpen, value);
            }
        }
    }
    ...
    {
        return (float)
            Physics.Common.Position.CylinderDistance(
                PhysicsObj.GetRadius(),
                PhysicsObj.GetHeight(),
                PhysicsObj.Position,
                wo.PhysicsObj.GetRadius(),
                wo.PhysicsObj.GetHeight(),
                wo.PhysicsObj.Position
            );
    }
    ...
    public virtual void OnActivate(WorldObject activator)
    {
        ...
        if (!(this is Creature) && ActivationTarget > 0)
        {
            ...
            {
                _log.Warning(
                    "{WorldObject}.OnActivate({Activator}): couldn't find activation target {ActivationTarget:X8}",
                    Name,
                    activator.Name,
                    ActivationTarget
                );
    ...
    public virtual void ActOnUse(WorldObject activator)
    {
        ...
        _log.Error(
            "{WorldObject}.ActOnUse({Activator}) - undefined for wcid {WeenieClassId} type {WeenieType}",
            Name,
            activator.Name,
            WeenieClassId,
            WeenieType
    ...
    public virtual ActivationResult CheckUseRequirements(WorldObject activator)
    {
        ...
        {
            _log.Error(
                "0x{Guid}:{WorldObject}.CheckUseRequirements() (wcid: {WeenieClassId}): activator is null",
                Guid,
                Name,
                WeenieClassId
            );
            ...
            if (playerSkill.AdvancementClass < SkillAdvancementClass.Trained)
            {
                ...
                player.Session.Network.EnqueueSend(
                    new GameEventCommunicationTransientString(
                        player.Session,
                        $"You must have {playerSkill.Skill.ToSentence()} trained to use that item's magic"
                    )
                );
                return new ActivationResult(false);
            }
        ...
        if (UseRequiresLevel != null)
        {
            ...
            if (playerLevel < UseRequiresLevel.Value)
            {
                ...
                return new ActivationResult(
                    new GameEventCommunicationTransientString(
                        player.Session,
                        "You are not high enough level to use that!"
                    )
                );
            }
        }
        ...
        if (!player.EnchantmentManager.CheckCooldown(CooldownId))
        {
            ...
            player.Session.Network.EnqueueSend(
                new GameEventCommunicationTransientString(player.Session, "You have used this item too recently")
            );
            return new ActivationResult(false);
        }
        ...
        if (player.IsOlthoiPlayer)
        {
            ...
            else if (this is Container && !(this is Corpse))
            {
                ...
                this is AttributeTransferDevice
                || this is AugmentationDevice
                || this is Bindstone
                || this is Book
                || this is Game
                || this is Gem
                || this is GenericObject
                || this is Key
                || this is SkillAlterationDevice
            )
            {
                player.SendWeenieError(WeenieError.OlthoiCannotInteractWithThat);
                return new ActivationResult(false);
                ...
                case CombatAbility.FocusedTaunt:
                    {
                        ...
                        if (
                            player.LastAttackTime > Time.GetUnixTime() - 10.0
                        ...
                        if (target == null)
                        {
                            player.Session.Network.EnqueueSend(
                                new GameMessageSystemChat(
                                    $"You have not recently attacked a creature.",
