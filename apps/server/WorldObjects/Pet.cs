...
public class Pet : Creature
{
    ...
    public uint? PetDevice
    {
        ...
        set
        {
            if (value.HasValue)
            {
                SetProperty(PropertyInstanceId.PetDevice, value.Value);
            }
            else
            {
                RemoveProperty(PropertyInstanceId.PetDevice);
            }
        }
    }
    ...
    /// </summary>
    public Pet(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
