...
public class Hook : Container
{
    ...
    /// </summary>
    public Hook(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    public override ActivationResult CheckUseRequirements(WorldObject activator)
    {
        ...
        if (
            player.House == null
            || houseOwner == null
            || (player.Guid.Full != houseOwner && player.House?.HouseOwner != houseOwner)
        )
        {
            if (Item == null)
            {
                return new ActivationResult(
                    new GameEventWeenieError(player.Session, WeenieError.HookItemNotUsable_CannotOpen)
                );
            }
    ...
    private static readonly ConcurrentDictionary<uint, WorldObject> cachedHookReferences =
        new ConcurrentDictionary<uint, WorldObject>();
