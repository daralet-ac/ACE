...
public class Switch : WorldObject
{
    ...
    /// </summary>
    public Switch(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    public uint? UseTargetAnimation
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyDataId.UseTargetAnimation);
            }
            else
            {
                SetProperty(PropertyDataId.UseTargetAnimation, value.Value);
            }
        }
    }
                ...
                actionChain.AddAction(
                    ...
                    () =>
                        player.Session.Network.EnqueueSend(
                            new GameMessageSystemChat(activationFailure, ChatMessageType.Broadcast)
                        )
                );
            }
        }
