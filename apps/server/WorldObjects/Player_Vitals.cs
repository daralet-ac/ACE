...
partial class Player
{
    ...
    public bool HandleActionRaiseVital(PropertyAttribute2nd vital, uint amount)
    {
        ...
        if (prevRank != creatureVital.Ranks)
        {
            ...
            var msg = new GameMessageSystemChat(
                $"Your base {vital.ToSentence()} is now {creatureVital.Base}{suffix}!",
    ...
    private bool SpendVitalXp(CreatureVital creatureVital, uint amount, bool sendNetworkUpdate = true)
    {
        ...
        {
            _log.Error(
                $"{Name}.SpendVitalXp({creatureVital.Vital}, {amount}) - player tried to raise vital beyond max rank"
            );
            return false;
        }
        ...
        {
            _log.Error(
                $"{Name}.SpendVitalXp({creatureVital.Vital}, {amount}) - player tried to raise vital beyond {amountToEnd} experience"
            );
        ...
        {
            _log.Error($"{Name}.SpendVitalXp({creatureVital.Vital}, {amount}) - SpendXP failed");
            return false;
        }
    ...
    public void HandleMaxHealthUpdate()
    {
        ...
        Session.Network.EnqueueSend(
            new GameMessagePrivateUpdatePropertyInt(this, PropertyInt.GearMaxHealth, gearMaxHealth)
        );
