...
public class SlumLord : Container
{
    ...
    /// </summary>
    public SlumLord(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    public bool HouseRequiresMonarch
    {
        ...
        set
        {
            if (!value)
            {
                RemoveProperty(PropertyBool.HouseRequiresMonarch);
            }
            else
            {
                SetProperty(PropertyBool.HouseRequiresMonarch, value);
            }
        }
    }
    ...
    public int? AllegianceMinLevel
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyInt.AllegianceMinLevel);
            }
            else
            {
                SetProperty(PropertyInt.AllegianceMinLevel, value.Value);
            }
        }
    }
    ...
    public bool HasRequirements(Player player)
    {
        ...
        {
            Console.WriteLine(
                $"{Name}.HasRequirements({player.Name}) - allegiance rank {player.AllegianceNode?.Rank ?? 0} < {allegianceMinLevel}"
            );
            return false;
        }
        return true;
