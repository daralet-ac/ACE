...
partial class WorldObject
{
    ...
    public void TryProcEquippedItems(WorldObject attacker, Creature target, bool selfTarget, WorldObject weapon)
    {
        ...
        {
            var equippedAetheria = wielder.EquippedObjects.Values.Where(i =>
                Aetheria.IsAetheria(i.WeenieClassId) && i.HasProc && i.ProcSpellSelfTargeted == selfTarget
            );

            var equippedProcJewelry = wielder.EquippedObjects.Values.Where(i =>
                i.ItemType == ItemType.Jewelry && i.HasProc
            );
            ...
            foreach (var jewelry in equippedProcJewelry)
            {
                if (attacker is Player)
                {
                    ...
                    if (
                        jewelry.ItemDifficulty != null
                        && player.GetCreatureSkill(Skill.ArcaneLore).Current > jewelry.ItemDifficulty
                    )
                    {
