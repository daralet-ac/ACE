...
partial class Player
{
    ...
    private void AddLuminance(long amount, XpType xpType)
    {
        ...
        if (xpType == XpType.Quest)
        {
            Session.Network.EnqueueSend(
                new GameMessageSystemChat($"You've earned {amount:N0} Luminance.", ChatMessageType.Broadcast)
            );
        }
    ...
    {
        Session.Network.EnqueueSend(
            new GameMessagePrivateUpdatePropertyInt64(this, PropertyInt64.AvailableLuminance, AvailableLuminance ?? 0)
        );
    }
}
