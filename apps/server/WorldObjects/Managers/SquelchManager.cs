...
public class SquelchManager
{
    ...
    /// </summary>
    public bool HasSquelches =>
        Squelches.Accounts.Count > 0 || Squelches.Characters.Count > 0 || Squelches.Globals.Filters.Count > 0;
    ...
    /// </summary>
    public void HandleActionModifyCharacterSquelch(
        bool squelch,
        uint playerGuid,
        string playerName,
    ...
    )
    {
        ...
        if (messageType != ChatMessageType.AllChannels && !IsLegalChannel(messageType))
        {
            Player.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{messageType} is not a legal squelch channel", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        if (playerGuid != 0)
        {
            ...
            {
                Player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat("Couldn't find player to squelch.", ChatMessageType.Broadcast)
                );
                return;
            }
        }
        ...
        {
            Player.Session.Network.EnqueueSend(
                new GameMessageSystemChat("You can't squelch yourself!", ChatMessageType.Broadcast)
            );
            return;
        }
    ...
    {
        Squelches.Characters.TryGetValue(player.Guid.Full, out var existing);
            ...
            if (existingMask == newMask)
            {
                Player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"{player.Name} is already squelched{channelMsg}.",
        ...
        Player.Session.Network.EnqueueSend(
            new GameMessageSystemChat($"{player.Name} has been squelched{channelMsg}.", ChatMessageType.Broadcast)
        );
    }
    ...
    public void UnsquelchCharacter(IPlayer player, ChatMessageType messageType)
    {
        ...
        {
            Player.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{player.Name} is not squelched.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        if (messageType == ChatMessageType.AllChannels)
        {
            ...
            Player.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{player.Name} has been unsquelched.", ChatMessageType.Broadcast)
            );
        }
        else
        {
            ...
            if (!existingMask.HasFlag(removeMask))
            {
                Player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"{player.Name} is not squelched on the {messageType} channel.",
                        ChatMessageType.Broadcast
                    )
                );
                return;
            ...
            Player.Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"{player.Name} has been unsquelched on the {messageType} channel.",
                    ChatMessageType.Broadcast
                )
            );
        }
    ...
    public void HandleActionModifyAccountSquelch(bool squelch, string playerName)
    {
        ...
        if (squelch)
        {
            if (existing != null)
            {
                Player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"{player.Name}'s account is already squelched.",
                        ChatMessageType.Broadcast
                    )
                );
                return;
            ...
            // always all channels?
            Player.Character.AddOrUpdateSquelch(
                player.Guid.Full,
                player.Account.AccountId,
                (uint)SquelchMask.AllChannels,
                Player.CharacterDatabaseLock
            );
            Player.CharacterChangesDetected = true;

            Player.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{player.Name}'s account has been squelched.", ChatMessageType.Broadcast)
            );
        }
        else
        {
            ...
            {
                Player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat($"{player.Name}'s account is not squelched.", ChatMessageType.Broadcast)
                );
                return;
            }

            Player.Character.TryRemoveSquelch(
                existing.SquelchCharacterId,
                player.Account.AccountId,
                out _,
                Player.CharacterDatabaseLock
            );
            ...
            Player.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{player.Name}'s account has been unsquelched.", ChatMessageType.Broadcast)
            );
        }
    ...
    public void HandleActionModifyGlobalSquelch(bool squelch, ChatMessageType messageType)
    {
        ...
        {
            Player.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"The {messageType} channel is already {op}ed.", ChatMessageType.Broadcast)
            );
            return;
        }
        ...
        Player.Session.Network.EnqueueSend(
            new GameMessageSystemChat($"The {messageType} channel has been {op}ed.", ChatMessageType.Broadcast)
        );
