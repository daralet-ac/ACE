...
public class EnchantmentManager
{
    ...
    /// </summary>
    public virtual bool HasEnchantments =>
        WorldObject.Biota.PropertiesEnchantmentRegistry.HasEnchantments(WorldObject.BiotaDatabaseLock);

    /// <summary>
    /// Returns TRUE If this object has a vitae penalty
    /// </summary>
    public virtual bool HasVitae =>
        WorldObject.Biota.PropertiesEnchantmentRegistry.HasEnchantment(
            (uint)SpellId.Vitae,
    ...
    {
        return WorldObject.Biota.PropertiesEnchantmentRegistry.GetEnchantmentBySpell(
            (int)spellID,
            casterGuid,
            WorldObject.BiotaDatabaseLock
        );
    ...
    {
        return WorldObject.Biota.PropertiesEnchantmentRegistry.GetEnchantmentBySpellSet(
            (int)spellID,
            equipmentSet,
            WorldObject.BiotaDatabaseLock
        );
    ...
    public List<PropertiesEnchantmentRegistry> GetEnchantments(MagicSchool magicSchool)
    {
        ...
        var topLayerEnchantments = WorldObject.Biota.PropertiesEnchantmentRegistry.GetEnchantmentsTopLayer(
            WorldObject.BiotaDatabaseLock,
            SpellSet.SetSpells
        );
        ...
        foreach (var enchantment in topLayerEnchantments)
        {
            ...
            {
                Console.WriteLine(
                    $"EnchantmentManager.GetEnchantments({magicSchool}): couldn't find spell {enchantment.SpellId} for {WorldObject.Name}"
                );
                continue;
            }
    ...
    {
        return WorldObject.Biota.PropertiesEnchantmentRegistry.GetEnchantmentsByCategory(
            spellCategory,
            WorldObject.BiotaDatabaseLock
        );
    }
    ...
    {
        return WorldObject.Biota.PropertiesEnchantmentRegistry.GetEnchantmentsTopLayerByStatModType(
            statModType,
            WorldObject.BiotaDatabaseLock,
            SpellSet.SetSpells
        );
    ...
    /// </summary>
    public List<PropertiesEnchantmentRegistry> GetEnchantments_TopLayer(
        EnchantmentTypeFlags statModType,
        uint statModKey,
        bool handleMultiple = false
    )
    {
        return WorldObject.Biota.PropertiesEnchantmentRegistry.GetEnchantmentsTopLayerByStatModType(
            statModType,
    ...
    /// </summary>
    public virtual AddEnchantmentResult Add(
        Spell spell,
        WorldObject caster,
        WorldObject weapon,
        bool equip = false,
    ...
    )
    {
        ...
        else
        {
            ...
            if (
                caster is Player player
                && player.AugmentationIncreasedSpellDuration > 0
                && !isWeaponSpell
                && spell.DotDuration == 0
            )
            {
    ...
    /// </summary>
    private PropertiesEnchantmentRegistry BuildEntry(
        Spell spell,
    ...
    )
    {
        ...
        if (spell.IsDamageOverTime)
        {
            ...
            if (caster != null)
            {
                entry.StatModValue = caster.CalculateDotEnchantment_StatModValue(
                    spell,
                    WorldObject,
                    weapon,
                    entry.StatModValue
    ...
    public virtual bool StartCooldown(WorldObject item)
    {
        ...
        Player.Session.Network.EnqueueSend(
            new GameEventMagicUpdateEnchantment(Player.Session, new Enchantment(Player, newEntry))
        );
    ...
    public virtual void Remove(PropertiesEnchantmentRegistry entry, bool sound = true)
    {
        ...
        if (
            WorldObject.Biota.PropertiesEnchantmentRegistry.TryRemoveEnchantment(
                entry.SpellId,
                ...
                if (owner != null)
                {
                    ...
                    owner.Session.Network.EnqueueSend(
                        new GameMessageSystemChat(
                            $"The spell {spell.Name} on {WorldObject.Name} has expired.",
    ...
    public virtual void Dispel(PropertiesEnchantmentRegistry entry)
    {
        ...
        {
            Player.Session.Network.EnqueueSend(
                new GameEventMagicDispelEnchantment(Player.Session, (ushort)entry.SpellId, entry.LayerId)
            );
        }
    }
    ...
    {
        var enchantments = GetEnchantments_TopLayer(
    ...
    )
    {
        ...
        foreach (var enchantment in enchantments)
        {
            ...
            {
                Console.WriteLine(
                    $"{WorldObject.Name}.ApplyDamageTick() - couldn't find damager {enchantment.CasterObjectId:X8}"
                );
                continue;
            }
