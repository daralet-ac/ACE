...
public class ConfirmationManager
{
    ...
    private ConcurrentDictionary<ConfirmationType, Confirmation> confirmations =
        new ConcurrentDictionary<ConfirmationType, Confirmation>();
    ...
    public bool EnqueueSend(Confirmation confirmation, string text)
    {
        ...
        if (confirmations.TryAdd(confirmation.ConfirmationType, confirmation))
        {
            Player.Session.Network.EnqueueSend(
                new GameEventConfirmationRequest(
                    Player.Session,
            ...
            timeoutConfirmation.AddAction(
                Player,
                () => EnqueueAbort(confirmation.ConfirmationType, confirmation.ContextId)
            );
    ...
    public void EnqueueAbort(ConfirmationType confirmationType, uint contextId)
    {
        ...
        {
            Player.Session.Network.EnqueueSend(
                new GameEventConfirmationDone(Player.Session, confirmationType, contextId)
            );
    ...
    public bool HandleResponse(ConfirmationType confirmType, uint contextId, bool response, bool timeout = false)
    {
        if (!confirmations.TryRemove(confirmType, out var confirm))
        {
            switch (confirmType)
            {
                ...
                default:
                    _log.Error(
                        $"{Player.Name}.ConfirmationManager.HandleResponse({confirmType}, {contextId}, {response}, {timeout}) - confirmType not found"
                    );
                    break;
            }
        ...
        if (confirm.ContextId != contextId)
        {
            if (confirm.ConfirmationType == ConfirmationType.Fellowship)
            {
                ...
                {
                    _log.Error(
                        $"{Player.Name}.ConfirmationManager.HandleResponse({confirm.ConfirmationType}, {confirm.ContextId}) - Unable to re-add confirmation, duplicate confirmation type"
                    );
                }
            ...
            _log.Error(
                $"{Player.Name}.ConfirmationManager.HandleResponse({confirmType}, {contextId}, {response}, {timeout}) - contextId != confirm.ContextId"
            );
