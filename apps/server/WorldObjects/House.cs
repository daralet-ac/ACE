...
public class House : WorldObject
{
    ...
    {
        get => ChildLinks.FirstOrDefault(l => l as SlumLord != null) as SlumLord;
    }
    public List<Hook> Hooks
    {
        get => ChildLinks.OfType<Hook>().ToList();
    }
    public List<Storage> Storage
    {
        get => ChildLinks.OfType<Storage>().ToList();
    }
    ...
    public HousePortal HousePortal
    {
        get => ChildLinks.FirstOrDefault(l => l as HousePortal != null) as HousePortal;
    }
    public List<WorldObject> Linkspots =>
        ChildLinks
            .Where(l => l.WeenieType == WeenieType.Generic && l.WeenieClassName.Equals("portaldestination"))
            .ToList();

    /// <summary>
    /// A new biota be created taking all of its values from weenie.
    /// </summary>
    public House(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
    ...
    public override void SetLinkProperties(WorldObject wo)
    {
        ...
        {
            var biota = DatabaseManager
                .Shard.BaseDatabase.GetBiotasByWcid(WeenieClassId)
                .Where(bio => bio.BiotaPropertiesPosition.Count > 0)
                .FirstOrDefault(b =>
                    b.BiotaPropertiesPosition.FirstOrDefault(p =>
                        p.PositionType == (ushort)PositionType.Location
                    ).ObjCellId >> 16
                    != Location.Landblock
                );
            if (biota != null)
    ...
    public override void UpdateLinkProperties(WorldObject wo)
    {
        if (wo.HouseOwner != HouseOwner)
        {
            ...
            wo.EnqueueBroadcast(
                new GameMessagePublicUpdateInstanceID(
                    wo,
                    PropertyInstanceId.HouseOwner,
                    new ObjectGuid(HouseOwner ?? 0)
                )
            );
        }
    ...
    public bool? HouseHooksVisible
    {
        ...
        set
        {
            if (!value.HasValue)
            {
                RemoveProperty(PropertyBool.HouseHooksVisible);
            }
            else
            {
                SetProperty(PropertyBool.HouseHooksVisible, value.Value);
            }
        }
    }
    ...
    public bool? GetAllegianceAccessLevel()
    {
        ...
        if (!Guests.TryGetValue(new ObjectGuid(MonarchId.Value), out var storage))
        {
            return null;
        }
    ...
    public void UpdateRestrictionDB(RestrictionDB restrictions = null)
    {
        ...
        {
            restrictions = new RestrictionDB(this);
        }
        ...
        foreach (var player in nearbyPlayers)
        {
            player.Session.Network.EnqueueSend(
                new GameMessagePublicUpdateInstanceID(
                    this,
                    PropertyInstanceId.HouseOwner,
                    new ObjectGuid(restrictions.HouseOwner)
                ),
    ...
    public bool OpenToEveryone
    {
        ...
        set
        {
            if (!value)
            {
                RemoveProperty(PropertyInt.OpenToEveryone);
            }
            else
            {
                SetProperty(PropertyInt.OpenToEveryone, 1);
            }
        }
    }
    ...
    public int HouseMaxHooksUsable
    {
        ...
        set
        {
            if (value == 25)
            {
                RemoveProperty(PropertyInt.HouseMaxHooksUsable);
            }
            else
            {
                SetProperty(PropertyInt.HouseMaxHooksUsable, value);
            }
        }
    }
    ...
    public int HouseCurrentHooksUsable
    {
        ...
        set
        {
            if (value == HouseMaxHooksUsable)
            {
                RemoveProperty(PropertyInt.HouseCurrentHooksUsable);
            }
            else
            {
                SetProperty(PropertyInt.HouseCurrentHooksUsable, value);
            }
        }
    }

    public static Dictionary<HouseType, Dictionary<HookGroupType, int>> HookGroupLimits = new Dictionary<
        HouseType,
        Dictionary<HookGroupType, int>
    >()
    {
        {
            HouseType.Undef,
            new Dictionary<HookGroupType, int>
            {
    ...
    public int GetHookGroupCurrentCount(HookGroupType hookGroupType) =>
        Hooks.Count(h => h.HasItem && (h.Item?.HookGroup ?? HookGroupType.Undef) == hookGroupType);
