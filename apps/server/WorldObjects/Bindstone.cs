...
public class Bindstone : WorldObject
{
    ...
    /// </summary>
    public Bindstone(Weenie weenie, ObjectGuid guid)
        : base(weenie, guid)
    {
        SetEphemeralValues();
    }
    ...
    public override void ActOnUse(WorldObject worldObject)
    {
        ...
        if (player.Allegiance == null)
        {
            player.Session.Network.EnqueueSend(
                new GameEventWeenieError(player.Session, WeenieError.YouAreNotInAllegiance)
            );
            return;
        }
        ...
        {
            player.Session.Network.EnqueueSend(
                new GameEventWeenieError(player.Session, WeenieError.YouDoNotHaveAuthorityInAllegiance)
            );
            return;
        }
        ...
        actionChain.AddAction(
            ...
            () =>
            {
                if (player.IsWithinUseRadiusOf(this))
                {
                    ...
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat(GetProperty(PropertyString.UseMessage), ChatMessageType.Magic)
                    );
                }
                else
                {
                    player.Session.Network.EnqueueSend(
                        new GameEventWeenieError(player.Session, WeenieError.YouHaveMovedTooFar)
                    );
                }
            }
        );
