...
public class RetainingChisel : WorldObject
{
    ...
    public RetainingChisel(Biota biota)
        : base(biota)
    {
        SetEphemeralValues();
    }
    ...
    public static void UseObjectOnTarget(Player player, WorldObject source, WorldObject target, bool confirmed = false)
    {
        ...
        if (target.Workmanship == null)
        {
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"The {target.Name} cannot be retained.", ChatMessageType.Craft)
            );
        ...
        actionChain.AddAction(
            ...
            () =>
            {
                if (target.Retained == false)
                {
                    ...
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat(
                            $"The {target.NameWithMaterial} is now retained.",
                ...
                else
                {
                    ...
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat(
                            $"The {target.NameWithMaterial} is no longer retained.",
                            ChatMessageType.Craft
                        )
                    );
                    player.SendUseDoneEvent();
