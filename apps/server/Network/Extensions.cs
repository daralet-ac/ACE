...
public static class Extensions
{
    ...
    public static void WritePackedDword(this BinaryWriter writer, uint value)
    {
        if (value <= 32767)
        {
            var networkValue = Convert.ToUInt16(value);
            writer.Write(BitConverter.GetBytes(networkValue));
        }
        else
        {
            var packedValue = (value << 16) | ((value >> 16) | 0x8000);
    ...
    {
        var beValue = (ushort)((ushort)((value & 0xFF) << 8) | ((value >> 8) & 0xFF));
        writer.Write(beValue);
    }

    public static void Pad(this BinaryWriter writer, uint pad)
    {
        writer.Write(new byte[pad]);
    }

    public static void Pad(this BinaryReader reader, uint pad)
    {
        reader.ReadBytes((int)pad);
    }
    ...
    public static string BuildPacketString(this byte[] bytes, int startPosition = 0, int bytesToOutput = 9999)
    {
        ...
        var asciiLine = "";
        for (var i = startPosition; i < startPosition + bytesToOutput; i++)
        {
            if (i >= buffer.Length)
            {
                break;
    ...
    {
        var originalPosition = writer.BaseStream.Position;
        writer.BaseStream.Position = position;
        writer.Write(value);
        writer.BaseStream.Position = originalPosition;
    }

    public static ObjectGuid ReadGuid(this BinaryReader reader)
    {
        return new ObjectGuid(reader.ReadUInt32());
    }

    public static void WriteGuid(this BinaryWriter writer, ObjectGuid guid)
    {
