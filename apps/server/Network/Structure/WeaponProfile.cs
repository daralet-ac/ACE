...
public class WeaponProfile
{
    ...
    public uint GetDamage(WorldObject weapon)
    {
        ...
        var mainHandEnchantmentDamageBonus =
            creatureWielder != null && creatureWielder.GetEquippedWeapon() != null
                ? creatureWielder.GetEquippedWeapon().EnchantmentManager.GetDamageBonus()
                : 1;
        ...
        var damageBonus =
            ...
            && (weapon.WeenieType == WeenieType.Ammunition && PropertyManager.GetBool("show_ammo_buff").Item)
                ? mainHandEnchantmentDamageBonus
                : weapon.EnchantmentManager.GetDamageBonus();
        var auraDamageBonus =
            weapon.Wielder != null
            && (weapon.WeenieType != WeenieType.Ammunition || PropertyManager.GetBool("show_ammo_buff").Item)
                ? weapon.Wielder.EnchantmentManager.GetDamageBonus()
                : 0;
    ...
    public float GetWeaponDefense(WorldObject weapon)
    {
        ...
        if (
            weapon.WeaponPhysicalDefense.HasValue
            && weapon.WeaponPhysicalDefense.Value > 0
            && weapon.WeaponPhysicalDefense.Value < 1
