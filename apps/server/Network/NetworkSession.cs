...
public class NetworkSession
{
    ...
    private ConcurrentDictionary<uint, MessageBuffer> partialFragments =
        new ConcurrentDictionary<uint, MessageBuffer>();
    private ConcurrentDictionary<uint, ClientMessage> outOfOrderFragments =
        new ConcurrentDictionary<uint, ClientMessage>();
    ...
    public NetworkSession(Session session, ConnectionListener connectionListener, ushort clientId, ushort serverId)
    {
        ...
        for (var i = 0; i < currentBundles.Length; i++)
        {
    ...
    public void EnqueueSend(params GameMessage[] messages)
    {
        ...
        foreach (var message in messages)
        {
            ...
            lock (currentBundleLock)
            {
                ...
                _log.Verbose(
                    "[{LoggingIdentifier}] Enqueuing Message {MessageOpCode}",
    ...
    public void EnqueueSend(params ServerPacket[] packets)
    {
        ...
        {
            _log.Verbose(
                "[{LoggingIdentifier}] Enqueuing Packet {PacketHashCode}",
                session.LoggingIdentifier,
                packet.GetHashCode()
            );
        ...
        for (var i = 0; i < currentBundles.Length; i++)
        {
            ...
            lock (currentBundleLock)
            {
                ...
                if (group == GameMessageGroup.InvalidQueue)
                {
                    ...
                    {
                        _log.Verbose(
                            "[{LoggingIdentifier}] Setting to send TimeSync packet",
                            session.LoggingIdentifier
                        );
    ...
    private void PruneCachedPackets()
    {
        ...
        var removalList = cachedPackets.Values.Where(x =>
            (currentTime >= x.Header.Time ? currentTime : currentTime + ushort.MaxValue) - x.Header.Time
    ...
    public void ProcessPacket(ClientPacket packet)
    {
        ...
        _log.Verbose(
            "[{LoggingIdentifier}] Processing packet {PacketSequence}",
            session.LoggingIdentifier,
        ...
        if (
            (packet.Header.Flags & PacketHeaderFlags.RequestRetransmit) == PacketHeaderFlags.RequestRetransmit
            && !((packet.Header.Flags & PacketHeaderFlags.EncryptedChecksum) == PacketHeaderFlags.EncryptedChecksum)
        )
        {
            List<uint> uncached = null;

            foreach (var sequence in packet.HeaderOptional.RetransmitData)
            {
        ...
        session.Network.TimeoutTick =
            (session.State == SessionState.AuthLoginRequest)
        ...
        if (
            packet.Header.Sequence <= lastReceivedPacketSequence
        ...
        {
            _log.Verbose(
                "[{LoggingIdentifier}] Packet {PacketSequence} received again",
                session.LoggingIdentifier,
                packet.Header.Sequence
            );
            return;
        ...
        {
            _log.Verbose(
                "[{LoggingIdentifier}] Packet {PacketSequence} received out of order",
                session.LoggingIdentifier,
                packet.Header.Sequence
            );
            ...
            if (
                ...
                && DateTime.UtcNow - LastRequestForRetransmitTime > new TimeSpan(0, 0, 1)
            )
            {
    ...
    {
        var desiredSeq = lastReceivedPacketSequence + 1;
        var needSeq = new List<uint>();
        needSeq.Add(desiredSeq);
        var bottom = desiredSeq + 1;
        ...
        var reqData = new byte[4 + (needSeq.Count * 4)];
        var msReqData = new MemoryStream(reqData, 0, reqData.Length, true, true);
        ...
        _log.Verbose(
            "[{LoggingIdentifier}] Requested retransmit of {RequestedSequence}",
            session.LoggingIdentifier,
            needSeq.Select(k => k.ToString()).Aggregate((a, b) => a + ", " + b)
        );
    ...
    {
        _log.Verbose(
            "[{LoggingIdentifier}] Handling packet {PacketSequence}",
            session.LoggingIdentifier,
            packet.Header.Sequence
        );
        ...
        {
            _log.Verbose(
                "[{LoggingIdentifier}] Incoming TimeSync TS: {PacketTimeSync}",
                session.LoggingIdentifier,
                packet.HeaderOptional.TimeSynch
            );
    ...
    {
        _log.Verbose(
            "[{LoggingIdentifier}] Processing fragment {FragmentSequence}",
            session.LoggingIdentifier,
        ...
        if (fragment.Header.Count != 1)
        {
            // Packet is split
            _log.Verbose(
                "[{LoggingIdentifier}] Fragment {FragmentSequence} is split, this index {FragmentIndex} of {FragmentCount} fragments",
                session.LoggingIdentifier,
                fragment.Header.Sequence,
                fragment.Header.Index,
                fragment.Header.Count
            );
            ...
            if (partialFragments.TryGetValue(fragment.Header.Sequence, out var buffer))
            {
                ...
                _log.Verbose(
                    "[{LoggingIdentifier}] Added fragment {FragmentSequence} to existing buffer. Buffer at {BufferCount} of {TotalFragments}",
                    session.LoggingIdentifier,
                    fragment.Header.Sequence,
                    buffer.Count,
                ...
                if (buffer.Complete)
                {
                    ...
                    _log.Verbose(
                        "[{LoggingIdentifier}] Buffer {BufferSequence} is complete",
                        session.LoggingIdentifier,
                        buffer.Sequence
                    );
            ...
            else
            {
                ...
                _log.Verbose(
                    "[{LoggingIdentifier}] Creating new buffer {FragmentSequence} for this split fragment",
                    session.LoggingIdentifier,
                    fragment.Header.Sequence
                );
                ...
                _log.Verbose(
                    "[{LoggingIdentifier}] Added fragment {FragmentSequence} to the new buffer. Buffer at {BufferCount} of {TotalFragments}",
                    session.LoggingIdentifier,
                    fragment.Header.Sequence,
                    newBuffer.Count,
                    newBuffer.TotalFragments
                );
        ...
        else
        {
            ...
            _log.Verbose(
                "[{LoggingIdentifier}] Fragment {FragmentSequence} is not split",
                session.LoggingIdentifier,
                fragment.Header.Sequence
            );
        ...
        if (message != null)
        {
            ...
            {
                _log.Verbose(
                    "[{LoggingIdentifier}] Handling fragment {FragmentSequence}",
                    session.LoggingIdentifier,
                    fragment.Header.Sequence
                );
            ...
            {
                _log.Verbose(
                    "[{LoggingIdentifier}] Fragment {FragmentSequence} is early, lastReceivedFragmentSequence = {LastFragmentSequenceReceived}",
                    session.LoggingIdentifier,
                    fragment.Header.Sequence,
    ...
    private void CheckOutOfOrderPackets()
    {
        ...
        {
            _log.Verbose(
                "[{LoggingIdentifier}] Ready to handle out-of-order packet {PacketSequence}",
                session.LoggingIdentifier,
                packet.Header.Sequence
            );
            HandleOrderedPacket(packet);
    ...
    private void CheckOutOfOrderFragments()
    {
        ...
        {
            _log.Verbose(
                "[{LoggingIdentifier}] Ready to handle out of order fragment {LastFragmentSequenceReceived}",
                session.LoggingIdentifier,
                lastReceivedFragmentSequence + 1
            );
            HandleFragment(message);
        }
    }
    ...
    private void VerifyEcho(float clientTime)
    {
        ...
        if (diff > EchoThreshold && diff - lastDiff > DiffThreshold)
        {
            ...
            {
                _log.Warning(
                    "{Player} - TimeSync error: {EchoDiff} (diff: {TimeDifference})",
                    session.Player.Name,
                    echoDiff,
                    diff
                );
                ...
                actionChain.AddAction(
                    ...
                    () =>
                    {
                        ...
                        session.Network.EnqueueSend(
                            new GameMessageSystemChat("TimeSync: client speed error", ChatMessageType.Broadcast)
                        );
    ...
    private bool Retransmit(uint sequence)
    {
        ...
        if (cachedPackets.Count > 0)
        {
            ...
            {
                _log.Verbose(
                    "Session {ClientId}\\{EndPointC2S} ({Account}:{PlayerName}) retransmit requested packet {Sequence} not in cache. Cache range {CacheMin} - {CacheMax}.",
                    session.Network?.ClientId,
                    session.EndPointC2S,
                    session.Account,
                    session.Player?.Name,
                    sequence,
                    cachedPackets.Keys.Min(),
                    cachedPackets.Keys.Max()
                );
            }
            catch
            {
                _log.Verbose(
                    "Session {ClientId}\\{EndPointC2S} ({Account}:{PlayerName}) retransmit requested packet {Sequence} not in cache. Cache is empty. Race condition threw exception.",
                    session.Network?.ClientId,
                    session.EndPointC2S,
                    session.Account,
                    session.Player?.Name,
        ...
        {
            _log.Verbose(
                "Session {ClientId}\\{EndPointC2S} ({Account}:{PlayerName}) retransmit requested packet {Sequence} not in cache. Cache is empty.",
                session.Network?.ClientId,
                session.EndPointC2S,
                session.Account,
                session.Player?.Name,
    ...
    private void FlushPackets()
    {
        ...
        {
            _log.Verbose(
                "[{LoggingIdentifier}] Flushing packets, count {PacketCount}",
                session.LoggingIdentifier,
                packetQueue.Count
            );
            ...
            var isNak = packet.Header.Flags.HasFlag(PacketHeaderFlags.RequestRetransmit);
    ...
    {
        _log.Verbose(
            "[{LoggingIdentifier}] Sending packet {PacketHashCode}",
            session.LoggingIdentifier,
            packet.GetHashCode()
        );
        ...
        {
            var issacXor = ConnectionData.IssacServer.Next();
            _log.Verbose(
                "[{LoggingIdentifier}] Setting Issac for packet {PacketHashCode} to {IssacXor}",
                session.LoggingIdentifier,
                packet.GetHashCode(),
                issacXor
            );
    ...
    private void SendPacketRaw(ServerPacket packet)
    {
        var buffer = ArrayPool<byte>.Shared.Rent(
            (int)(
                ...
                + (packet.Fragments.Count * PacketFragment.MaxFragementSize)
            )
        );
        ...
        try
        {
            ...
            if (_log.IsEnabled(LogEventLevel.Verbose))
            {
                ...
                _log.Verbose(
                    "[{ClientId}] Sending Packet (Len: {PacketLength}) [{ListenerAddress}:{ListenerPort}=>{EndpointAddress}:{EndpointPort}]\n{Packet}",
                    session.Network.ClientId,
                    size,
                    ...
                    endPoint.Address,
                    endPoint.Port,
                    buffer.BuildPacketString(0, size)
                );
            }
    ...
    private void SendBundle(NetworkBundle bundle, GameMessageGroup group)
    {
        ...
        _log.Verbose(
            "[{LoggingIdentifier}] Bundle Fragment Count: {FragmentCount}",
            session.LoggingIdentifier,
            fragments.Count
        );
        ...
        while (fragments.Count > 0 || writeOptionalHeaders)
        {
            ...
            var availableSpace = ServerPacket.MaxPacketSize;
            ...
            {
                _log.Verbose(
                    "[{LoggingIdentifier}] No messages, just sending optional headers",
                    session.LoggingIdentifier
                );
                if (writeOptionalHeaders)
                {
    ...
    private void WriteOptionalHeaders(NetworkBundle bundle, ServerPacket packet)
    {
        ...
        if (bundle.SendAck) // 0x4000
        {
            ...
            _log.Verbose(
                "[{LoggingIdentifier}] Outgoing AckSeq: {LastPacketSequenceReceived}",
                session.LoggingIdentifier,
        ...
        if (bundle.TimeSync) // 0x1000000
        {
            ...
            _log.Verbose(
                "[{LoggingIdentifier}] Outgoing TimeSync TS: {PortalYearTicks}",
                session.LoggingIdentifier,
        ...
        if (bundle.ClientTime != -1f) // 0x4000000
        {
            ...
            _log.Verbose(
                "[{LoggingIdentifier}] Outgoing EchoResponse: {ClientTime}",
                session.LoggingIdentifier,
                bundle.ClientTime
            );
            packet.InitializeDataWriter();
