...
public static class InboundMessageManager
{
    ...
    public static void HandleClientMessage(ClientMessage message, Session session)
    {
        ...
        if (messageHandlers.TryGetValue(opcode, out var messageHandlerInfo))
        {
            ...
            {
                NetworkManager.InboundMessageQueue.EnqueueAction(
                    new ActionEventDelegate(() =>
                    {
                        ...
                        if (
                            messageHandlerInfo.Attribute.State == Enum.SessionState.WorldConnected
                            && session.Player == null
                        )
                        {
                            return;
                        ...
                        catch (Exception ex)
                        {
                            _log.Error(
                                ex,
                                $"Received GameMessage packet that threw an exception from account: {session.AccountId}:{session.Account}, player: {session.Player?.Name}, opcode: 0x{((int)opcode):X4}:{opcode}"
                            );
                        }
                    })
                );
    ...
    public static void HandleGameAction(GameActionType opcode, ClientMessage message, Session session)
    {
        if (actionHandlers.TryGetValue(opcode, out var actionHandlerInfo))
        {
            ...
            catch (Exception ex)
            {
                _log.Error(
                    ex,
                    $"Received GameAction packet that threw an exception from account: {session.AccountId}:{session.Account}, player: {session.Player?.Name}, opcode: 0x{((int)opcode):X4}:{opcode}"
                );
            }
        }
        else
