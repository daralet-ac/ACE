...
public class ConnectionListener
{
    ...
    {
        _log.Debug(
            "Starting ConnectionListener, host {ListeningHost} port {ListeningPort}",
            listeningHost,
            listeningPort
        );
    ...
    {
        _log.Debug(
            "Shutting down ConnectionListener, host {ListeningHost} port {ListeningPort}",
            listeningHost,
            listeningPort
        );
    ...
    private void Listen()
    {
        ...
        catch (SocketException socketException)
        {
            _log.Debug(
                socketException,
                "ConnectionListener({ListeningHost}, {ListeningPort}).Listen() has thrown {SocketErrorCode}",
                listeningHost,
                listeningPort,
        ...
        catch (Exception exception)
        {
            _log.Fatal(
                exception,
                "ConnectionListener({ListeningHost}, {ListeningPort}).Listen() has thrown",
                listeningHost,
                listeningPort
            );
        }
    ...
    private void OnDataReceive(IAsyncResult result)
    {
        ...
        try
        {
            ...
            var dataSize = Socket.EndReceiveFrom(result, ref clientEndPoint);

            var ipEndpoint = (IPEndPoint)clientEndPoint;
            ...
            if (_log.IsEnabled(LogEventLevel.Verbose))
            {
                ...
                _log.Verbose(
                    "Received Packet (Len: {DataLength}) [{IpEndpointAddress}:{IpEndpointPort}=>{ListenerEndpointAddress}:{ListenerEndpointPort}] {PacketString}",
                    data.Length,
                    ipEndpoint.Address,
                    ipEndpoint.Port,
                    ListenerEndpoint.Address,
                    ListenerEndpoint.Port,
                    data.BuildPacketString()
                );
            }
        ...
        catch (SocketException socketException)
        {
            ...
            )
            {
                _log.Debug(
                    socketException,
                    "ConnectionListener({ListeningHost}, {ListeningPort}).OnDataReceive() has thrown {SocketErrorCode}: from client {ClientEndpoint}",
                    listeningHost,
                    listeningPort,
                    socketException.SocketErrorCode,
                    clientEndPoint != null ? clientEndPoint.ToString() : "Unknown"
                );
            }
            else
            {
