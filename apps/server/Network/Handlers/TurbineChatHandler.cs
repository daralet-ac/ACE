...
public static class TurbineChatHandler
{
    ...
    public static void TurbineChatReceived(ClientMessage clientMessage, Session session)
    {
        ...
        if (chatBlobType == ChatNetworkBlobType.NETBLOB_REQUEST_BINARY)
        {
            ...
            {
                var lowbyte = clientMessage.Payload.ReadByte();
            ...
            {
                _log.Debug(
                    "[CHAT] ChannelID ({ChannelID}) was adjusted to {AdjustedChannelID} | ChatNetworkBlobDispatchType: {ChatBlobDispatchType}",
                    channelID,
                    adjustedChannelID,
                    chatBlobDispatchType
                );
            }
            ...
            {
                _log.Debug(
                    "[CHAT] ChatType ({ChatType}) was adjusted to {AdjustedChatType} | ChatNetworkBlobDispatchType: {ChatBlobDispatchType}",
                    chatType,
                    adjustedchatType,
                    chatBlobDispatchType
                );
            ...
            if (adjustedChannelID > TurbineChatChannel.Olthoi || adjustedChannelID == TurbineChatChannel.Allegiance) // Channel must be an allegiance channel
            {
                ...
                if (allegiance != null)
                {
                    ...
                    foreach (var member in allegiance.Members.Keys)
                    {
                        ...
                        if (
                            allegiance.IsFiltered(member)
                            || online.SquelchManager.Squelches.Contains(session.Player, ChatMessageType.Allegiance)
                        )
                        {
                            continue;
                        }
            ...
            else // Channel must be one of the channels available to all players
            {
                ...
                var chat_requires_account_time_seconds = PropertyManager
                    .GetLong("chat_requires_account_time_seconds")
                    .Item;
                if (
                    chat_requires_account_time_seconds > 0
                    && (DateTime.UtcNow - session.Player.Account.CreateTime).TotalSeconds
                ...
                foreach (var recipient in PlayerManager.GetAllOnline())
                {
                    ...
                    if (
                        channelID == TurbineChatChannel.General
                            && !recipient.GetCharacterOption(CharacterOption.ListenToGeneralChat)
                        || channelID == TurbineChatChannel.Trade
                            && !recipient.GetCharacterOption(CharacterOption.ListenToTradeChat)
                        || channelID == TurbineChatChannel.LFG
                            && !recipient.GetCharacterOption(CharacterOption.ListenToLFGChat)
                        || channelID == TurbineChatChannel.Roleplay
                            && !recipient.GetCharacterOption(CharacterOption.ListenToRoleplayChat)
                    )
                    {
                        continue;
                    }
                    ...
                    if (
                        (
                            channelID == TurbineChatChannel.General
                            && PropertyManager.GetBool("chat_disable_general").Item
                        )
                        ...
                        || (
                            channelID == TurbineChatChannel.Roleplay
                            && PropertyManager.GetBool("chat_disable_roleplay").Item
                        )
                    )
                    {
                        if (PropertyManager.GetBool("chat_echo_reject").Item)
    ...
    private static void HandleChatReject(
        ...
        uint contextId,
        ChatType chatType,
        GameMessageTurbineChat gameMessageTurbineChat,
        string rejectReason
    )
    {
        if (PropertyManager.GetBool("chat_echo_reject").Item)
        ...
        if (PropertyManager.GetBool("chat_inform_reject").Item)
        {
            session.Network.EnqueueSend(
                new GameEventCommunicationTransientString(
                    session,
                    $"{chatType} is currently disabled{(string.IsNullOrEmpty(rejectReason) ? "" : $" for you {rejectReason}")}."
                )
            );
            session.Network.EnqueueSend(
    ...
    private static void LogTurbineChat(uint channelID, string name, string message, uint senderID, ChatType chatType)
    {
        ...
        _log.Information(
            $"[CHAT][{chatType}]{(chatType == ChatType.Allegiance ? $"[{channelID}]" : "")} {name} says, \"{message}\""
        );
    }
}
