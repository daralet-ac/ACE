...
{
    private readonly ILogger _log = Log.ForContext<RawMotionState>();
    ...
    public RawMotionState(MoveToState moveToState, BinaryReader reader)
    {
        ...
        if (CommandListLength > 0)
        {
            ...
            for (var i = 0; i < CommandListLength; i++)
            {
                ...
                {
                    _log.Error(
                        $"RawMotionState reader - received non-standard action {motionItem.MotionCommand}, Speed: {motionItem.Speed} for {moveToState.WorldObject?.Name}"
                    );
                }
            }
        }
    ...
    {
        return (Flags & (RawMotionFlags.ForwardCommand | RawMotionFlags.TurnCommand | RawMotionFlags.SideStepCommand))
            != 0;
    }
    ...
    public bool HasSoulEmote(bool checkForward = true)
    {
        if (
            checkForward
            && (Flags & RawMotionFlags.ForwardCommand) != 0
            && SoulEmote.SoulEmotes.Contains(ForwardCommand)
        )
        {
            return true;
        }
