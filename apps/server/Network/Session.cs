...
public class Session
{
    ...
    public void DropSession()
    {
        ...
        if (PendingTermination.Reason != SessionTerminationReason.PongSentClosingConnection)
        {
            ...
            var reas = (reason != SessionTerminationReason.None) ? $", Reason: {reason.GetDescription()}" : "";
            ...
            {
                _log.Information(
                    $"Session {Network?.ClientId}\\{EndPointC2S} dropped. Account: {Account}, Player: {Player?.Name}{reas}"
                );
            }
            else
            {
                _log.Debug(
                    "Session {ClientId}\\{EndPointC2S} dropped. Account: {Account}, Player: {PlayerName}{Reason}",
                    Network?.ClientId,
                    EndPointC2S,
                    Account,
                    Player?.Name,
