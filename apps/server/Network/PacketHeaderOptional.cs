...
public class PacketHeaderOptional
{
    ...
    public void Unpack(BinaryReader reader, PacketHeader header)
    {
        ...
        var writer = new BinaryWriter(headerBytes);
        ...
        if (header.HasFlag(PacketHeaderFlags.RequestRetransmit)) // 0x1000
        {
            ...
            var retransmitCount = reader.ReadUInt32();
            writer.Write(retransmitCount);
            RetransmitData = new List<uint>();
            for (var i = 0u; i < retransmitCount; i++)
            {
                if (reader.BaseStream.Length < reader.BaseStream.Position + 4)
                {
                    IsValid = false;
        ...
        if (header.HasFlag(PacketHeaderFlags.LoginRequest)) // 0x10000
        {
            ...
            var length = reader.BaseStream.Length - position;
            if (length < 1)
            {
                IsValid = false;
                return;
    ...
    public override string ToString()
    {
        ...
        {
            nice +=
                $" requesting {RetransmitData.DefaultIfEmpty().Select(t => t.ToString()).Aggregate((a, b) => a + "," + b)}";
        }
