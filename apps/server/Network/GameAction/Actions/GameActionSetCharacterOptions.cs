...
public static class GameActionSetCharacterOptions
{
    ...
    public static void Handle(ClientMessage message, Session session)
    {
        if (!session.Player.FirstEnterWorldDone)
        {
            ...
            _log.Warning(
                $"{session.Player.Name} sent GameAction 0x1A1 - SetCharacterOptions before FirstEnterWorldDone, ignoring..."
            );
            return;
        }

        int characterOptions1Flag;
        var characterOptions2Flag = 0;
        uint spellbookFilters = 0;
        var desiredComponents = new Dictionary<uint, int>();

        // Thanks to tfarley (aclogview) for guidance on how to parse some of these flags.  The protocol docs are incomplete.

        // Flags
        var flags = message.Payload.ReadUInt32();
        ...
        {
            var numShortcuts = message.Payload.ReadUInt32();
        ...
        var numTab1Spells = message.Payload.ReadUInt32();

        if (numTab1Spells > 0)
        {
            var tab1Spells = new uint[numTab1Spells];
            for (var i = 0; i < numTab1Spells; i++)
            {
        ...
        if ((flags & (uint)CharacterOptionDataFlag.MultiSpellList) != 0)
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                var count = message.Payload.ReadUInt32();
        ...
        {
            var sizeInfo = message.Payload.ReadUInt32(); // sizeInfo
            var num = sizeInfo & 0xFFFF;
            for (var i = 0; i < num; ++i)
            {
        ...
        if ((flags & (uint)CharacterOptionDataFlag.GenericQualitiesData) != 0)
        {
            ...
            var genericQualitiesHeader = message.Payload.ReadUInt32();
            if ((genericQualitiesHeader & (uint)GenericQualitiesPackHeader.Packed_IntStats) != 0)
            {
                var sizeInfo = message.Payload.ReadUInt32();
                var _currNum = sizeInfo & 0xFFFF;
                for (var i = 0; i < _currNum; ++i)
                {
        ...
        if ((flags & (uint)CharacterOptionDataFlag.GameplayOptions) != 0)
        {
            ...
            var size = (int)(message.Payload.BaseStream.Length - message.Payload.BaseStream.Position);

            var gameplayOptions = new byte[size];
