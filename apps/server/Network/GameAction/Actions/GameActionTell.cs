    ...
    public static void Handle(ClientMessage clientMessage, Session session)
    {
        ...
        if (session.Player != targetPlayer)
        {
            session.Network.EnqueueSend(
                new GameMessageSystemChat($"You tell {targetPlayer.Name}, \"{message}\"", ChatMessageType.OutgoingTell)
            );
        }
        ...
        if (targetPlayer.IsAfk)
        {
            session.Network.EnqueueSend(
                new GameEventWeenieErrorWithString(
                    ...
                    $"{targetPlayer.Name} is away: "
                        + (
                            string.IsNullOrWhiteSpace(targetPlayer.AfkMessage)
                                ? WorldObjects.Player.DefaultAFKMessage
                                : targetPlayer.AfkMessage
                        )
                )
            );
            //return;
