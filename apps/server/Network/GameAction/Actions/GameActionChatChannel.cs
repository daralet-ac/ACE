            ...
            case Channel.Fellow:
                {
                    ...
                    {
                        if (
                            fellowmember.Session != session
                            && !fellowmember.SquelchManager.Squelches.Contains(
                                session.Player,
                                ChatMessageType.Fellowship
                            )
                        )
                        {
                            fellowmember.Session.Network.EnqueueSend(
                                new GameEventChannelBroadcast(
                                    fellowmember.Session,
                                    groupChatType,
                                    session.Player.Name,
                                    message
                                )
                        ...
                        else
                        {
                            session.Network.EnqueueSend(
                                new GameEventChannelBroadcast(session, groupChatType, "", message)
                            );
                        }
                    }
                }
            ...
            case Channel.Vassals:
                {
                    ...
                    foreach (var vassalGuid in session.Player.AllegianceNode.Vassals.Keys)
                    {
                        ...
                        if (
                            vassalPlayer != null
                            && !vassalPlayer.SquelchManager.Squelches.Contains(
                                session.Player,
                                ChatMessageType.Allegiance
                            )
                        )
                        {
                            vassalPlayer.Session.Network.EnqueueSend(
                                new GameEventChannelBroadcast(
                                    vassalPlayer.Session,
                                    groupChatType,
                                    session.Player.Name,
                                    message
                                )
            ...
            case Channel.Patron:
                {
                    ...
                    if (
                        patronPlayer != null
                        && !patronPlayer.SquelchManager.Squelches.Contains(session.Player, ChatMessageType.Allegiance)
                    )
                    {
                        patronPlayer.Session.Network.EnqueueSend(
                            new GameEventChannelBroadcast(
                                patronPlayer.Session,
                                groupChatType,
                                session.Player.Name,
                                message
                            )
            ...
            case Channel.Monarch:
                {
                    ...
                    if (
                        monarchPlayer != null
                        && !monarchPlayer.SquelchManager.Squelches.Contains(session.Player, ChatMessageType.Allegiance)
                    )
                    {
                        monarchPlayer.Session.Network.EnqueueSend(
                            new GameEventChannelBroadcast(
                                monarchPlayer.Session,
                                Channel.Monarch,
                                session.Player.Name,
                                message
                            )
            ...
            case Channel.CoVassals:
                {
                    ...
                    foreach (var covassalGuid in session.Player.AllegianceNode.Patron.Vassals.Keys)
                    {
                        ...
                        else
                        {
                            ...
                            if (
                                covassalPlayer != null
                                && !covassalPlayer.SquelchManager.Squelches.Contains(
                                    session.Player,
                                    ChatMessageType.Allegiance
                                )
                            )
                            {
                                covassalPlayer.Session.Network.EnqueueSend(
                                    new GameEventChannelBroadcast(
                                        covassalPlayer.Session,
                                        groupChatType,
                                        session.Player.Name,
                                        message
                                    )
            ...
            case Channel.AllegianceBroadcast:
                {
                    ...
                    {
                        session.Network.EnqueueSend(
                            new GameEventWeenieError(session, WeenieError.YouAreNotInAllegiance)
                        );
                        break;
                    }
                    ...
                    foreach (var member in player.Allegiance.Members.Keys)
                    {
                        ...
                        online.Session.Network.EnqueueSend(
                            new GameEventChannelBroadcast(online.Session, groupChatType, session.Player.Name, message)
                        );
                    }
                }
                break;
