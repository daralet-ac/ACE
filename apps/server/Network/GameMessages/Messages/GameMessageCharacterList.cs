...
{
    public GameMessageCharacterList(List<Character> characters, Session session)
        : base(GameMessageOpcode.CharacterList, GameMessageGroup.UIQueue)
    {
        Writer.Write(0u);
        ...
        foreach (var character in characters)
        {
            ...
            if (
                ...
                && session.AccessLevel > ACE.Entity.Enum.AccessLevel.Advocate
            )
            {
            ...
            Writer.Write(
                character.DeleteTime != 0ul ? (uint)Math.Max(1, Time.GetUnixTime() - character.DeleteTime) : 0u
            );
        }
