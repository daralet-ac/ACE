...
{
    public GameEventBookDataResponse(
        Session session,
        uint bookID,
        int maxChars,
        int maxPages,
        List<PropertiesBookPageData> pageData,
        string inscription,
        uint authorId,
        string authorName,
        bool ignoreAuthor
        ...
        : base(GameEventType.BookDataResponse, GameMessageGroup.UIQueue, session)
    {
        ...
        for (var i = 0; i < pageData.Count; i++)
        {
            ...
            if (
                ...
                || session.Player.IsSentinel
                || session.Player.IsEnvoy
                || session.Player.IsArch
                || session.Player.IsPsr
            )
            {
