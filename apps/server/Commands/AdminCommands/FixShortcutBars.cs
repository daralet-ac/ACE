...
public class FixShortcutBars
{
    ...
    public static void HandleFixShortcutBars(Session session, params string[] parameters)
    {
        ...
        {
            var results = ctx
                .CharacterPropertiesShortcutBar.FromSqlRaw(
            ...
            foreach (var result in results)
            {
                if (characterId != result.CharacterId)
                {
                    if (buggedChar)
                    {
                        buggedPlayerCount++;
                        Console.WriteLine(
                            $"Player {playerName} ({characterId}) was found to have errors in their shortcuts."
                        );
    ...
    public static List<string> OutputShortcutSQLCommand(
        string playerName,
        uint characterID,
        Dictionary<uint, uint> idxToObj
    )
    {
        ...
        {
            strings.Add(
                $"INSERT INTO `character_properties_shortcut_bar` SET `character_Id`={characterID}, `shortcut_Bar_Index`={shortcut.Key}, `shortcut_Object_Id`={shortcut.Value};"
            );
        }
