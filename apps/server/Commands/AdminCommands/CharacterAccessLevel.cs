...
public class CharacterAccessLevel
{
    ...
    public static void HandleCharacterTokenization(Session session, params string[] parameters)
    {
        ...
        DatabaseManager.Shard.SetCharacterAccessLevelByName(
            ...
            (
                (uint characterId) =>
                {
                    ...
                    else
                    {
                        CommandHandlerHelper.WriteOutputInfo(
                            session,
                            "There is no character by the name of "
                                + characterName
                                + " found in the database. Has it been deleted?",
                            ChatMessageType.Broadcast
                        );
                    }
                }
