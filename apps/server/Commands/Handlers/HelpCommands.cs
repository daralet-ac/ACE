...
public static class HelpCommands
{
    ...
    public static void HandleACEHelp(Session session, params string[] parameters)
    {
        ...
        {
            var msg =
                "Use @acecommands to get a complete list of commands available for you to use.\n"
                + "To get more information about a specific command, use @acehelp command\n";

            session.Network.EnqueueSend(
                new GameMessageSystemChat($"Unknown command: {parameters[0]}", ChatMessageType.Help),
    ...
    public static void HandleACECommands(Session session, params string[] parameters)
    {
        ...
        var commands =
            from cmd in CommandManager.GetCommands()
            where
                (exact ? cmd.Attribute.Access == accessLevel : cmd.Attribute.Access <= accessLevel)
                && cmd.Attribute.Flags != restrict
                && (
                    search != null
                        ? $"{cmd.Attribute.Access} {cmd.Attribute.Command} {cmd.Attribute.Description}".Contains(
                            search,
