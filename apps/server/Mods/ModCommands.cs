...
public static class ModCommands
{
    ...
    [CommandHandler("mod", AccessLevel.Developer, CommandHandlerFlag.None, -1, "Lazy mod control")]
    public static void HandleListMods(Session session, params string[] parameters)
    {
        if (parameters.Length < 1 || !Enum.TryParse<ModCommand>(parameters[0], true, out var verb))
        {
            Log(USAGE, session);
            return;
        }
        ...
        if (
            match is null
            && (
                verb == ModCommand.Enable
                || verb == ModCommand.Disable
                || verb == ModCommand.Toggle
                || verb == ModCommand.Restart
                || verb == ModCommand.Settings
            )
        )
        {
            Log(USAGE, session);
