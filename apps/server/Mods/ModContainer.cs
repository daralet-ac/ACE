...
public class ModContainer
{
    ...
    public string DllPath => Path.Combine(FolderPath, FolderName + ".dll");

    // C:\ACE\Mods\SomeMod\Meta.json
    public string MetadataPath => Path.Combine(FolderPath, "Meta.json");

    // MyModNamespace.Mod
    public string TypeName => ModAssembly.ManifestModule.ScopeName.Replace(".dll", "." + ModMetadata.TYPENAME);
    ...
    private void Reload(object sender, PluginReloadedEventArgs eventArgs)
    {
        ...
        Restart();
        _log.Information(
            $"Reloaded {FolderName} @ {DateTime.Now} after {lapsed.TotalSeconds}/{RELOAD_TIMEOUT.TotalSeconds} seconds"
        );
    }
