...
public static class PlayerFactoryEx
{
    ...
    private static CharacterCreateInfo CreateCharacterCreateInfo(
        string name,
        uint strength,
        uint endurance,
        uint coordination,
        ...
        uint focus,
        uint self,
        bool randomizeHeritageAndApperance = true
    )
    {
    ...
    private static void RandomizeHeritage(CharacterCreateInfo characterCreateInfo)
    {
        ...
        characterCreateInfo.Gender = (uint)
            heritageGroup.Genders.ElementAt(rand.Next(0, heritageGroup.Genders.Count)).Key;
    ...
    /// </summary>
    private static Player Create275Base(
    ...
    )
    {
        ...
        player.TryDequipObject(
            player.EquippedObjects.FirstOrDefault(k => k.Value.Name.Contains("Leather Boots")).Key,
            out var wo,
            out _
        );
        if (wo != null)
        ...
        player.TryRemoveFromInventory(
            player.Inventory.FirstOrDefault(k => k.Value.Name.Contains("Letter From Home")).Key
        );
    ...
    {
        if (
            player.Skills.ContainsKey(Skill.WarMagic)
            && player.Skills[Skill.WarMagic].AdvancementClass == SkillAdvancementClass.Specialized
        )
        {
            // War
            uint barNumber = 7;
    ...
    public static Player Create275HeavyWeapons(Weenie weenie, ObjectGuid guid, uint accountId, string name)
    {
        ...
        var profile = new Database.Models.World.TreasureDeath { Tier = 7, LootQualityMod = 0 };
    ...
    public static readonly HashSet<uint> CommonSpellComponents = new HashSet<uint>
    {
        691,
        689,
        686,
    ...
    {
        var imbuedEffectType = ImbuedEffectType.Undef;

        if (worldObject.W_DamageType == DamageType.Slash)
        {
            imbuedEffectType = ImbuedEffectType.SlashRending;
        }

        if (worldObject.W_DamageType == DamageType.Pierce)
        {
            imbuedEffectType = ImbuedEffectType.PierceRending;
        }

        if (worldObject.W_DamageType == DamageType.Bludgeon)
        {
            imbuedEffectType = ImbuedEffectType.BludgeonRending;
        }

        if (worldObject.W_DamageType == DamageType.Cold)
        {
            imbuedEffectType = ImbuedEffectType.ColdRending;
        }

        if (worldObject.W_DamageType == DamageType.Fire)
        {
            imbuedEffectType = ImbuedEffectType.FireRending;
        }

        if (worldObject.W_DamageType == DamageType.Acid)
        {
            imbuedEffectType = ImbuedEffectType.AcidRending;
        }

        if (worldObject.W_DamageType == DamageType.Electric)
        {
            imbuedEffectType = ImbuedEffectType.ElectricRending;
        }

        if (worldObject.W_DamageType == (DamageType.Slash | DamageType.Pierce))
        {
            imbuedEffectType = ImbuedEffectType.SlashRending;
        }
