...
public class StarterGearFactory
{
    ...
    private static StarterGearConfiguration LoadConfigFromResource()
    {
        ...
        try
        {
            ...
            config = JsonSerializer.Deserialize<StarterGearConfiguration>(
                starterGearText,
        ...
        {
            _log.Error(
                $"StarterGearFactory.LoadConfigFromResource() - {ex}: {ex.Message}; {ex.InnerException}: {ex.InnerException.Message}"
            );
        }
