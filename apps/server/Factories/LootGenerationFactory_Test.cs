...
public static class LootGenerationFactory_Test
{
    public static string TestLootGen(int numItems, int tier, bool logStats, string displayTable)
    {
        ...
        if (logStats)
        {
            var myfilename = string.Format("LootSim-{0:hh-mm-ss-tt_MM-dd-yyyy}.csv", DateTime.Now);
    ...
    public static string TestLootGenMonster(uint deathTreasureDID, int numCorpses, bool logStats, string displayTable)
    {
        ...
        {
            displayHeader +=
                $" Loot profile {deathTreasure.Id} (a Tier {deathTreasure.Tier} profile) from DID {deathTreasureDID} was used for creating {numCorpses} corpses. \n";
        }
        else
        {
        ...
        {
            var myfilename =
                $"LootSim_DeathTreasureDID-{deathTreasureDID}"
                + string.Format("_{0:hh-mm-ss-tt_MM-dd-yyyy}.csv", DateTime.Now);
            File.WriteAllText(myfilename, displayHeader + displayStats);
        }
