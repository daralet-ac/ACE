...
public static partial class LootGenerationFactory
{
    ...
    /// </summary>
    public static WorldObject CreateMeleeWeapon(
        TreasureDeath profile,
        bool isMagical,
        MeleeWeaponSkill weaponSkill = MeleeWeaponSkill.Undef,
        bool mutate = true
    )
    {
        var wcid = 0;
    ...
    private static bool MutateMeleeWeapon(
        WorldObject wo,
        TreasureDeath profile,
        bool isMagical,
    ...
    private static int GetMeleeMaxDamage(Skill weaponType, int wieldDiff, LootWeaponType baseWeapon)
    {
        ...
        var maxDamageVariance = ThreadSafeRandom.Next(-4, 0);
    ...
    private static bool GetMutateMeleeWeaponData(uint wcid)
    {
        ...
        for (var weaponType = 0; weaponType < LootTables.MeleeWeaponsMatrices.Count; weaponType++)
        {
            var lootTable = LootTables.MeleeWeaponsMatrices[weaponType];
            for (var subtype = 0; subtype < lootTable.Length; subtype++)
            {
    ...
    private static void TryMutateMeleeWeaponDamage(
        WorldObject wo,
        TreasureRoll roll,
        TreasureDeath profile,
        out int maxPossibleDamage
    )
    {
        maxPossibleDamage = 0;
        ...
        {
            _log.Error(
                "TryMutateMeleeWeaponDamage({WeaponName}, {TreasureRoll}, {TreasureDeath}) - DamageVariance is null.",
                wo.Name,
                roll,
                profile
            );
        ...
        {
            _log.Error(
                "TryMutateMeleeWeaponDamage({WeaponName}, {TreasureRoll}, {TreasureDeath}) - WeaponTime is null.",
                wo.Name,
                roll,
                profile
            );
