...
public static class WorldObjectFactory
{
    ...
    public static WorldObject CreateWorldObject(Weenie weenie, ObjectGuid guid)
    {
        ...
        switch (objWeenieType)
        {
            case WeenieType.Undef:
                _log.Warning(
                    $"CreateWorldObject: {weenie.GetName()} (0x{guid}:{weenie.WeenieClassId}) - WeenieType is Undef, Object cannot be created."
                );
                return null;
    ...
    )
    {
        ...
        foreach (var instance in sourceObjects.Where(x => x.IsLinkChild == false))
        {
            ...
            {
                _log.Warning(
                    $"CreateNewWorldObjects: Database does not contain weenie {instance.WeenieClassId} for instance 0x{instance.Guid:X8} at {new Position(instance.ObjCellId, instance.OriginX, instance.OriginY, instance.OriginZ, instance.AnglesX, instance.AnglesY, instance.AnglesZ, instance.AnglesW).ToLOCString()}"
                );
                continue;
            }
            ...
            if (biota == null)
            {
                ...
                worldObject.Location = new Position(
                    instance.ObjCellId,
                    instance.OriginX,
                    instance.OriginY,
                    instance.OriginZ,
            ...
            else
            {
                ...
                {
                    _log.Warning(
                        $"CreateNewWorldObjects: {worldObject.Name} (0x{worldObject.Guid}) Location was null. CreationTimestamp = {worldObject.CreationTimestamp} ({Common.Time.GetDateTimeFromTimestamp(worldObject.CreationTimestamp ?? 0).ToLocalTime()}) | Location restored from world db instance."
                    );
                    worldObject.Location = new Position(
                        instance.ObjCellId,
                        instance.OriginX,
