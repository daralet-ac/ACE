...
public static partial class LootGenerationFactory
{
    ...
    private static void AssignMagic_Gem(WorldObject wo, TreasureDeath profile)
    {
        ...
        var table =
            magicSchool == 0 ? GemSpells.GemCreatureSpellMatrix[spellLevel] : GemSpells.GemLifeSpellMatrix[spellLevel];
    ...
    private static bool AssignMagic_Gem_New(WorldObject wo, TreasureDeath profile, TreasureRoll roll, out int level)
    {
        ...
        {
            _log.Error(
                $"AssignMagic_Gem_New({wo.Name}, {profile.TreasureType}, {roll.ItemType}) - unknown spell {spell}"
            );
            return false;
        }
