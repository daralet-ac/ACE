...
internal static class SigilTrinketConfig
{
    ...
    private static string ResolveUseTextBuilder(string token, SigilStatRaw raw, int effectId)
    {
        ...
        return token switch
        {
            ...
            "dualwield_puzzle_box_assailment" => $"Whenever the wielder performs a critical hit on an enemy, there is a chance to gain a 50% critical damage buff, additively, for 10 seconds.\n\nCan only occur while dual-wielding, with a wield requirement of up to {{wieldReq}}.",
            ...
            "perception_goggles_exposure" => $"Whenever the wielder successfully uses the Expose Weakness ability, there is a chance to gain a 25% damage boost for 10 seconds.\n\nCan only occur while wielding a weapon with a wield requirement of up to {{wieldReq}}.",
