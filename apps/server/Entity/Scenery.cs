...
public class Scenery
{
            ...
            for (uint j = 0; j < scene.Objects.Count; j++)
            {
                ...
                if (noise < obj.Freq && obj.WeenieObj == 0)
                {
                    ...
                    if (
                        lx < 0
                        || ly < 0
                        || lx > LandblockMesh.LandblockSize
                        || ly > LandblockMesh.LandblockSize
                        || OnRoad(obj, landblock, lx, ly)
                    )
                    {
                        continue;
                    }
                    ...
                    model.Rotation = Quaternion.CreateFromYawPitchRoll(
                        0,
                        0,
                        RotateObj(obj, globalCellX, globalCellY, j)
                    );
    ...
    public static Vector2 Displace(ObjectDesc obj, uint ix, uint iy, uint iq)
    {
        ...
        {
            x = (float)(
                (1813693831 * iy - (iq + 45773) * (1360117743 * iy * ix + 1888038839) - 1109124029 * ix)
                    * 2.3283064e-10
                    * obj.DisplaceX
                + loc.X
            );
        ...
        {
            y = (float)(
                (1813693831 * iy - (iq + 72719) * (1360117743 * iy * ix + 1888038839) - 1109124029 * ix)
                    * 2.3283064e-10
                    * obj.DisplaceY
                + loc.Y
            );
    ...
    public static float RotateObj(ObjectDesc obj, uint x, uint y, uint k)
    {
        ...
        return (float)(
            (1813693831 * y - (k + 63127) * (1360117743 * y * x + 1888038839) - 1109124029 * x)
            * 2.3283064e-10
            * obj.MaxRotation
            * 0.0174533f
        );
