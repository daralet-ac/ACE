...
public class DamageHistory
{
    ...
    public readonly Dictionary<ObjectGuid, DamageHistoryInfo> TotalDamage =
        new Dictionary<ObjectGuid, DamageHistoryInfo>();
    ...
    {
        var sorted = TotalDamage
            .Values.Where(wo => includeSelf || wo.Guid != Creature.Guid)
            .OrderByDescending(wo => wo.TotalDamage);
    ...
    private void OnHealInternal(uint healAmount, uint currentHealth, uint maxHealth)
    {
        ...
        if (healAmount == 0 || missingHealth == 0)
        {
            return;
        }
