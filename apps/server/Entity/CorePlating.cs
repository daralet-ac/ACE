...
public class CorePlating
{
    ...
    public static void UseObjectOnTarget(Player player, WorldObject source, WorldObject target)
    {
        ...
        var clapTime = Physics.Animation.MotionTable.GetAnimationLength(
            player.MotionTableId,
            currentStance,
            motionCommand
        );
    ...
    public static WeenieError VerifyUseRequirements(Player player, WorldObject source, WorldObject target)
    {
        ...
        if ((target.ValidLocations & (EquipMask.Clothing | EquipMask.Armor)) == 0)
        {
            player.SendTransientError(
                $"You can't use the {source.Name} on {target.Name} because it is not a piece of armor or clothing."
            );
            return WeenieError.YouDoNotPassCraftingRequirements;
        }
    ...
    public static void Integrate(Player player, WorldObject source, WorldObject target)
    {
        ...
        player.UpdateProperty(
            target,
            PropertyString.Use,
            "This Aetherium core plating installs into the frame of a Gear Knight to strengthen it."
        );
