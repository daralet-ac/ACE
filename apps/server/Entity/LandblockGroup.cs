...
public class LandblockGroup : IEnumerable<Landblock>
{
    ...
    private const int numberOfUniqueLandblocksRemovedBeforeSplitEligibility =
        LandblockGroupMinSpacing * LandblockGroupMinSpacing;
    ...
    public bool Add(Landblock landblock)
    {
        if (landblocks.Count > 0)
        {
            ...
            {
                _log.Error(
                    $"[LANDBLOCK GROUP] You cannot add a landblock ({landblock.Id}) to a LandblockGroup that represents a single Dungeon Landblock"
                );
                return false;
            }
            ...
            {
                _log.Error(
                    $"[LANDBLOCK GROUP] You cannot add a dungeon landblock ({landblock.Id}) to an existing LandblockGroup"
                );
                return false;
            }
        }
        ...
        if (landblocks.Add(landblock))
        {
            ...
            {
                XMin = landblock.Id.LandblockX;
            }

            if (landblock.Id.LandblockX > XMax)
            {
                XMax = landblock.Id.LandblockX;
            }

            if (landblock.Id.LandblockY < YMin)
            {
                YMin = landblock.Id.LandblockY;
            }

            if (landblock.Id.LandblockY > YMax)
            {
                YMax = landblock.Id.LandblockY;
            }
    ...
    public bool Remove(Landblock landblock)
    {
        if (landblocks.Remove(landblock))
        {
            ...
            if (
                landblock.Id.LandblockX == XMin
                || landblock.Id.LandblockX == XMax
                || landblock.Id.LandblockY == YMin
                || landblock.Id.LandblockY == YMax
            )
            {
    ...
    private void RecalculateBoundaries()
    {
        ...
        {
            if (existing.Id.LandblockX < XMin)
            {
                XMin = existing.Id.LandblockX;
            }

            if (existing.Id.LandblockX > XMax)
            {
                XMax = existing.Id.LandblockX;
            }

            if (existing.Id.LandblockY < YMin)
            {
                YMin = existing.Id.LandblockY;
            }

            if (existing.Id.LandblockY > YMax)
            {
                YMax = existing.Id.LandblockY;
            }
        }
    ...
    private LandblockGroup DoTrySplit()
    {
        ...
        for (var i = remainingLandblocks.Count - 1; i >= 0; i--)
        {
    ...
    {
        if (
            width < landblockGroupSpanRequiredBeforeSplitEligibility
            && height < landblockGroupSpanRequiredBeforeSplitEligibility
        )
        {
            return null;
        }
