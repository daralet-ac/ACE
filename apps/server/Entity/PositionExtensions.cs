...
public static class PositionExtensions
{
    ...
    public static string GetMapCoordStr(this Position pos)
    {
        ...
        return string.Format("{0:0.0}", Math.Abs(mapCoords.Value.Y) - 0.05f)
            + northSouth
            + ", "
            + string.Format("{0:0.0}", Math.Abs(mapCoords.Value.X) - 0.05f)
            + eastWest;
    }
    ...
    {
        _log.Warning(
            $"detected bad quaternion x y z w for {wo.Name} (0x{wo.Guid}) | WCID: {wo.WeenieClassId} | WeenieType: {wo.WeenieType} | PositionType: {positionType}"
        );
