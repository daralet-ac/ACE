...
public class Proficiency
{
    ...
    public static void OnSuccessUse(Player player, CreatureSkill skill, uint difficulty)
    {
        ...
        if (timeDiff < 0)
        {
            ...
            _log.Warning(
                $"Proficiency.OnSuccessUse({player.Name}, {skill.Skill}, {difficulty}) - timeDiff: {timeDiff}"
            );
        ...
        if (difficulty_check || time_check)
        {
            ...
            {
                _log.Warning(
                    $"Proficiency.OnSuccessUse({player.Name}, {skill.Skill}, {difficulty}) - totalXPGranted: {totalXPGranted:N0}"
                );
            }
    ...
    public static void OnSuccessUse(Player player, CreatureSkill skill, int difficulty)
    {
        ...
        {
            _log.Error(
                $"Proficiency.OnSuccessUse({player.Name}, {skill.Skill}, {difficulty}) - difficulty cannot be negative"
            );
            return;
        }
