...
public class Cloak
{
    ...
    public static bool RollProc(WorldObject cloak, float damage_percent)
    {
        ...
        if (UseCustomMath)
        {
            ...
            var maxProcAtDamagePercent = Convert.ToSingle(
                PropertyManager.GetDouble("cloak_max_proc_damage_percentage", 30.0).Item
            );
    ...
    public static bool HandleProcSpell(Creature defender, WorldObject attacker, WorldObject cloak)
    {
            ...
            if (defender is Player player)
            {
                if (spell._spellBase == null)
                {
                    player.Session.Network.EnqueueSend(
                        new GameMessageSystemChat($"SpellId {cloak.ProcSpell} Invalid.", ChatMessageType.System)
                    );
                }
                else
                {
        ...
        var msg = new GameMessageSystemChat(
            $"The cloak of {defender.Name} weaves the magic of {spell.Name}!",
    ...
    public static void ShowMessage(Creature defender, WorldObject attacker, int origDamage, int reducedDamage)
    {
        ...
        if (defender is Player playerDefender)
        {
            playerDefender.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"Your cloak {suffix}", ChatMessageType.Magic)
            );
        }
        ...
        if (attacker is Player playerAttacker)
        {
            playerAttacker.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"The cloak of {defender.Name} {suffix}", ChatMessageType.Magic)
            );
        }
    }
