...
public static class HouseList
{
    ...
    public static void AddToAvailable(SlumLord slumLord, House houseToAdd)
    {
        ...
        var landblockInstance =
            slumLord.House?.LinkedInstances.FirstOrDefault(i => i.Guid == slumLord.Guid.Full)
            ?? houseToAdd.LinkedInstances.FirstOrDefault(i => i.Guid == slumLord.Guid.Full);
        //var landblockInstance = houseToAdd.LinkedInstances.FirstOrDefault(i => i.Guid == slumLord.Guid.Full);

        if (weenie == null || landblockInstance == null)
        {
            return;
        }
