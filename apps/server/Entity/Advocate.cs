...
{
    public const Channel AdvocateChannels =
        Channel.Help | Channel.Abuse | Channel.Advocate1 | Channel.Advocate2 | Channel.Advocate3;
    ...
    public static bool Bestow(WorldObject playerToBeBestowed, int advocateLevelToBestow)
    {
        ...
        player.Session.Network.EnqueueSend(
            new GameMessagePrivateUpdatePropertyBool(player, PropertyBool.IsAdvocate, player.IsAdvocate)
        );
        ...
        if (player.AdvocateLevel.HasValue)
        {
            ...
            objectsToBeConsumed.AddRange(
                player.GetInventoryItemsOfWeenieClass($"shieldadvocate{player.AdvocateLevel}")
            );
            ...
            equippedAegis.AddRange(
                player
                    .EquippedObjects.Values.Where(a =>
                        a.WeenieClassName.Equals(
                            $"shieldadvocate{player.AdvocateLevel}",
        ...
        player.Session.Network.EnqueueSend(
            new GameMessagePrivateUpdatePropertyInt(player, PropertyInt.AdvocateLevel, player.AdvocateLevel ?? 1)
        );
        ...
        if (player.AdvocateLevel > 4)
        {
            player.IsPsr = true;
            player.Session.Network.EnqueueSend(
                new GameMessagePrivateUpdatePropertyBool(player, PropertyBool.IsPsr, player.IsPsr)
            );
        }
        ...
        player.Session.Network.EnqueueSend(
            new GameMessageSystemChat(
                $"You have been bestowed as an Advocate, level {player.AdvocateLevel}!",
    ...
    public static bool Remove(WorldObject playerToBeRemoved)
    {
        ...
        for (var i = 1; i < 8; i++)
        {
            equippedAegis.AddRange(
                player
                    .EquippedObjects.Values.Where(a =>
                        a.WeenieClassName.Equals($"shieldadvocate{i}", StringComparison.OrdinalIgnoreCase)
                    )
                    .ToList()
            );
        }
        ...
        player.Session.Network.EnqueueSend(
            new GameMessagePrivateUpdatePropertyInt(player, PropertyInt.AdvocateLevel, 0)
        );
        player.Session.Network.EnqueueSend(
            new GameMessagePrivateUpdatePropertyBool(player, PropertyBool.IsAdvocate, false)
        );
        ...
        player.Session.Network.EnqueueSend(
            new GameMessageSystemChat($"You have been removed from the Advocate ranks!", ChatMessageType.Broadcast)
        );
