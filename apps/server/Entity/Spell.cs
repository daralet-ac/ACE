...
public partial class Spell : IEquatable<Spell>
{
    ...
    {
        get => _spellBase == null || _spell == null;
    }
    ...
    public void Init(uint spellID, bool loadDB = true)
    {
        ...
        {
            _log.Debug(
                $"Spell.Init(spellID = {spellID}, loadDB = {loadDB}) failed! {(_spell == null ? "_spell was null" : "")} {(_spellBase == null ? "_spellBase was null" : "")}"
            );
        }
    ...
    public uint Level
    {
        ...
        get
        {
            for (var spellLevel = SpellFormula.MaxSpellLevel; spellLevel > 0; spellLevel--)
            {
    ...
    public bool IsImpenBaneType
    {
        get
        {
            switch (Category)
            {
                case SpellCategory n
                    when n >= SpellCategory.ArmorValueRaising && n <= SpellCategory.AcidicResistanceLowering:
    ...
    public bool UpdatesRunRate
    {
        get
        {
            ...
            if (
                StatModType.HasFlag(EnchantmentTypeFlags.Attribute)
                && (StatModKey == (uint)PropertyAttribute.Strength || StatModKey == (uint)PropertyAttribute.Quickness)
            )
            {
                return true;
            }
