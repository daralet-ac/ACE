...
public class Tailoring
{
    ...
    public static bool HasAvailableSpace(Player player, uint sourceWCID, uint targetWCID, uint resultWCID)
    {
        ...
        if (itemsToReceive.PlayerExceedsLimits)
        {
            ...
            {
                player.Session.Network.EnqueueSend(
                    new GameEventCommunicationTransientString(player.Session, "You are too encumbered to tailor that!")
                );
            }
    ...
    public static void Finalize(Player player, WorldObject source, WorldObject target, WorldObject result)
    {
        ...
        {
            _log.Error(
                $"[TAILORING] Tailoring.Finalize({player.Name} (0x{player.Guid}), {source.Name} (0x{source.Guid}), {target.Name} (0x{target.Guid}), {result.Name}) - couldn't add {result.Name} ({result.Guid}) to player inventory after validation, this shouldn't happen!"
            );
    ...
    public static void TailorWeapon(Player player, WorldObject source, WorldObject target)
    {
        ...
        player.Session.Network.EnqueueSend(
            new GameMessageSystemChat("You tailor the appearance off the weapon.", ChatMessageType.Broadcast)
        );
    ...
    public static void TailorReduceArmor(Player player, WorldObject source, WorldObject target)
    {
        ...
        player.Session.Network.EnqueueSend(
            new GameMessageSystemChat("You modify your armor.", ChatMessageType.Broadcast)
        );
    ...
    public static void WeaponApply(Player player, WorldObject source, WorldObject target)
    {
        ...
        switch (source.TargetType)
        {
            ...
            case ItemType.MissileWeapon:
                ...
                if (
                    source.DefaultCombatStyle != target.DefaultCombatStyle
        ...
        player.Session.Network.EnqueueSend(
            new GameMessageSystemChat("You tailor the appearance onto a different weapon.", ChatMessageType.Broadcast)
        );
    ...
    public static void UpdateArmorProps(Player player, WorldObject source, WorldObject target)
    {
        ...
        {
            player.UpdateProperty(
                target,
                PropertyDataId.IconOverlay,
                source.GetProperty(PropertyDataId.IconOverlaySecondary)
            );
        }
    ...
    public static uint? GetArmorWCID(EquipMask validLocations)
    {
        ...
        if (
            validLocations.HasFlag(EquipMask.Armor)
            || validLocations == EquipMask.Cloak
            || validLocations == EquipMask.Shield
            || validLocations.HasFlag(EquipMask.ChestWear)
            || validLocations.HasFlag(EquipMask.AbdomenWear)
        )
        {
            return Tentacles;
        }
