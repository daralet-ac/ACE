...
public class ChessMatch
{
            ...
            if (ai_enabled > 0)
            {
                player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"If another player doesn't join within {ai_enabled} seconds, the game will automatically start with AI",
    ...
    public void Finish(int winner)
    {
        ...
        foreach (var side in Sides)
        {
            ...
            var player = PlayerManager.FindByGuid(side.PlayerGuid, out var isOnline);
            ...
            if (winner != Chess.ChessWinnerEndGame)
            {
                ...
                if (isOnline)
                {
                    onlinePlayer.Session.Network.EnqueueSend(
                        new GameMessagePrivateUpdatePropertyInt(onlinePlayer, PropertyInt.ChessTotalGames, totalGames)
                    );
                }
            }
            ...
            if (winner >= 0)
            {
                ...
                if (winnerColor == side.Color)
                {
                    ...
                    {
                        onlinePlayer.Session.Network.EnqueueSend(
                            new GameMessagePrivateUpdatePropertyInt(onlinePlayer, PropertyInt.ChessGamesWon, won)
                        );
                    }
                }
                else
                {
                    ...
                    {
                        onlinePlayer.Session.Network.EnqueueSend(
                            new GameMessagePrivateUpdatePropertyInt(onlinePlayer, PropertyInt.ChessGamesLost, lost)
                        );
                    }
                }
            }
    ...
    {
        player.Session.Network.EnqueueSend(
            new GameEventOpponentTurn(
                player.Session,
                ChessBoard.Guid,
                new ChessMoveData(opponentGuid, piece.Guid, data)
            )
        );
    }
    ...
    {
        player.Session.Network.EnqueueSend(
            new GameEventOpponentStalemate(player.Session, ChessBoard.Guid, color, stalemate)
        );
    }
    ...
    public static void AdjustPlayerRanks(ObjectGuid playerGuid, ObjectGuid opponentGuid, ObjectGuid winnerGuid)
    {
        ...
        var opponent = PlayerManager.FindByGuid(opponentGuid, out var opponentIsOnline);
        ...
        if (playerIsOnline)
        {
            ...
            onlinePlayer.Session.Network.EnqueueSend(
                new GameMessagePrivateUpdatePropertyInt(onlinePlayer, PropertyInt.ChessRank, rank + delta)
            );
        }
        ...
        if (opponent != null && opponentIsOnline)
        {
            ...
            onlineOp.Session.Network.EnqueueSend(
                new GameMessagePrivateUpdatePropertyInt(onlineOp, PropertyInt.ChessRank, opRank - delta)
            );
        }
    }
