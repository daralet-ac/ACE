...
public class ChessLogic
{
    ...
    public ChessMoveResult AsyncCalculateAiSimpleMove(
        ChessAiAsyncTurnKey key,
        ref ChessPieceCoord from,
        ref ChessPieceCoord to
    )
    {
        var color = Turn;
    ...
    public ChessMoveResult AsyncCalculateAiComplexMove(
        ChessAiAsyncTurnKey key,
        ref ChessPieceCoord from,
        ref ChessPieceCoord to,
        ref uint counter
    ...
    public void BuildMove(
        List<ChessMove> storage,
        ChessMoveFlag result,
        ChessColor color,
        ChessPieceType type,
        ChessPieceCoord from,
        ChessPieceCoord to
    )
    {
        var fromPiece = GetPiece(from);
    ...
    public ChessMoveResult FinalizeMove(ChessMove move)
    {
        ...
        var result =
            (move.Flags & (ChessMoveFlag.Capture | ChessMoveFlag.EnPassantCapture)) != 0
    ...
    public void InternalMove(ChessMove move)
    {
        ...
        if (move.Type == ChessPieceType.Pawn || (flags & (ChessMoveFlag.Capture | ChessMoveFlag.EnPassantCapture)) != 0)
        {
            HalfMove = 0;
        }
        else
