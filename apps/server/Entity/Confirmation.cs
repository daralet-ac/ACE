...
public class Confirmation_AlterAttribute : Confirmation
{
    ...
    public override void ProcessConfirmation(bool response, bool timeout = false)
    {
        ...
        var attributeTransferDevice =
            player.FindObject(AttributeTransferDevice.Full, Player.SearchLocations.MyInventory)
            as AttributeTransferDevice;
...
public class Confirmation_AlterSkill : Confirmation
{
    ...
    public override void ProcessConfirmation(bool response, bool timeout = false)
    {
        ...
        var skillAlterationDevice =
            player.FindObject(SkillAlterationDevice.Full, Player.SearchLocations.MyInventory) as SkillAlterationDevice;
...
public class Confirmation_Augmentation : Confirmation
{
    ...
    public override void ProcessConfirmation(bool response, bool timeout = false)
    {
        ...
        var augmentation =
            player.FindObject(AugmentationGuid.Full, Player.SearchLocations.MyInventory) as AugmentationDevice;
...
public class Confirmation_Fellowship : Confirmation
{
    ...
    public override void ProcessConfirmation(bool response, bool timeout = false)
    {
        ...
        {
            inviter?.SendMessage(
                $"{invited.Name} {(timeout ? "did not respond to" : "has declined")} your offer of fellowship."
            );
            return;
        }
...
public class Confirmation_SwearAllegiance : Confirmation
{
    ...
    public override void ProcessConfirmation(bool response, bool timeout = false)
    {
        ...
        {
            vassal?.SendMessage(
                $"{patron.Name} {(timeout ? "did not respond to" : "has declined")} your offer of allegiance."
            );
            return;
        }
...
public class Confirmation_YesNo : Confirmation
{
    ...
    public override void ProcessConfirmation(bool response, bool timeout = false)
    {
        ...
        if (source != null)
        {
            source.EmoteManager.ExecuteEmoteSet(
                response ? EmoteCategory.TestSuccess : EmoteCategory.TestFailure,
                Quest,
                player
            );
        }
