...
public class Fellowship
{
    ...
    public Fellowship(Player leader, string fellowshipName, bool shareXP)
    {
        ...
        FellowshipMembers = new Dictionary<uint, WeakReference<Player>>()
        {
            { leader.Guid.Full, new WeakReference<Player>(leader) }
        };
    ...
    public void AddFellowshipMember(Player inviter, Player newMember)
    {
        ...
        if (IsLocked)
        {
            ...
            {
                inviter.Session.Network.EnqueueSend(
        ...
        {
            inviter.Session.Network.EnqueueSend(
                new GameEventWeenieError(inviter.Session, WeenieError.YourFellowshipIsFull)
            );
            return;
        }
        ...
        else
        {
            ...
            {
                inviter.Session.Network.EnqueueSend(
                    new GameMessageSystemChat($"{newMember.Name} is busy.", ChatMessageType.Broadcast)
                );
                return;
            }
            ...
            {
                if (
                    !newMember.ConfirmationManager.EnqueueSend(
                        new Confirmation_Fellowship(inviter.Guid, newMember.Guid),
                        inviter.Name
                    )
                )
                {
    ...
    {
        if (inviter == null || inviter.Session == null || inviter.Session.Player == null || player == null)
        {
            return;
        }
        ...
        if (!response)
        {
            ...
            inviter.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{player.Name} declines your invite", ChatMessageType.Fellowship)
            );
            inviter.Session.Network.EnqueueSend(
                new GameEventWeenieError(inviter.Session, WeenieError.FellowshipDeclined)
            );
            return;
        }
        ...
        if (ShareLoot)
        {
            foreach (var member in fellowshipMembers.Values.Where(i => i.Guid != player.Guid))
            {
                member.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"{player.Name} has given you permission to loot his or her kills.",
                        ChatMessageType.Broadcast
                    )
                );
                member.Session.Network.EnqueueSend(
                    new GameMessageSystemChat($"{player.Name} may now loot your kills.", ChatMessageType.Broadcast)
                );

                player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat(
                        $"{member.Name} has given you permission to loot his or her kills.",
                        ChatMessageType.Broadcast
                    )
                );
                player.Session.Network.EnqueueSend(
                    new GameMessageSystemChat($"{member.Name} may now loot your kills.", ChatMessageType.Broadcast)
                );
            }
        }
    ...
    public void RemoveFellowshipMember(Player player, Player leader)
    {
        ...
        {
            _log.Warning(
                "{Leader} tried to dismiss {Player} from the fellowship, but player was not found in the fellowship",
                leader.Name,
                player.Name
            );
            ...
            if (player.Fellowship != null)
            {
                ...
                {
                    _log.Warning(
                        "{Player} still has a reference to this fellowship somehow. This shouldn't happen",
                        player.Name
                    );
                    done = false;
                }
                ...
                {
                    _log.Warning(
                        "{Player} has a reference to a different fellowship. {Leader} is possibly sending crafted data!",
                        player.Name,
                        leader.Name
                    );
                }
        ...
        foreach (var member in fellowshipMembers.Values)
        {
            ...
            member.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{player.Name} dismissed from fellowship", ChatMessageType.Fellowship)
            );
        }
    ...
    private void SendBroadcastAndUpdate(string message)
    {
        ...
        {
            member.Session.Network.EnqueueSend(
                new GameEventChannelBroadcast(member.Session, Channel.FellowBroadcast, "", message)
            );
    ...
    public void TellFellow(WorldObject sender, string message)
    {
        ...
        {
            member.Session.Network.EnqueueSend(
                new GameEventChannelBroadcast(member.Session, Channel.Fellow, sender.Name, message)
            );
        }
    }
            ...
            else
            {
                ...
                foreach (var member in fellowshipMembers.Values)
                {
                    ...
                    if (ShareLoot)
                    {
                        member.Session.Network.EnqueueSend(
                            new GameMessageSystemChat(
                                $"You have lost permission to loot the kills of {player.Name}.",
                                ChatMessageType.Broadcast
                            )
                        );
                        player.Session.Network.EnqueueSend(
                            new GameMessageSystemChat(
                                $"{member.Name} does not have permission to loot your kills.",
                                ChatMessageType.Broadcast
                            )
                        );
                    }
    ...
    public void AssignNewLeader(Player oldLeader, Player newLeader)
    {
        if (newLeader != null)
        {
            ...
            if (oldLeader != null)
            {
                oldLeader.Session.Network.EnqueueSend(
                    new GameEventWeenieErrorWithString(
                        oldLeader.Session,
    ...
    public double GetDistanceScalar(Player earner, Player fellow, XpType xpType)
    {
        ...
        if (
            earner.Location.Indoors
            && fellow.Location.Indoors
            && earner.Location.Landblock != fellow.Location.Landblock
        )
        {
            return 0.0f;
        }
    ...
    public List<Player> WithinRange(Player player, bool includeSelf = false)
    {
        ...
        foreach (var fellow in fellows.Values)
        {
            ...
            var shareable =
                player == fellow || landblockRange
                    ? player.CurrentLandblock == fellow.CurrentLandblock
                        || player.Location.DistanceTo(fellow.Location) <= 192.0f
                    : player.Location.Distance2D(fellow.Location) <= player.CurrentRadarRange
                        && player.ObjMaint.VisibleObjectsContainsKey(fellow.Guid.Full); // 2d visible distance / radar range?
    ...
    public void OnFellowLevelUp(Player player)
    {
        ...
        foreach (var fellow in fellowshipMembers.Values)
        {
            ...
            fellow.Session.Network.EnqueueSend(
                new GameMessageSystemChat($"{player.Name} is now level {player.Level}!", ChatMessageType.Broadcast)
            );
        }
    }
    ...
    public void OnVitalUpdate(Player player)
    {
        ...
        foreach (var fellow in fellowshipMembers.Values)
        {
            ...
            {
                fellow.Session.Network.EnqueueSend(
                    new GameEventFellowshipUpdateFellow(fellow.Session, player, ShareLoot, FellowUpdateType.Vitals)
                );
            }
        }
    }
    ...
    public void OnDeath(Player player)
    {
        ...
        foreach (var fellow in fellowshipMembers.Values)
        {
            ...
            {
                fellow.Session.Network.EnqueueSend(
                    new GameMessageSystemChat($"Your fellow {player.Name} has died!", ChatMessageType.Broadcast)
                );
            }
        }
    }
