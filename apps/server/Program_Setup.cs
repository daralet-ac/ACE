...
partial class Program
{
    private static void DoOutOfBoxSetup(string configFile)
    {
        ...
        else
        {
            ...
            {
                Console.WriteLine(
                    "config.js Configuration file is missing, ACEmulator is running in a container, cloning from docker file."
                );
        ...
        Console.WriteLine();
        Console.WriteLine(
            "Welcome to ACEmulator! To configure your world for first use, please follow the instructions below. Press enter at each prompt to accept default values."
        );
        Console.WriteLine();
        Console.WriteLine();
        ...
        Console.Write(
            $"Enter the directory location for your DAT files (default: \"{config.Server.DatFilesDirectory}\"): "
        );
        if (!nonInteractiveSetup)
        {
            variable = Console.ReadLine();
        ...
        Console.WriteLine();
        Console.WriteLine(
            "Next we will configure your SQL server connections. You will need to provide a database name, username and password for each."
        );
        Console.WriteLine(
            "Default names for the databases are recommended, and it is also recommended you not use root for login to database. The password must not be blank."
        );
        Console.WriteLine(
            "It is also recommended the SQL server be hosted on the same machine as this server, so defaults for Host and Port would be ideal as well."
        );
        ...
        Console.Write(
            $"Enter the database name for your authentication database (default: \"{config.MySql.Authentication.Database}\"): "
        );
        if (!nonInteractiveSetup)
        {
            variable = Console.ReadLine();
        ...
        Console.Write(
            $"Enter the database name for your shard database (default: \"{config.MySql.Shard.Database}\"): "
        );
        if (!nonInteractiveSetup)
        {
            variable = Console.ReadLine();
        ...
        Console.Write(
            $"Enter the database name for your world database (default: \"{config.MySql.World.Database}\"): "
        );
        if (!nonInteractiveSetup)
        {
            variable = Console.ReadLine();
        ...
        Console.WriteLine();
        Console.Write(
            "Typically, all three databases will be on the same SQL server, is this how you want to proceed? (Y/n) "
        );
        if (!nonInteractiveSetup)
        {
            variable = Console.ReadLine();
        ...
        {
            Console.Write(
                $"Enter the Host address for your authentication database (default: \"{config.MySql.Authentication.Host}\"): "
            );
            if (!nonInteractiveSetup)
            {
                variable = Console.ReadLine();
            ...
            Console.Write(
                $"Enter the Port for your authentication database (default: \"{config.MySql.Authentication.Port}\"): "
            );
            if (!nonInteractiveSetup)
            {
                variable = Console.ReadLine();
        ...
        Console.WriteLine();
        Console.Write(
            "Typically, all three databases will be on the using the same SQL server credentials, is this how you want to proceed? (Y/n) "
        );
        if (!nonInteractiveSetup)
        {
            variable = Console.ReadLine();
        ...
        {
            Console.Write(
                $"Enter the username for your authentication database (default: \"{config.MySql.Authentication.Username}\"): "
            );
            variable = Console.ReadLine();
            if (!string.IsNullOrWhiteSpace(variable))
            {
            ...
            Console.Write(
                $"Enter the password for your authentication database (default: \"{config.MySql.Authentication.Password}\"): "
            );
            variable = Console.ReadLine();
            if (!string.IsNullOrWhiteSpace(variable))
            {
        ...
        Console.WriteLine();
        Console.Write(
            "Do you want to ACEmulator to attempt to initialize your SQL databases? This will erase any existing ACEmulator specific databases that may already exist on the server (Y/n): "
        );
        if (!nonInteractiveSetup)
        {
            variable = Console.ReadLine();
        ...
        var sqlConnectInfo =
            $"server={config.MySql.World.Host};port={config.MySql.World.Port};user={config.MySql.World.Username};password={config.MySql.World.Password};{config.MySql.World.ConnectionOptions}";
        var sqlConnect = new MySqlConnector.MySqlConnection(sqlConnectInfo);
        if (nonInteractiveSetup)
        {
            variable = Convert.ToBoolean(Environment.GetEnvironmentVariable("ACE_SQL_INITIALIZE_DATABASES"))
                ? "y"
                : "n";
            Console.WriteLine($"{variable}");
        }
        ...
        )
        {
            ...
            Console.Write(
                $"Waiting for connection to SQL server at {config.MySql.World.Host}:{config.MySql.World.Port} .... "
            );
            for (; ; )
            {
                try
                {
                    using (
                        var sqlTestConnection = new MySqlConnector.MySqlConnection(
                            $"server={config.MySql.World.Host};port={config.MySql.World.Port};user={config.MySql.World.Username};password={config.MySql.World.Password};{config.MySql.World.ConnectionOptions}"
                        )
                    )
                    {
        ...
        {
            variable = Convert.ToBoolean(Environment.GetEnvironmentVariable("ACE_SQL_DOWNLOAD_LATEST_WORLD_RELEASE"))
                ? "y"
                : "n";
            Console.WriteLine($"{variable}");
        }
        ...
        )
        {
            ...
            if (IsRunningInContainer)
            {
                Console.WriteLine(" ");
                Console.WriteLine(
                    "This process will take a while, depending on many factors, and may look stuck while reading and importing the world database, please be patient! "
                );
                Console.WriteLine(" ");
            }
            ...
            var dbURL = json.GetProperty("assets")[0].GetProperty("browser_download_url").GetString();
            var dbFileName = json.GetProperty("assets")[0].GetProperty("name").GetString();
            ...
            Console.Write(
                $"Importing {sqlFile} into SQL server at {config.MySql.World.Host}:{config.MySql.World.Port} (This will take a while, please be patient) .... "
            );
