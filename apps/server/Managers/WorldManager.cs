...
public static class WorldManager
{
    ...
    public static void Initialize()
    {
        ...
        _log.Debug(
            "Current maximum allowed sessions: {MaximumAllowedSessions}",
            ConfigManager.Config.Server.Network.MaximumAllowedSessions
        );

        _log.Information(
            $"World started and is currently {WorldStatus.ToString()}{(PropertyManager.GetBool("world_closed", false).Item ? "" : " and will open automatically when server startup is complete.")}"
        );
    ...
    public static void PlayerEnterWorld(Session session, Character character)
    {
        ...
        {
            _log.Error(
                $"PlayerEnterWorld requested for character.Id 0x{character.Id:X8} not found in PlayerManager OfflinePlayers."
            );
            return;
        }
        ...
        DatabaseManager.Shard.GetPossessedBiotasInParallel(
            ...
            {
                _log.Debug(
                    "GetPossessedBiotasInParallel for {CharacterName} took {ProcessTime:N0} ms",
                    character.Name,
    ...
    )
    {
        ...
        if (playerBiota.WeenieType == WeenieType.Admin)
        {
            player = new Admin(
                playerBiota,
                possessedBiotas.Inventory,
                possessedBiotas.WieldedItems,
                character,
                session
            );
        }
        ...
        if (addSentinelProperties || addAdminProperties) // continue restoring properties to default
        {
            ...
            {
                weenie = Factories.WorldObjectFactory.CreateWorldObject(
                    DatabaseManager.World.GetCachedWeenie("admin"),
                    new ACE.Entity.ObjectGuid(ACE.Entity.ObjectGuid.Invalid.Full)
                );
            }
            else
            {
                weenie = Factories.WorldObjectFactory.CreateWorldObject(
                    DatabaseManager.World.GetCachedWeenie("sentinel"),
                    new ACE.Entity.ObjectGuid(ACE.Entity.ObjectGuid.Invalid.Full)
                );
            }
        ...
        var olthoiPlayerReturnedToLifestone =
        ...
        var popup_welcome = player.IsOlthoiPlayer
            ? PropertyManager.GetString("popup_welcome_olthoi").Item
            : PropertyManager.GetString("popup_welcome").Item;

        if (character.TotalLogins <= 1)
        {
            // Automatically use the welcome letter so it's already open for the new player to read.
            var welcomeLetter = player.Inventory.Values.FirstOrDefault(i => i.WeenieClassId == 1077);
            ...
            else
            {
                session.Network.EnqueueSend(
                    new GameEventPopupString(session, AppendLines(popup_header, popup_motd, popup_welcome))
                );
            }
        }
    ...
    /// </summary>
    public static void ThreadSafeTeleport(
        Player player,
        ...
        while (!pendingWorldStop)
        {
            ...
            */
            ...
            ServerPerformanceMonitor.RestartEvent(
                ServerPerformanceMonitor.MonitorType.NetworkManager_InboundClientMessageQueueRun
            );
            ...
            var sessionCount = NetworkManager.DoSessionWork();
            ServerPerformanceMonitor.RegisterEventEnd(
                ServerPerformanceMonitor.MonitorType.NetworkManager_DoSessionWork
            );
