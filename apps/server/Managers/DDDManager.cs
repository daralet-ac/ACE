...
public static class DDDManager
{
    ...
    public static Dictionary<
        DatDatabaseType,
        ConcurrentDictionary<uint, (int UncompressedFileSize, int CompressedFileSize)>
    > DatFileSizes;
    ...
    public static byte[] TryGetDatFileContentsForTransmission(
        uint datFileId,
        DatDatabaseType datDatabaseType,
        out DatFile datFile,
    ...
    )
    {
        ...
        if (compressDatFile)
        {
            ...
            compressedData = PrependUncompressedFileSize(
                Compress(datDatabase.GetReaderForFile(datFileId).Buffer),
                datFile.FileSize
            );
    ...
    public static uint GetMissingIterations(
    ...
    private static void GetMissingIterations(
        DatDatabaseType datDatabaseType,
        CMostlyConsecutiveIntSet clientDatIterations,
        ref uint totalFileSize,
        Dictionary<DatDatabaseType, Dictionary<uint, List<uint>>> iterations,
        ref uint totalMissingIterations
    )
    {
    ...
    private static List<uint> GetMissingIterationsFromClient(
        DatDatabaseType datDatabaseType,
        CMostlyConsecutiveIntSet clientIterations
    )
    {
        ...
        if (Debug)
        {
            Console.WriteLine(
                debugStr
                    + Environment.NewLine
                    + $"Total Completed Iterations: {iterations} of {totalIterations} ({(double)iterations / totalIterations:P2})"
                    + Environment.NewLine
            );
        }
        ...
        if (Debug)
        {
            ...
            Console.WriteLine(
                debugStr
                    + Environment.NewLine
                    + Environment.NewLine
                    + $"Total Missing Iterations: {allIterations.Count} of {totalIterations} ({(double)allIterations.Count / totalIterations:P2})"
                    + Environment.NewLine
            );
        }
    ...
    public static string Show(this IEnumerable<(uint begin, uint end)> ranges)
    {
        ...
        return string.Join(
            Environment.NewLine,
            ranges.Select(r => r.end - r.begin == 1 ? $"  |    {r.begin}" : $"  |    {r.begin:####} - {r.end - 1:####}")
        );
    }
