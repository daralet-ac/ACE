...
public static class ServerManager
{
    ...
    private static void ShutdownServer()
    {
        ...
        while (shutdownTime != DateTime.MinValue && shutdownTime >= DateTime.UtcNow)
        {
            ...
            if (!ShutdownInitiated)
            {
                ...
                var shutdownText = $"The server shut down has been cancelled @ {DateTime.Now} ({DateTime.UtcNow} UTC)";
        ...
        {
            logUpdateTS = LogStatusUpdate(
                logUpdateTS,
                $"Waiting for {playerCount} player{(playerCount > 1 ? "s" : "")} to log off..."
            );
            Thread.Sleep(10);
            if (playerCount > 0 && DateTime.UtcNow - playerLogoffStart > TimeSpan.FromMinutes(5))
            {
                ...
                {
                    _log.Warning(
                        "5 minute log off failsafe reached and there are {PlayerCount} players still online.",
                        playerCount
                    );
                }
                else
                {
                    _log.Warning(
                        "5 minute log off failsafe reached and there is {PlayerCount} player still online.",
                        playerCount
                    );
                }
                ...
                {
                    _log.Warning(
                        "Player {PlayerName} (0x{PlayerGuid}) appears to be stuck in world and unable to log off normally. Requesting Forced Logoff...",
                        player.Name,
                        player.Guid
                    );
        ...
        while ((sessionCount = NetworkManager.GetAuthenticatedSessionCount()) > 0)
        {
            logUpdateTS = LogStatusUpdate(
                logUpdateTS,
                $"Waiting for {sessionCount} authenticated session{(sessionCount > 1 ? "s" : "")} to disconnect..."
            );
            Thread.Sleep(10);
        }
        ...
        while ((landblockCount = LandblockManager.GetLoadedLandblocks().Count) > 0)
        {
            logUpdateTS = LogStatusUpdate(
                logUpdateTS,
                $"Waiting for {landblockCount} loaded landblock{(landblockCount > 1 ? "s" : "")} to unload..."
            );
            Thread.Sleep(10);
        }
    ...
    public static string ShutdownNoticeText()
    {
        ...
        timeToShutdown +=
            $"{(timeToShutdown.Length > 0 ? ", " : "")}{(sdt.Minutes > 0 ? $"{sdt.Minutes} minute{(sdt.Minutes > 1 ? "s" : "")}" : "")}";
        timeToShutdown +=
            $"{(timeToShutdown.Length > 0 ? " and " : "")}{(sdt.Seconds > 0 ? $"{sdt.Seconds} second{(sdt.Seconds > 1 ? "s" : "")}" : "")}";
