...
public class QuestManager
{
    ...
    private ICollection<CharacterPropertiesQuestRegistry> runtimeQuests { get; set; } =
        new HashSet<CharacterPropertiesQuestRegistry>();
    ...
    public bool HasQuestCompletes(string questName)
    {
        if (Debug)
        {
            Console.WriteLine($"{Name}.QuestManager.HasQuestCompletes({questName})");
        }
    ...
    private CharacterPropertiesQuestRegistry GetOrCreateQuest(string questName, out bool questRegistryWasCreated)
    {
        ...
        // Not a player
        var existing = runtimeQuests.FirstOrDefault(q =>
    ...
    public void Update(string questFormat)
    {
        ...
        else
        {
            if (IsMaxSolves(questName))
            {
                ...
                {
                    Console.WriteLine(
                        $"{Name}.QuestManager.Update({quest}): can not update existing quest. IsMaxSolves({questName}) is true."
                    );
                }
            ...
            if (Debug)
            {
                Console.WriteLine($"{Name}.QuestManager.Update({quest}): updated quest ({quest.NumTimesCompleted})");
            }
    ...
    public void SetQuestCompletions(string questFormat, int questCompletions = 0)
    {
        ...
        if (questRegistryWasCreated)
        {
            ...
            {
                Console.WriteLine(
                    $"{Name}.QuestManager.SetQuestCompletions({questFormat}): initialized quest to {quest.NumTimesCompleted}"
                );
            }
    ...
    public bool CanSolve(string questFormat)
    {
        ...
        if (Debug)
        {
            Console.WriteLine($"{Name}.QuestManager.CanSolve({questName}): {canSolve}");
        }
    ...
    public TimeSpan GetNextSolveTime(string questFormat)
    {
        ...
        {
            nextSolveTime =
                playerQuest.LastTimeCompleted
                + (uint)(quest.MinDelta * PropertyManager.GetDouble("quest_mindelta_rate", 1).Item);
        }
        else
        {
    ...
    public void Decrement(string quest, int amount = 1)
    {
        ...
        if (existing != null)
        {
            ...
            {
                Console.WriteLine(
                    $"{Name}.QuestManager.Decrement({quest}): updated quest ({existing.NumTimesCompleted})"
                );
            }
    ...
    public void Erase(string questFormat)
    {
        ...
        {
            // Not a player
            var quests = runtimeQuests
                .Where(q => q.QuestName.Equals(questName, StringComparison.OrdinalIgnoreCase))
                .ToList();
    ...
    public bool HasQuestSolves(string questFormat, int? _min, int? _max)
    {
        ...
        var min = _min ?? int.MinValue; // use defaults?
        var max = _max ?? int.MaxValue;
    ...
    public void HandlePortalQuestError(string questName)
    {
        ...
        {
            player.Session.Network.EnqueueSend(
                new GameEventWeenieError(player.Session, WeenieError.YouMustCompleteQuestToUsePortal)
            );
        }
        else if (CanSolve(questName))
        {
            var error = new GameEventWeenieError(player.Session, WeenieError.QuestSolvedTooLongAgo);
            var text = new GameMessageSystemChat(
    ...
    public void HandleSolveError(string questName)
    {
        ...
        else
        {
            ...
            var remain = new GameMessageSystemChat(
                $"You may complete this quest again in {remainStr}.",
                ChatMessageType.Broadcast
            );
    ...
    public void HandleKillTask(string killQuestName, WorldObject killedCreature)
    {
        ...
        {
            _log.Error(
                "{PlayerName}.QuestManager.HandleKillTask({KillQuestName}): input object is null!",
                Name,
                killQuestName
            );
            return;
        ...
        {
            _log.Error(
                "{PlayerName}.QuestManager.HandleKillTask({KillQuestName}): couldn't find kill task {QuestName} in database",
                Name,
                killQuestName,
                questName
            );
        ...
        if (playerQuest == null)
        {
            ...
            _log.Error(
                "{PlayerName}.QuestManager.HandleKillTask({KillQuestName}): couldn't find kill task {QuestName} in player quests",
                Name,
                killQuestName,
                questName
            );
    ...
    public void SetQuestBits(string questFormat, int bits, bool on = true)
    {
        ...
        if (quest != null)
        {
            questBits = quest.NumTimesCompleted;
        }
    ...
    public float HandleProsperity()
    {
        ...
        foreach (var quest in quests)
        {
            ...
            {
                var prosperity = questName.Split("/");
        ...
        var prosMod = (totalModifiedDamage / totalDamage) / 2;
    ...
    public double? HandleMagicFind()
    {
        ...
        var updatedQualityMod = (double?)(totalModifiedDamage / totalDamage);
