...
namespace ACE.Server.Managers
{
    ...
    {
        private static readonly ILogger _log = Log.ForContext(typeof(HouseManager));
        ...
        private static void BuildHouseIdToGuid()
        {
            using (var ctx = new WorldDbContext())
            {
                ...
                foreach (var result in results)
                {
                    ...
                    {
                        _log.Error($"[HOUSE] HouseManager.BuildHouseIdToGuid(): couldn't parse {classname}");
                        continue;
                    }
        ...
        private static void AddRentQueue(Biota slumlord)
        {
            ...
            if (owner == null)
            {
                _log.Error($"[HOUSE] HouseManager.AddRentQueue(): couldn't find owner {biotaOwner.Value:X8}");
                return;
            }
            var houseId = slumlord.BiotaPropertiesDID.FirstOrDefault(i => i.Type == (ushort)PropertyDataId.HouseId);
            if (houseId == null)
            {
                _log.Error($"[HOUSE] HouseManager.AddRentQueue(): couldn't find house id for {slumlord.Id:X8}");
                return;
            }
            if (!HouseIdToGuid.TryGetValue(houseId.Value, out var houseGuids))
            {
                _log.Error($"[HOUSE] HouseManager.AddRentQueue(): couldn't find house instance for {slumlord.Id:X8}");
                return;
            }
            var houseInstance = GetHouseGuid(slumlord.Id, houseGuids);
            if (houseInstance == 0)
            {
                _log.Error($"[HOUSE] HouseManager.AddRentQueue(): couldn't find house guid for {slumlord.Id:X8}");
                return;
            }
            if (RentQueueContainsHouse(houseInstance))
            {
                _log.Error($"[HOUSE] HouseManager.AddRentQueue(): rent queue already contains house {houseInstance}");
                return;
            }
        ...
        public static void AddRentQueue(IPlayer player, uint houseGuid)
        {
            ...
            {
                _log.Warning("[HOUSE] HouseManager.AddRentQueue({PlayerName}, {HouseGuid:X8}): player has null HouseRentTimestamp", player.Name, houseGuid);
        ...
        public static string GetCoords(Position position)
        {
            ...
            if (coords == null)
            {
                ...
                if (ApartmentBlocks.TryGetValue(position.Landblock, out var apartmentBlock))
                    ...
                    _log.Error($"[HOUSE] HouseManager.GetCoords({position}) - couldn't find apartment block");
        ...
        private static void ProcessRent(PlayerHouse playerHouse)
        {
            ...
            GetHouse(playerHouse.House.Guid.Full, (house) =>
            {
                ...
                _log.Debug("[HOUSE] {PlayerName}.ProcessRent(): isPaid = {IsPaid} | HasRequirements = {HasRequirements} | MaintenanceFree = {MaintenanceFree}", playerHouse.PlayerName, isPaid, hasRequirements, house.HouseStatus == HouseStatus.InActive);
        ...
        private static void HandleRentPaid(PlayerHouse playerHouse)
        {
            ...
            if (player == null)
            {
                _log.Warning("[HOUSE] HouseManager.HandleRentPaid({PlayerName}): couldn't find player", playerHouse.PlayerName);
                return;
            }
            ...
            {
                _log.Warning("[HOUSE] HouseManager.HandleRentPaid({PlayerName}): nextRentTime {NextRentTime} <= rentTime {RentTime}", playerHouse.PlayerName, nextRentTime, rentTime);
                return;
            }
            ...
            _log.Debug("[HOUSE] HouseManager.HandleRentPaid({PlayerName}): rent payment successful!", playerHouse.PlayerName);
        ...
        public static void HandleEviction(House house, uint playerGuid, bool multihouse = false, bool force = false)
        {
            ...
            if (!PropertyManager.GetBool("house_rent_enabled", true).Item && !multihouse && !force)
            {
                ...
                _log.Debug("[HOUSE] HouseManager.HandleRentPaid({PlayerName}): house rent disabled via config", player.Name);
            ...
            if (player.HouseInstance == house.Guid.Full)
            {
                ...
                _log.Warning("[HOUSE] HouseManager.HandleRentEviction({HouseGuid}, {PlayerName}, {Multihouse}): house guids don't match {PlayerHouseInstance}", house.Guid, player.Name, multihouse, player.HouseInstance);

            house.ClearRestrictions();

            _log.Debug("[HOUSE] HouseManager.HandleRentEviction({PlayerName})", player.Name);
            ...
            if (!isOnline)
            {
                ...
                {
                    _log.Warning("[HOUSE] {PlayerName}.HandleEviction(): couldn't find offline player", player.Name);
                    return;
                }
        ...
        private static bool IsRentPaid(PlayerHouse playerHouse)
        {
            ...
            foreach (var rentItem in houseData.Rent)
            {
                ...
                {
                    _log.Debug("[HOUSE] {PlayerName}.IsRentPaid() - required {NumRentItems:N0}x {RentItemName} ({RentItemWeenieID}), found {RentItemPaid:N0}", playerHouse.PlayerName, rentItem.Num, rentItem.Num > 1 ? $"{rentItem.PluralName}" : $"{rentItem.Name}", rentItem.WeenieID, rentItem.Paid);
                    return false;
                }
            }
            return true;
        ...
        private static bool HasRequirements(PlayerHouse playerHouse)
        {
            ...
            if (player == null)
            {
                _log.Warning("[HOUSE] {PlayerName}.HasRequirements() - couldn't find player", playerHouse.PlayerName);
                return false;
            }
            ...
            {
                _log.Debug("[HOUSE] {PlayerName}.HasRequirements() - allegiance rank {Rank} < {AllegianceMinLevel}", playerHouse.PlayerName, rank, allegianceMinLevel);
                return false;
            }
            return true;
        }
        ...
        public static void GetHouse(uint houseGuid, Action<House> callback)
        {
            ...
            else if (!loaded.CreateWorldObjectsCompleted)
            {
                ...
                _log.Error("[HOUSE] HouseManager.GetHouse({HouseGuid:X8}): couldn't find house on loaded landblock", houseGuid);
        }
        ...
        private static void PayRent(PlayerHouse playerHouse)
        {
            ...
            GetHouse(playerHouse.House.Guid.Full, (house) =>
            {
                ...
                if (!isPaid)
                {
                    ...
                    foreach (var rentItem in houseData.Rent)
                    {
                        if (rentItem.Paid < rentItem.Num)
                        {
                            ...
                            while (amountLeftToPay > 0)
                            {
                                ...
                                if (payment == null)
                                {
                                    _log.Error($"[HOUSE] HouseManager.PayRent({house.Guid}): couldn't create payment for WCID {rentItem.WeenieID}");
                                    return;
                                }
                                ...
                                {
                                    _log.Error($"[HOUSE] HouseManager.PayRent({house.Guid}): couldn't place {payment.Name} (0x{payment.Guid}) in SlumLord's Inventory");
                                    return;
                                }
                    ...
                    _log.Debug("[HOUSE] HouseManager.PayRent({HouseGuid}): fully paid rent into SlumLord.", house.Guid);
                }
            });
        }
