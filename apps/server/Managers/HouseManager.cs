...
public static class HouseManager
{
    ...
    /// </summary>
    private static Dictionary<uint, List<HouseCallback>> SlumlordCallbacks =
        new Dictionary<uint, List<HouseCallback>>();
    ...
    private static void BuildHouseIdToGuid()
    {
        using (var ctx = new WorldDbContext())
        {
            var query =
                ...
                select new { Weenie = weenie, Instance = inst };
    ...
    {
        var biotaOwner = slumlord.BiotaPropertiesIID.FirstOrDefault(i =>
            i.Type == (ushort)PropertyInstanceId.HouseOwner
        );
        if (biotaOwner == null)
        {
    ...
    public static void AddRentQueue(IPlayer player, uint houseGuid)
    {
        ...
        {
            _log.Warning(
                "[HOUSE] HouseManager.AddRentQueue({PlayerName}, {HouseGuid:X8}): player has null HouseRentTimestamp",
                player.Name,
                houseGuid
            );
    ...
    private static string GetCoords(Biota biota)
    {
        ...
        return GetCoords(
            new Position(p.ObjCellId, p.OriginX, p.OriginY, p.OriginZ, p.AnglesX, p.AnglesY, p.AnglesZ, p.AnglesW)
        );
    }
    ...
    private static void ProcessRent(PlayerHouse playerHouse)
    {
        ...
        GetHouse(
            ...
            (house) =>
            {
                ...
                _log.Debug(
                    "[HOUSE] {PlayerName}.ProcessRent(): isPaid = {IsPaid} | HasRequirements = {HasRequirements} | MaintenanceFree = {MaintenanceFree}",
                    playerHouse.PlayerName,
                    isPaid,
                    hasRequirements,
    ...
    private static void HandleRentPaid(PlayerHouse playerHouse)
    {
        ...
        {
            _log.Warning(
                "[HOUSE] HouseManager.HandleRentPaid({PlayerName}): couldn't find player",
                playerHouse.PlayerName
            );
            return;
        }
        ...
        {
            _log.Warning(
                "[HOUSE] HouseManager.HandleRentPaid({PlayerName}): nextRentTime {NextRentTime} <= rentTime {RentTime}",
                playerHouse.PlayerName,
                nextRentTime,
                rentTime
            );
        ...
        _log.Debug(
            "[HOUSE] HouseManager.HandleRentPaid({PlayerName}): rent payment successful!",
            playerHouse.PlayerName
        );
    ...
    public static void HandleEviction(House house, uint playerGuid, bool multihouse = false, bool force = false)
    {
        ...
        var player = PlayerManager.FindByGuid(playerGuid, out var isOnline);
        ...
        if (!PropertyManager.GetBool("house_rent_enabled", true).Item && !multihouse && !force)
        {
            ...
            _log.Debug(
                "[HOUSE] HouseManager.HandleRentPaid({PlayerName}): house rent disabled via config",
                player.Name
            );
        ...
        {
            _log.Warning(
                "[HOUSE] HouseManager.HandleRentEviction({HouseGuid}, {PlayerName}, {Multihouse}): house guids don't match {PlayerHouseInstance}",
                house.Guid,
                player.Name,
                multihouse,
                player.HouseInstance
            );
        }
    ...
    private static bool IsRentPaid(PlayerHouse playerHouse)
    {
        ...
        foreach (var rentItem in houseData.Rent)
        {
            ...
            {
                _log.Debug(
                    "[HOUSE] {PlayerName}.IsRentPaid() - required {NumRentItems:N0}x {RentItemName} ({RentItemWeenieID}), found {RentItemPaid:N0}",
                    playerHouse.PlayerName,
                    rentItem.Num,
                    rentItem.Num > 1 ? $"{rentItem.PluralName}" : $"{rentItem.Name}",
                    rentItem.WeenieID,
                    rentItem.Paid
                );
                return false;
    ...
    private static bool HasRequirements(PlayerHouse playerHouse)
    {
        ...
        {
            _log.Debug(
                "[HOUSE] {PlayerName}.HasRequirements() - allegiance rank {Rank} < {AllegianceMinLevel}",
                playerHouse.PlayerName,
                rank,
                allegianceMinLevel
            );
    ...
    public static void GetHouse(uint houseGuid, Action<House> callback)
    {
        ...
        {
            _log.Error(
                "[HOUSE] HouseManager.GetHouse({HouseGuid:X8}): couldn't find house on loaded landblock",
                houseGuid
            );
        }
    }
                    ...
                    foreach (var rentItem in houseData.Rent)
                    {
                        if (rentItem.Paid < rentItem.Num)
                        {
                            ...
                            while (amountLeftToPay > 0)
                            {
                                ...
                                {
                                    _log.Error(
                                        $"[HOUSE] HouseManager.PayRent({house.Guid}): couldn't create payment for WCID {rentItem.WeenieID}"
                                    );
                                    return;
                                }
                                ...
                                {
                                    _log.Error(
                                        $"[HOUSE] HouseManager.PayRent({house.Guid}): couldn't place {payment.Name} (0x{payment.Guid}) in SlumLord's Inventory"
                                    );
                                    return;
                                }
