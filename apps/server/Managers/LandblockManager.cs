...
public static class LandblockManager
{
    ...
    /// </summary>
    private static readonly ReaderWriterLockSlim landblockLock = new ReaderWriterLockSlim(
    ...
    public static void PreloadConfigLandblocks()
    {
        ...
        _log.Information(
            "Found {0} landblock entries in PreloadedLandblocks configuration, {1} are set to preload.",
            ConfigManager.Config.Server.PreloadedLandblocks.Count,
            ConfigManager.Config.Server.PreloadedLandblocks.Count(x => x.Enabled == true)
        );
                ...
                if (landblock == 0)
                {
                    switch (preloadLandblock.Description)
                    {
                        case "Apartment Landblocks":
                            _log.Information(
                                "Preloading landblock group: {0}, IncludeAdjacents = {1}, Permaload = {2}",
                                preloadLandblock.Description,
                                preloadLandblock.IncludeAdjacents,
                                preloadLandblock.Permaload
                            );
    ...
    private static void PreloadLandblock(uint landblock, PreloadedLandblocks preloadLandblock)
    {
        ...
        _log.Debug(
            "Landblock {0:X4}, ({1}) preloaded. IncludeAdjacents = {2}, Permaload = {3}",
    ...
    private static void ProcessPendingLandblockGroupAdditions()
    {
        ...
        try
        {
            for (var i = landblockGroupPendingAdditions.Count - 1; i >= 0; i--)
            {
                ...
                else
                {
                    ...
                    for (var j = 0; j < landblockGroups.Count; j++)
                    {
                    ...
                    if (landblockGroupsIndexMatchesByDistance.Count > 0)
                    {
                        ...
                        landblockGroups[landblockGroupsIndexMatchesByDistance[0]]
                            .Add(landblockGroupPendingAdditions[i]);

                        if (landblockGroupsIndexMatchesByDistance.Count > 1)
                        {
                            // Merge the additional eligible groups into the first one
                            for (var j = landblockGroupsIndexMatchesByDistance.Count - 1; j > 0; j--)
                            {
            ...
            {
                _log.Error(
                    $"[LANDBLOCK GROUP] ProcessPendingAdditions count ({count}) != loadedLandblocks.Count ({loadedLandblocks.Count})"
                );
            }
        }
        finally
    ...
    public static void Tick(double portalYearTicks)
    {
        ...
        ServerPerformanceMonitor.RestartEvent(
            ServerPerformanceMonitor.MonitorType.LandblockManager_TickMultiThreadedWork
        );
        ...
        ServerPerformanceMonitor.RestartEvent(
            ServerPerformanceMonitor.MonitorType.LandblockManager_TickSingleThreadedWork
        );
    ...
    /// Used to debug cross-landblock group (and potentially cross-thread) operations
    /// </summary>
    public static readonly ThreadLocal<LandblockGroup> CurrentMultiThreadedTickingLandblockGroup =
        new ThreadLocal<LandblockGroup>();

    public static readonly RollingAmountOverTimeTracker TickPhysicsEfficiencyTracker = new RollingAmountOverTimeTracker(
        TimeSpan.FromMinutes(1)
    );
    public static readonly RollingAmountOverTimeTracker TickMultiThreadedWorkEfficiencyTracker =
        new RollingAmountOverTimeTracker(TimeSpan.FromMinutes(1));
        ...
        if (ConfigManager.Config.Server.Threading.MultiThreadedLandblockGroupPhysicsTicking)
        {
            ...
            var partitioner = Partitioner.Create(
                landblockGroups
                    ...
                    .ThenByDescending(r => r.TickPhysicsTracker.AverageAmount)
            );
            ...
            if (landblockGroups.Count > 0)
            {
                ...
                var totalThreadsUsed = Math.Min(
                    landblockGroups.Count,
                    ConfigManager.Config.Server.Threading.LandblockManagerParallelOptions.MaxDegreeOfParallelism
                );
    ...
    private static void TickMultiThreadedWork()
    {
        ...
        if (ConfigManager.Config.Server.Threading.MultiThreadedLandblockGroupTicking)
        {
            ...
            var partitioner = Partitioner.Create(
                landblockGroups
                    .OrderByDescending(r => r.Count)
                    .ThenByDescending(r => r.TickMultiThreadedWorkTracker.AverageAmount)
            );
        ...
        try
        {
            ...
            if (landblock == null)
            {
                ...
                try
                {
                    ...
                    {
                        _log.Error(
                            "LandblockManager: failed to add {LandblockId:X8} to active landblocks!",
                            landblock.Id.Raw
                        );
                        return landblock;
                    }
    ...
    private static void UnloadLandblocks()
    {
        ...
        {
            if (destructionQueue.TryTake(out var landblock))
            {
                landblock.Unload();
                        ...
                        for (var i = landblockGroups.Count - 1; i >= 0; i--)
                        {
                            if (landblockGroups[i].Remove(landblock))
                            {
                                if (landblockGroups[i].Count == 0)
                                {
                                    ...
                                    ConfigManager.Config.Server.Threading.MultiThreadedLandblockGroupPhysicsTicking
                                    || ConfigManager.Config.Server.Threading.MultiThreadedLandblockGroupTicking
                                ) // Only try to split if multi-threading is enabled
                                {
                                    ...
                                    {
                                        _log.Warning(
                                            "[LANDBLOCK GROUP] TrySplit for {0} took: {1:N2} ms",
                                            landblockGroups[i],
                                            swTrySplitEach.Elapsed.TotalMilliseconds
                                        );
                                    }
                                    ...
                                    if (splits != null)
                                    {
                                        ...
                                        {
                                            _log.Debug(
                                                "[LANDBLOCK GROUP] TrySplit resulted in {0} split(s) and took: {1:N2} ms",
                                                splits.Count,
                                                swTrySplitEach.Elapsed.TotalMilliseconds
                                            );
                                            _log.Debug("[LANDBLOCK GROUP] split for old: {0}", landblockGroups[i]);
                                        }

                                        foreach (var split in splits)
                                        {
                                            landblockGroups.Add(split);
                                            _log.Debug("[LANDBLOCK GROUP] split and new: {0}", split);
                                        }
                                    }
                                }
                ...
                if (unloadFailed)
                {
                    _log.Error("LandblockManager: failed to unload {LandblockId:X8}", landblock.Id.Raw);
                }
            }
        }
    }
