...
public static class ServerPerformanceMonitor
{
    ...
    private static readonly TimedEventHistory[] cumulative5m = new TimedEventHistory[
        (int)CumulativeEventHistoryType.MaxItems
    ];
    private static readonly TimedEventHistory[] cumulative1h = new TimedEventHistory[
        (int)CumulativeEventHistoryType.MaxItems
    ];
    private static readonly TimedEventHistory[] cumulative24h = new TimedEventHistory[
        (int)CumulativeEventHistoryType.MaxItems
    ];
    ...
    {
        for (var i = 0; i < monitors5m.Length; i++)
        {
        ...
        for (var i = 0; i < cumulative5m.Length; i++)
        {
    ...
    public static void RestartCumulativeEvents()
    {
        ...
        for (var i = 0; i < cumulativeSeconds.Length; i++)
        {
    ...
    public static new string ToString()
    {
        ...
        sb.Append(
            $"Monitoring Durations: ~5m {Monitors5mRunTime.TotalMinutes:N2} min, ~1h {Monitors1hRunTime.TotalMinutes:N2} min, ~24h {Monitors24hRunTime.TotalMinutes:N2} min{'\n'}"
        );
        sb.Append(
            $"~5m Hits   Avg  Long  Last Tot - ~1h Hits   Avg  Long  Last  Tot - ~24h Hits  Avg  Long  Last   Tot (s) - Name{'\n'}"
        );

        sb.Append($"Calls from WorldManager.UpdateWorld(){'\n'}");
        for (var i = (int)MonitorType.PlayerManager_Tick; i <= (int)MonitorType.NetworkManager_DoSessionWork; i++)
        {
            AddMonitorOutputToStringBuilder(
                monitors5m[i].EventHistory,
                monitors1h[i].EventHistory,
                monitors24h[i].EventHistory,
                ((MonitorType)i).ToString(),
                sb
            );
        }
        ...
        AddMonitorOutputToStringBuilder(
            monitors5m[(int)MonitorType.UpdateGameWorld_Entire].EventHistory,
            monitors1h[(int)MonitorType.UpdateGameWorld_Entire].EventHistory,
            monitors24h[(int)MonitorType.UpdateGameWorld_Entire].EventHistory,
            MonitorType.UpdateGameWorld_Entire.ToString(),
            sb
        );

        sb.Append($"Calls from WorldManager.UpdateGameWorld(){'\n'}");
        for (
            var i = (int)MonitorType.LandblockManager_TickPhysics;
            i <= (int)MonitorType.LandblockManager_TickSingleThreadedWork;
            i++
        )
        {
            AddMonitorOutputToStringBuilder(
        ...
        if (IsRunningCumulative)
        {
            ...
            for (
                var i = (int)CumulativeEventHistoryType.Player_Tick_UpdateObjectPhysics;
                i <= (int)CumulativeEventHistoryType.WorldObject_Tick_UpdateObjectPhysics;
                i++
            )
            {
                AddMonitorOutputToStringBuilder(
                    ...
                    cumulative1h[i],
                    cumulative24h[i],
                    ((CumulativeEventHistoryType)i).ToString(),
                    sb
                );
            }
            ...
            for (
                var i = (int)CumulativeEventHistoryType.Landblock_Tick_RunActions;
                i <= (int)CumulativeEventHistoryType.Landblock_Tick_Database_Save;
                i++
            )
            {
                AddMonitorOutputToStringBuilder(
            ...
            for (
                var i = (int)CumulativeEventHistoryType.Monster_Awareness_FindNextTarget;
                i <= (int)CumulativeEventHistoryType.LootGenerationFactory_CreateRandomLootObjects;
                i++
            )
            {
                AddMonitorOutputToStringBuilder(
            ...
            for (
                var i = (int)CumulativeEventHistoryType.Landblock_Tick_Player_Tick;
                i <= (int)CumulativeEventHistoryType.Landblock_Tick_WorldObject_Heartbeat;
                i++
            )
            {
                AddMonitorOutputToStringBuilder(
        ...
        for (
            var i = (int)MonitorType.DoSessionWork_TickOutbound;
            i <= (int)MonitorType.DoSessionWork_RemoveSessions;
            i++
        )
        {
            AddMonitorOutputToStringBuilder(
    ...
    private static void AddMonitorOutputToStringBuilder(
        TimedEventHistory eventHistory5m,
        TimedEventHistory eventHistory1h,
        TimedEventHistory eventHistory24h,
        string name,
        StringBuilder sb
    )
    {
        sb.Append(
            $"{eventHistory5m.TotalEvents.ToString().PadLeft(7)} {eventHistory5m.AverageEventDuration:N4} {eventHistory5m.LongestEvent:N3} {eventHistory5m.LastEvent:N3} {((int)eventHistory5m.TotalSeconds).ToString().PadLeft(3)} - "
