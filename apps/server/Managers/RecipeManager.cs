...
public partial class RecipeManager
{
    ...
    public static void UseObjectOnTarget(Player player, WorldObject source, WorldObject target, bool confirmed = false)
    {
        ...
        if (source == target)
        {
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"The {source.NameWithMaterial} cannot be combined with itself.",
            ...
            player.Session.Network.EnqueueSend(
                new GameEventCommunicationTransientString(
                    player.Session,
                    $"You can't use the {source.NameWithMaterial} on itself."
                )
            );
        ...
        if (target.Wielder != null)
        {
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"You cannot use the {source.NameWithMaterial} on a wielded item.",
                    ChatMessageType.Craft
                )
            );
            player.SendUseDoneEvent();
        ...
        if (recipe == null)
        {
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"The {source.NameWithMaterial} cannot be used on the {target.NameWithMaterial}.",
                    ChatMessageType.Craft
                )
            );
            player.SendUseDoneEvent();
        ...
        {
            _log.Debug(
                "[TINKERING] {PlayerName}.UseObjectOnTarget({SourceNameWithMaterial}, {TargetNameWithMaterial}) | Status: {Confirmed}",
                player.Name,
        ...
        {
            player.SendMessage(
                $"Your Aura of the Craftman augmentation increased your skill by {player.LumAugSkilledCraft}!"
            );
        }
    ...
    public static double? GetRecipeChance(Player player, WorldObject source, WorldObject target, Recipe recipe)
    {
        ...
        if (playerSkill == null)
        {
            ...
            _log.Warning(
                $"RecipeManager.GetRecipeChance({player.Name}, {source.Name}, {target.Name}): recipe {recipe.Id} missing skill"
            );
            return null;
        }
    ...
    public static double? GetTinkerChance(Player player, WorldObject tool, WorldObject target, Recipe recipe)
    {
        ...
        var difficulty = (int)
            Math.Floor(
                (
                    (salvageMod * 5.0f)
                    + (itemWorkmanship * salvageMod * 2.0f)
                    - (toolWorkmanship * workmanshipMod * salvageMod / 5.0f)
                ) * attemptMod
            );
    ...
    )
    {
        ...
        {
            var exactMsg =
                $"You have a {(float)percent} percent chance of using {source.NameWithMaterial} on {target.NameWithMaterial}.";
    ...
    public static bool TryMutateNative(
        Player player,
        WorldObject source,
        WorldObject target,
        Recipe recipe,
    ...
    public static void AddSpell(Player player, WorldObject target, SpellId spell, int difficulty = 25)
    {
        ...
        if (target.UiEffects == null)
        {
            ...
            player.Session.Network.EnqueueSend(
                new GameMessagePublicUpdatePropertyInt(target, PropertyInt.UiEffects, (int)target.UiEffects)
            );
        }
    }
    ...
    public static bool VerifyRequirements(Recipe recipe, Player player, WorldObject source, WorldObject target)
    {
        ...
        if (!VerifyRequirements(recipe, player, source, RequirementType.Source))
        {
            return false;
        }

        if (!VerifyRequirements(recipe, player, player, RequirementType.Player))
        {
            return false;
        }
    ...
    public static bool VerifyUse(Player player, WorldObject source, WorldObject target, bool blockWielded = false)
    {
        ...
        {
            _log.Warning(
                $"{player.Name}.RecipeManager.VerifyUse({source.Name} ({source.Guid}), {target.Name} ({target.Guid})) - source not usable, falling back on defaults"
            );
    ...
    public static bool VerifyRequirements(Recipe recipe, Player player, WorldObject obj, RequirementType reqType)
    {
        ...
        var totalReqs =
            boolReqs.Count + intReqs.Count + floatReqs.Count + strReqs.Count + iidReqs.Count + didReqs.Count;
        ...
        {
            var value = obj.GetProperty((PropertyBool)requirement.Stat);
            var normalized = value != null ? (double?)Convert.ToDouble(value.Value) : null;

            if (Debug)
            {
                Console.WriteLine(
                    $"PropertyBool.{(PropertyBool)requirement.Stat} {(CompareType)requirement.Enum} {requirement.Value}, current: {value}"
                );
            }
            ...
            if (
                !VerifyRequirement(
                    player,
                    (CompareType)requirement.Enum,
                    normalized,
                    Convert.ToDouble(requirement.Value),
                    requirement.Message
                )
            )
            {
        ...
        {
            var value = obj.GetProperty((PropertyInt)requirement.Stat);
            var normalized = value != null ? (double?)Convert.ToDouble(value.Value) : null;

            if (Debug)
            {
                Console.WriteLine(
                    $"PropertyInt.{(PropertyInt)requirement.Stat} {(CompareType)requirement.Enum} {requirement.Value}, current: {value}"
                );
            }
        ...
        {
            var value = obj.GetProperty((PropertyFloat)requirement.Stat);

            if (Debug)
            {
                Console.WriteLine(
                    $"PropertyFloat.{(PropertyFloat)requirement.Stat} {(CompareType)requirement.Enum} {requirement.Value}, current: {value}"
                );
            }

            if (
                !VerifyRequirement(player, (CompareType)requirement.Enum, value, requirement.Value, requirement.Message)
            )
            {
                return false;
            }
        ...
        {
            var value = obj.GetProperty((PropertyString)requirement.Stat);

            if (Debug)
            {
                Console.WriteLine(
                    $"PropertyString.{(PropertyString)requirement.Stat} {(CompareType)requirement.Enum} {requirement.Value}, current: {value}"
                );
            }
        ...
        foreach (var requirement in iidReqs)
        {
            ...
            {
                Console.WriteLine(
                    $"PropertyInstanceId.{(PropertyInstanceId)requirement.Stat} {(CompareType)requirement.Enum} {requirement.Value}, current: {value}"
                );
            }
        ...
        {
            var value = obj.GetProperty((PropertyDataId)requirement.Stat);
            var normalized = value != null ? (double?)Convert.ToDouble(value.Value) : null;

            if (Debug)
            {
                Console.WriteLine(
                    $"PropertyDataId.{(PropertyDataId)requirement.Stat} {(CompareType)requirement.Enum} {requirement.Value}, current: {value}"
                );
            }
    ...
    public static bool VerifyRequirement(
        Player player,
    ...
    /// </summary>
    public static HashSet<uint> CreateDestroyItems(
        Player player,
        Recipe recipe,
        WorldObject source,
        WorldObject target,
    ...
    )
    {
        ...
        {
            _log.Error(
                $"RecipeManager.CreateDestroyItems: Recipe.Id({recipe.Id}) couldn't find {(success ? "Success" : "Fail")}WCID {createItem} in database."
            );
            player.Session.Network.EnqueueSend(
                new GameEventWeenieError(player.Session, WeenieError.CraftGeneralErrorUiMsg)
            );
            return null;
        }
    ...
    )
    {
        ...
        var msg =
            $"{player.Name} {(success ? "successfully applies" : "fails to apply")} the {sourceName} (workmanship {(tool.Workmanship ?? 0):#.00}) to the {target.NameWithMaterial}{((target.Inscription != null && target.ScribeName != null) ? $" inscribed by {target.ScribeName}" : "")}.{(!success ? " The target is destroyed." : "")}";

        player.EnqueueBroadcast(
            new GameMessageSystemChat(msg, ChatMessageType.Craft),
    ...
    public static void DestroyItem(Player player, Recipe recipe, WorldObject item, uint amount, string msg)
    {
        if (item.OwnerId == player.Guid.Full || player.GetInventoryItem(item.Guid) != null)
        {
            ...
            {
                _log.Warning(
                    $"RecipeManager.DestroyItem({player.Name}, {item.Name}, {amount}, {msg}): failed to remove {item.Name}"
                );
            }
        }
    ...
    public static WorldObject GetTargetMod(
    ...
    /// Returns a list of object guids that have been modified
    /// </summary>
    public static HashSet<uint> ModifyItem(
        Player player,
        Recipe recipe,
        WorldObject source,
        WorldObject target,
    ...
    public static void ModifyBool(
        Player player,
        RecipeModsBool boolMod,
        WorldObject source,
        WorldObject target,
        WorldObject result,
    ...
    )
    {
        ...
        switch (op)
        {
            ...
            case ModificationOperation.Add:
                ...
                if (Debug)
                {
                    Console.WriteLine($"{targetMod.Name}.IncProperty({prop}, {value}) - {op}");
                }
            ...
            case ModificationOperation.CopyFromSourceToTarget:
                ...
                if (Debug)
                {
                    Console.WriteLine($"{target.Name}.SetProperty({prop}, {sourceMod.GetProperty(prop) ?? 0}) - {op}");
                }
            ...
            case ModificationOperation.CopyFromSourceToResult:
                ...
                if (Debug)
                {
                    Console.WriteLine($"{result.Name}.SetProperty({prop}, {player.GetProperty(prop) ?? 0}) - {op}");
                }
            ...
            case ModificationOperation.AddSpell:
                ...
                if (Debug)
                {
                    Console.WriteLine($"{targetMod.Name}.AddSpell({intMod.Stat}) - {op}");
                }
            ...
            case ModificationOperation.SetBitsOn:
                ...
                if (Debug)
                {
                    Console.WriteLine($"{targetMod.Name}.SetProperty({prop}, 0x{bits:X}) - {op}");
                }
    ...
    public static void ModifyFloat(
        Player player,
        RecipeModsFloat floatMod,
        WorldObject source,
        WorldObject target,
        WorldObject result,
        HashSet<uint> modified
    ...
    )
    {
        ...
        switch (op)
        {
            ...
            case ModificationOperation.CopyFromSourceToTarget:
                ...
                {
                    Console.WriteLine(
                        $"{target.Name}.SetProperty({prop}, {sourceMod.GetProperty(prop) ?? sourceMod.Name}) - {op}"
                    );
                }
            ...
            case ModificationOperation.CopyFromSourceToResult:
                ...
                {
                    Console.WriteLine(
                        $"{result.Name}.SetProperty({prop}, {player.GetProperty(prop) ?? player.Name}) - {op}"
                    );
                }
    ...
    public static void ModifyInstanceID(
        Player player,
        RecipeModsIID iidMod,
        WorldObject source,
        WorldObject target,
        WorldObject result,
        HashSet<uint> modified
    )
    {
        ...
        switch (op)
        {
            ...
            case ModificationOperation.CopyFromSourceToTarget:
                ...
                {
                    Console.WriteLine(
                        $"{target.Name}.SetProperty({prop}, {ModifyInstanceIDRuleSet(prop, sourceMod, targetMod)}) - {op}"
                    );
                }
            ...
            case ModificationOperation.CopyFromSourceToResult:
                ...
                {
                    Console.WriteLine(
                        $"{result.Name}.SetProperty({prop}, {ModifyInstanceIDRuleSet(prop, player, targetMod)}) - {op}"
                    );
                }
    ...
    private static uint ModifyInstanceIDRuleSet(
        PropertyInstanceId property,
        WorldObject sourceMod,
        WorldObject targetMod
    )
    {
        switch (property)
        {
    ...
    public static void ModifyDataID(
        Player player,
        RecipeModsDID didMod,
        WorldObject source,
        WorldObject target,
        WorldObject result,
        HashSet<uint> modified
    ...
    private static void ModifyWeenieClassId(WorldObject target, uint weenieClassId, HashSet<uint> modified)
    {
        ...
        switch (target.ItemType)
        {
            ...
            default:
                _log.Error(
                    $"RecipeManager.ModifyWeenieClassId({target.Guid}, {weenieClassId}) Unsupported ItemType: {target.ItemType}"
                );
                return;
        }
    ...
    private static bool RequiresEqualOrGreaterWork(Player player, WorldObject source, WorldObject target)
    {
        ...
        if (qualifyingSalvageTypes.Contains((uint)source.MaterialType))
        {
            ...
            player.Session.Network.EnqueueSend(
                new GameMessageSystemChat(
                    $"The {source.Name} (workmanship: {source.Workmanship}) does not have enough quality to apply it to the {target.Name} (workmanship: {target.Workmanship}).",
                    ChatMessageType.Craft
                )
            );
        }
