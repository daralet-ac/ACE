        ...
        public static void CorpseList(Session session, params string[] parameters)
        {
            ...
            if (corpses != null)
            {
                    ...
                    if (parameters.Length == 1)
                    {
                        if (int.TryParse(parameters[0], out var result))
                        {
                            ...
                            if (corpseLogs.Length >= result)
                            {
                                ...
                                if (split.Length == 6)
                                {
                                    session.Network.EnqueueSend(new GameMessageSystemChat($"Corpse {result}: {split[5]}", ChatMessageType.Broadcast));
                                    return;
                                }
                            }
                            else
                            {
                                session.Network.EnqueueSend(new GameMessageSystemChat($"Invalid corpse number.", ChatMessageType.Broadcast));
                                return;
                            }
                        }
                        else
                        {
                            session.Network.EnqueueSend(new GameMessageSystemChat("That is not a valid command.", ChatMessageType.Broadcast));
                            return;
                        }
                    ...
                    if (parameters.Length == 2)
                    {
                        if (parameters[0].ToLower() == "remove")
                        {
                            if (int.TryParse(parameters[1], out var result))
                            {
                                ...
                                if (corpseLogs.Length >= result)
                                {
                                    string newCorpseLog = string.Join(";", corpseLogs.Where(p => p != corpseLogs[result - 1]));
                                    session.Player.CorpseLog = newCorpseLog;
                                    session.Network.EnqueueSend(new GameMessageSystemChat($"Corpse {result} has been removed, however the corpse still exists and may be retrieved.", ChatMessageType.Broadcast));
                                    return;
                                }
                                else
                                {
                ...
                else if (parameters.Length == 0)
                {
                    ...
                    session.Network.EnqueueSend(new GameMessageSystemChat($"---------------------------Current Corpses---------------------------", ChatMessageType.Broadcast));

                    var revisedCorpses = session.Player.CorpseLog.Split(";");

                    foreach (var corpse in revisedCorpses)
                    {
                        if (corpse.Length == 0) continue;

                        var split = corpse.Split("|");

                        if (split == null || split.Length < 5) continue;

                        // Corpse GUID, Killer Name, DateTime of Death, Location, DateTime Decay, Items
                        session.Network.EnqueueSend(new GameMessageSystemChat($"{count}:  Killed By - {split[1]}  |  Time of Death - {split[2]}  |  Location - {split[3]}  |   Est. Decay - {split[4]}", ChatMessageType.Broadcast));
