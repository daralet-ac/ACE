...
public class DeveloperFixCommands
{
    ...
    public static void HandleVerifyAttributes(Session session, params string[] parameters)
    {
        ...
        foreach (var player in players)
        {
            ...
            foreach (
                var attr in new Dictionary<PropertyAttribute, PropertiesAttribute>(player.Biota.PropertiesAttribute)
            )
            {
    ...
    [CommandHandler(
        ...
        CommandHandlerFlag.ConsoleInvoke,
        0,
        "Fixes biota emotes with incorrect default delays"
    )]
    public static void HandleFixBiotaEmoteDelay(Session session, params string[] parameters)
    {
        ...
        CommandHandlerHelper.WriteOutputInfo(
            session,
            $"Found {weenieEmoteCache.Count:N0} weenie templates w/ emote actions with delay 0"
        );
        ...
        using (var ctx = new ShardDbContext())
        {
            ...
            CommandHandlerHelper.WriteOutputInfo(
                session,
                $"Found {biotas.Count} biotas matching {distinct.Count()} distinct wcids"
            );
            ...
            foreach (var biota in biotas)
            {
                ...
                var query =
                    from emote in ctx.BiotaPropertiesEmote
                    ...
                    select new { Emote = emote, Action = action };
                ...
                if (updated)
                {
                    CommandHandlerHelper.WriteOutputInfo(
                        session,
                        $"Fixed shard object {biota.Id:X8} of type {biota.WeenieClassId} - {(WeenieClassName)biota.WeenieClassId}"
                    );
                    totalUpdated++;
                }
            }
    ...
    private static Dictionary<uint, Dictionary<int, HashSet<uint>>> BuildWeenieEmoteCache()
    {
        ...
        {
            var query =
                from emote in ctx.WeeniePropertiesEmote
    ...
    private static int CalculateEmoteHash(
        uint category,
        float probability,
        uint? wcid,
        uint? style,
    ...
    private static Dictionary<uint, uint> PreToPostMeleeRareConversions =
        new()
        {
