...
public class BoundingBox
{
    ...
    public Matrix4x4 GetTransform(ModelMesh model)
    {
        ...
        var rotate = Matrix4x4.CreateFromQuaternion(
            new Quaternion(
                model.Frame.Orientation.X,
                model.Frame.Orientation.Y,
                model.Frame.Orientation.Z,
                model.Frame.Orientation.W
            )
        );
        var cellTranslate = Matrix4x4.CreateTranslation(
            new Vector3(model.Cell.X * LandblockMesh.CellSize, model.Cell.Y * LandblockMesh.CellSize, 0)
        );
        var cellTranslateInner = Matrix4x4.CreateTranslation(
