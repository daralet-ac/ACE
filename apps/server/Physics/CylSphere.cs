...
public class CylSphere : IEquatable<CylSphere>
{
    ...
    public TransitionState IntersectsSphere(Transition transition)
    {
        ...
        else
        {
            ...
            if (
                CollidesWithSphere(globSphere, disp, radsum)
    ...
    )
    {
        ...
        return checkPos.SlideSphere(
            transition,
            ref collisionNormal,
            transition.SpherePath.GlobalCurrCenter[sphereNum].Center
        );
    }
    ...
    public TransitionState StepSphereDown(Transition transition, Sphere checkPos, Vector3 disp, float radsum)
    {
        ...
        if (
            CollidesWithSphere(checkPos, disp, radsum)
            || path.NumSphere > 1 && CollidesWithSphere(globSphere_, disp_, radsum)
        )
        {
            ...
            var contactPoint = new Vector3(
                checkPos.Center.X,
                checkPos.Center.Y,
                checkPos.Center.Z + (deltaz - checkPos.Radius)
            );
    ...
    {
        return cylSphere != null
            && Height == cylSphere.Height
            && Radius == cylSphere.Radius
            && LowPoint == cylSphere.LowPoint;
    }
