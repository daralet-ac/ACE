...
public class PhysicsObj
{
    ...
    public bool IsMovingOrAnimating =>
        IsAnimating
        || !PartArray.Sequence.is_first_cyclic()
    ...
    public void AddPartToShadowCells(PhysicsPart part)
    {
        ...
        foreach (var shadowObj in ShadowObjects.Values)
        {
            ...
            if (shadowCell != null)
            {
                shadowCell.AddPart(part, null, shadowCell.Pos.Frame, ShadowObjects.Count);
            }
        }
    }
    ...
    public bool CheckPositionInternal(ObjCell newCell, Position newPos, Transition transition, SetPosition setPos)
    {
        ...
        if (!transition.FindValidPosition())
        {
            return false;
        }
    ...
    public void DoObjDescChanges(int objDesc)
    {
        ...
        Translucency = Translucency < TranslucencyOriginal ? TranslucencyOriginal : Translucency;
    ...
    public TransitionState FindObjCollisions(Transition transition)
    {
        ...
        if (
            WeenieObj != null
            && transition.ObjectInfo.State.HasFlag(ObjectInfoState.IsViewer)
        ...
        if (
            State.HasFlag(PhysicsState.Ethereal)
            || !State.HasFlag(PhysicsState.Static) && transition.ObjectInfo.Ethereal
        )
        {
        ...
        {
            if (
                transition.ObjectInfo.Object.WeenieObj.WorldObject != null
                && transition.ObjectInfo.Object.WeenieObj.WorldObject is SpellProjectile
            )
            {
                ethereal = true;
            }
        ...
        var exemption = !( /*WeenieObj == null*/
            DatObject
            || !WeenieObj.IsPlayer()
            || !state.HasFlag(ObjectInfoState.IsPlayer)
            || state.HasFlag(ObjectInfoState.IsImpenetrable)
            || WeenieObj.IsImpenetrable()
            || state.HasFlag(ObjectInfoState.IsPK) && WeenieObj.IsPK()
            || state.HasFlag(ObjectInfoState.IsPKLite) && WeenieObj.IsPKLite()
        );
    ...
    public TransitionState FindObjCollisions_Inner(
    ...
    {
        var result =
            PartArray != null && obj.PartArray != null ? PartArray.MorphToExistingObject(obj.PartArray) : false;
    ...
    public void MoveToObject_Internal(
        PhysicsObj obj,
        uint topLevelID,
        float objRadius,
        float objHeight,
    ...
    public bool SetMotionTableID(uint mtableID)
    {
        ...
        if (!PartArray.SetMotionTableID(mtableID))
        {
            return false;
        }
    ...
    public void SetPartTranslucency(int partIdx, float startTrans, float endTrans, double delta)
    {
        ...
        var hook = new FPHook(
            PhysicsHookType.PartTranslucency,
            PhysicsTimer_CurrentTime,
            delta,
            startTrans,
            endTrans,
            partIdx
    ...
    public SetPositionError SetPositionInternal(Position pos, SetPosition setPos, Transition transition)
    {
        ...
        var newCell = AdjustPosition(
            pos,
            transition.SpherePath.LocalSphere[0].Center,
            setPos.Flags.HasFlag(SetPositionFlags.DontCreateCells),
            true
        );
        ...
        if (!CheckPositionInternal(newCell, pos, transition, setPos))
        {
            return handle_all_collisions(transition.CollisionInfo, false, false)
                ? SetPositionError.Collided
                : SetPositionError.NoValidPosition;
        }
        ...
        var spellCollide =
            WeenieObj.WorldObject is SpellProjectile
            && transition.CollisionInfo.CollideObject.Count > 0
            && !PropertyManager.GetBool("spell_projectile_ethereal").Item;
        ...
        if (entering_world && transition.SpherePath.CurPos.Landblock != pos.Landblock)
        {
            ...
            _log.Debug(
                "{Name} ({Id:X8}) AddPhysicsObj() - {PositionLocation} resulted in {TransitionSpherePathCurPosLocation} discarding",
                Name,
                ID,
                pos.ShortLoc(),
                transition.SpherePath.CurPos.ShortLoc()
            );
        ...
        for (var i = 0; i < setPos.NumTries; i++)
        {
            ...
            if ((newPos.ObjCellID & 0xFFFF) < 0x100)
            {
                ...
                if (sortCell == null || !sortCell.has_building())
                {
                    ...
                    {
                        _log.Debug(
                            "{Name} ({Id:X8}).SetScatterPositionInternal() - tried to spawn outdoor object @ {NewPos} ground Z {GroundZ} (diff: {NewPosFrameOriginZ - GroundZ}), investigate ScatterThreshold_Z",
                            Name,
                            ID,
                            newPos,
                            groundZ,
    ...
    public void SetTranslucency(float translucency, double delta)
    {
        if (delta < PhysicsGlobals.EPSILON)
        {
            Translucency = translucency < TranslucencyOriginal ? translucency : TranslucencyOriginal;
    ...
    public void SetTranslucency2(float startTrans, float endTrans, double delta)
    {
        if (delta < PhysicsGlobals.EPSILON)
        {
            ...
            if (PartArray != null)
            {
                PartArray.SetTranslucencyInternal(startTrans);
            }
    ...
    {
        if (!ExaminationObject)
        {
            return true;
        }

        return !(
            CYpt > degradeDistance || CurCell == null /*|| vftable unknown release */
        );
    }
    ...
    {
        var frame =
            partIdx >= PartArray.NumParts
                ? AFrame.Combine(Position.Frame, childFrame)
                : AFrame.Combine(PartArray.Parts[partIdx].Pos.Frame, childFrame);

        childObj.set_frame(frame);

        if (childObj.ParticleManager != null)
        {
            childObj.ParticleManager.UpdateParticles();
        }
    ...
    {
        if (PartArray == null || Children == null || Children.NumObjects == 0)
        {
            return;
        }
        ...
        if (PartArray != null && PartArray.GetNumSphere() != 0)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        _log.Debug(
                            "{Name} ({Id:X8}).UpdateObjectInternal({Quantum}) - failed transition from {Position} to {NewPosition}",
                            Name,
                            ID,
                            quantum,
                            Position,
                    ...
                    {
                        _log.Warning(
                            $"{Name} ({ID:X8}).UpdateObjectInternal({quantum}) - avoided CurCell=null from {Position} to {newPos}"
                        );
                    }
    ...
    public bool UpdateObjectInternalServer(double quantum)
    {
        ...
        {
            _log.Debug(
                "{Name}.UpdateObjectInternalServer({Quantum}) - failed transition from {Position} to {RequestPosition}",
                Name,
                quantum,
                Position,
                RequestPos
    ...
    public void UpdatePositionInternal(double quantum, ref AFrame newFrame)
    {
        ...
        if (!State.HasFlag(PhysicsState.Hidden))
        {
            if (PartArray != null)
            {
                PartArray.Update(quantum, ref offsetFrame);
            }
    ...
    public void UpdateViewerDistance(float cypt, Vector3 heading)
    {
        if (PartArray != null)
        {
            PartArray.UpdateViewerDistance(cypt, heading);
        }
    ...
    public void UpdateViewerDistance()
    {
        var min_2D_degrade_dist_sq = State.HasFlag(PhysicsState.ParticleEmitter)
            ? Render.ParticleDistance2DSquared
            : Render.ObjectDistance2DSquared;
    ...
    public void UpdateViewerDistanceRecursive()
    {
        ...
        if (Children == null || Children.NumObjects == 0)
        {
            return;
        }
    ...
    {
        if (PartArray == null || obj.Equals(this))
        {
            return false;
        }
    ...
    {
        if (
            TransientState.HasFlag(TransientStateFlags.Contact)
        ...
        else
        {
            if (PartArray != null && PartArray.GetNumCylsphere() != 0)
            {
                ObjCell.find_cell_list(
                    Position,
                    PartArray.GetNumCylsphere(),
                    PartArray.GetCylSphere(),
                    CellArray,
                    null
                );
            }
            else
            {
                ...
                var sphere =
                    PartArray != null && PartArray.Setup.SortingSphere != null
                        ? PartArray.GetSortingSphere()
                        : PhysicsGlobals.DummySphere;
                ObjCell.find_cell_list(Position, sphere, CellArray, null);
            }
        }
        remove_shadows_from_cells();
    ...
    public Quadrant check_attack(
        Position attackerPos,
        float attackerScale,
        AttackCone attackCone,
        float attackerAttackRadius
    )
    {
        if (
            Parent != null
            || State.HasFlag(PhysicsState.IgnoreCollisions)
            || State.HasFlag(PhysicsState.ReportCollisionsAsEnvironment)
        )
        {
            return 0;
        }
    ...
    public bool check_contact(bool contact)
    {
        if (
            TransientState.HasFlag(TransientStateFlags.Contact)
            && Vector3.Dot(Velocity, ContactPlane.Normal) > PhysicsGlobals.EPSILON
        )
        {
            return false;
        }
    ...
    public void clear_transient_states()
    {
        ...
        if (MovementManager != null && walkable)
        {
            MovementManager.LeaveGround();
        }
    ...
    {
        if (PartArray == null || CurCell == null)
        {
            return;
        }
    ...
    public bool handle_all_collisions(CollisionInfo collisions, bool prev_has_contact, bool prev_on_walkable)
    {
        ...
        if (
            prev_on_walkable
            && TransientState.HasFlag(TransientStateFlags.OnWalkable)
            && !State.HasFlag(PhysicsState.Sledding)
        )
        {
            apply_bounce = false;
        }
        ...
        if (CollidingWithEnvironment)
        {
            ...
            collisions.CollidedWithEnvironment
            || !prev_on_walkable && TransientState.HasFlag(TransientStateFlags.OnWalkable)
        )
        {
        ...
        var spellProjectile =
            WeenieObj.WorldObject is SpellProjectile ? WeenieObj.WorldObject as SpellProjectile : null;
        var spellType =
            spellProjectile != null
                ? SpellProjectile.GetProjectileSpellType(spellProjectile.Spell.Id)
    ...
    public void handle_visible_cells_non_player()
    {
        ...
        else
        {
            ...
            var knownPlayers = ObjectMaint.InitialClamp
                ? ObjMaint.GetVisibleObjectsDist(CurCell, ObjectMaint.VisibleObjectType.Players)
    ...
    {
        return TransientState.HasFlag(TransientStateFlags.Contact)
            && TransientState.HasFlag(TransientStateFlags.OnWalkable);
    }
    ...
    public bool play_default_script(int partIdx)
    {
        ...
        for (var i = 0; i < Children.NumObjects; i++)
        {
            if (Children.PartNumbers[i] == partIdx && Children.Objects[i] != null)
            {
                ...
                {
                    if (childObj.PhysicsScriptTable == null)
                    {
                        return false;
                    }

                    var script = childObj.PhysicsScriptTable.GetScript(
                        childObj.DefaultScript,
                        childObj.DefaultScriptIntensity
                    );
    ...
    public void process_fp_hook(PhysicsHookType type, float curr_value, Object userData)
    {
        switch (type)
        {
            ...
            case PhysicsHookType.PartTranslucency:
                if (PartArray != null)
                {
                    PartArray.SetPartTranslucencyInternal((int)userData, curr_value);
                }
            ...
            case PhysicsHookType.Luminosity:
                if (PartArray != null)
                {
                    PartArray.SetLuminosityInternal(curr_value);
                }
            ...
            case PhysicsHookType.PartLuminosity:
                if (PartArray != null)
                {
                    PartArray.SetPartLuminosityInternal((int)userData, curr_value);
                }
            ...
            case PhysicsHookType.Diffusion:
                if (PartArray != null)
                {
                    PartArray.SetDiffusionInternal(curr_value);
                }
            ...
            case PhysicsHookType.PartDiffusion:
                if (PartArray != null)
                {
                    PartArray.SetPartDiffusionInternal((int)userData, curr_value);
                }
    ...
    public void recalc_cross_cells()
    {
        ...
        {
            if (!ExaminationObject || !State.HasFlag(PhysicsState.ParticleEmitter))
            {
                return;
            }
    ...
    {
        if (CollisionTable == null || ObjMaint == null)
        {
            return;
        }
    ...
    public bool report_object_collision(PhysicsObj obj, bool prev_has_contact)
    {
        ...
        if (
            obj.State.HasFlag(PhysicsState.ReportCollisions)
            && !State.HasFlag(PhysicsState.IgnoreCollisions)
            && obj.WeenieObj != null
        )
        {
            ...
            var profile = obj.build_collision_profile(
                this,
                obj.TransientState.HasFlag(TransientStateFlags.Contact),
                velocityCollide
            );
    ...
    public ObjCollisionProfile build_collision_profile(PhysicsObj obj, bool amIInContact, Vector3 velocityCollide)
    {
        ...
        {
            return build_collision_profile(
                obj,
                velocityCollide,
                amIInContact,
                obj.State.HasFlag(PhysicsState.Missile),
                obj.TransientState.HasFlag(TransientStateFlags.Contact)
            );
        }
    }

    public ObjCollisionProfile build_collision_profile(
        PhysicsObj obj,
        Vector3 velocity,
        bool amIInContact,
        bool objIsMissile,
    ...
    public void set_frame(AFrame frame)
    {
        ...
        if (
            PartArray != null
            && !State.HasFlag(PhysicsState.ParticleEmitter)
    ...
    public bool set_parent(PhysicsObj obj, int partIdx, AFrame frame)
    {
        ...
        if (!obj.add_child(this, partIdx, frame))
        {
            return false;
        }
    ...
    public bool track_object_collision(PhysicsObj obj, bool prev_has_contact)
    {
        ...
        CollisionTable[obj.ID] = new CollisionRecord(
            PhysicsTimer.CurrentTime,
            obj.State.HasFlag(PhysicsState.Ethereal)
        );
    ...
    public bool update_object_server(bool forcePos = true)
    {
        ...
        if (wo != null && wo.Teleporting)
        {
            ...
            setPosition.Flags =
                SetPositionFlags.SendPositionEvent
                | SetPositionFlags.Slide
                | SetPositionFlags.Placement
                | SetPositionFlags.Teleport;
...
return;
    ...
    {
        get => TransientState.HasFlag(TransientStateFlags.OnWalkable) && CachedVelocity.Equals(Vector3.Zero);
    }
