...
public class MoveToManager
{
    ...
    public void HandleMoveToPosition()
    {
        ...
        {
            var heading =
                MovementParams.get_desired_heading(CurrentCommand, MovingAway) + curPos.heading(CurrentTargetPosition);
            if (heading >= 360.0f)
            {
                heading -= 360.0f;
            }
            ...
            {
                var motionID = diff >= 180.0f ? (uint)MotionCommand.TurnLeft : (uint)MotionCommand.TurnRight;
        ...
        else
        {
            ...
            FailProgressCount = 0;
            if (
                MovingAway && dist >= MovementParams.MinDistance
                || !MovingAway && dist <= MovementParams.DistanceToObject
                || inRange
            )
            {
    ...
    public void BeginTurnToHeading()
    {
        ...
        if (PhysicsObj.IsAnimating && !AlwaysTurn)
        {
            return;
        }
