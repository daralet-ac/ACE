...
public class InterpolationManager
{
    ...
    public void InterpolateTo(Position position, bool keepHeading)
    {
        ...
        var dest =
            PositionQueue.Count > 0 && PositionQueue.Last.Value.Type == InterpolationNodeType.PositionType
                ? PositionQueue.Last.Value.Position
                : PhysicsObj.Position;
        ...
        if (PhysicsObj.GetAutonomyBlipDistance() >= dist)
        {
            if (PhysicsObj.Position.Distance(position) > 0.05f)
            {
                while (PositionQueue.Count > 0)
                {
                    ...
                    if (
                        lastNode.Type != InterpolationNodeType.PositionType
                        || lastNode.Position.Distance(position) >= 0.05f
                    )
                    {
                        break;
                    }
    ...
    {
        if (
            PositionQueue.Count == 0
            || PhysicsObj == null
            || !PhysicsObj.TransientState.HasFlag(TransientStateFlags.Contact)
        )
        {
            return;
        }
        ...
        if (
            FrameCounter < 5
            || (
                PhysicsObj.get_sticky_object() != 0
