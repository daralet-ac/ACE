...
public class ObjectInfo
{
    ...
    public bool MissileIgnore(PhysicsObj collideObj)
    {
        ...
        if (Object.State.HasFlag(PhysicsState.Missile))
        {
            ...
            {
                if (
                    collideObj.State.HasFlag(PhysicsState.Ethereal)
                    || TargetID != 0 && collideObj.WeenieObj.IsCreature()
                )
                {
                    return true;
                }
    ...
    public TransitionState ValidateWalkable(
        Sphere checkPos,
        Plane contactPlane,
        bool isWater,
        float waterDepth,
    ...
    )
    {
        ...
        {
            var dist =
                Vector3.Dot(checkPos.Center - new Vector3(0, 0, checkPos.Radius), contactPlane.Normal)
                + contactPlane.D
                + waterDepth;
            if (dist >= -PhysicsGlobals.EPSILON)
            {
                ...
                if (
                    path.StepDown
                    || !State.HasFlag(ObjectInfoState.OnWalkable)
                    || PhysicsObj.is_valid_walkable(contactPlane.Normal)
                )
                {
