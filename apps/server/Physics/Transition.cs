...
public class Transition
{
    ...
    public Vector3 AdjustOffset(Vector3 _offset)
    {
        ...
        var dist =
            Vector3.Dot(globSphere.Center - blockOffset, CollisionInfo.ContactPlane.Normal)
            + CollisionInfo.ContactPlane.D;
    ...
    {
        if (
            !ObjectInfo.State.HasFlag(ObjectInfoState.OnWalkable)
            || !ObjectInfo.State.HasFlag(ObjectInfoState.EdgeSlide)
        )
        {
    ...
    public bool FindPlacementPosition()
    {
        ...
        else
        {
            stepDownHeight *= 0.5f;
            if (
                !StepDown(stepDownHeight, PhysicsGlobals.LandingZ) && !StepDown(stepDownHeight, PhysicsGlobals.LandingZ)
            )
            {
                SpherePath.RestoreCheckPos();
                CollisionInfo.ContactPlaneValid = false;
    ...
    public bool FindTransitionalPosition()
    {
        ...
        for (var step = 0; step < numSteps; step++)
        {
            ...
            if ((ObjectInfo.State & ObjectInfoState.FreeRotate) == 0)
            {
                ...
                SpherePath.CheckPos.Frame.InterpolateRotation(
                    SpherePath.BeginPos.Frame,
                    SpherePath.EndPos.Frame,
                    delta
                );
            }
    ...
    public bool StepDown(float stepDownHeight, float zVal)
    {
        ...
        if (
            ...
            && CollisionInfo.ContactPlane.Normal.Z >= zVal
            ...
            if (transitState == TransitionState.OK)
            {
                if (!SpherePath.Collide)
                {
                    if (SpherePath.NegPolyHit && !SpherePath.StepDown && !SpherePath.StepUp)
                    {
                        ...
                        {
                            transitState = SpherePath
                                .GlobalSphere[0]
                                .SlideSphere(
                                    this,
                                    ref SpherePath.NegCollisionNormal,
                                    SpherePath.GlobalCurrCenter[0].Center
                                );
                        }
                    }
                    else
                    {
                        if (
                            CollisionInfo.ContactPlaneValid
                            || (ObjectInfo.State & ObjectInfoState.Contact) == 0
                            || SpherePath.StepDown
                            || SpherePath.CheckCell == null
                            || !ObjectInfo.StepDown
                        )
                        {
                            return TransitionState.OK;
            ...
            if (transitionState != TransitionState.OK)
            {
                if (transitionState != TransitionState.Invalid)
                {
                    if (CollisionInfo.LastKnownContactPlaneValid)
                    {
                        ...
                        var angle =
                            Vector3.Dot(
                                CollisionInfo.LastKnownContactPlane.Normal,
                                SpherePath.GlobalCurrCenter[0].Center
                            ) + CollisionInfo.LastKnownContactPlane.D;
                        if (SpherePath.GlobalSphere[0].Radius + PhysicsGlobals.EPSILON > Math.Abs(angle))
                        {
                            CollisionInfo.SetContactPlane(
                                CollisionInfo.LastKnownContactPlane,
                                CollisionInfo.LastKnownContactPlaneIsWater
                            );
