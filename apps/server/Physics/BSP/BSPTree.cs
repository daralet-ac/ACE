...
public class BSPTree : IEquatable<BSPTree>
{
    ...
    public TransitionState collide_with_pt(
    ...
    public TransitionState find_collisions(Transition transition, float scale)
    {
        ...
        if (path.InsertType == InsertType.Placement || path.ObstructionEthereal)
        {
            ...
            if (
                RootNode.sphere_intersects_solid(localSphere, clearCell)
                || path.NumSphere > 1 && RootNode.sphere_intersects_solid(localSphere_, clearCell)
            )
            {
                return TransitionState.Collided;
            }
        ...
        {
            if (
                RootNode.sphere_intersects_poly(localSphere_, movement, ref hitPoly, ref contactPoint)
                || hitPoly != null
            )
            {
                var collisionNormal = path.LocalSpacePos.LocalToGlobalVec(hitPoly.Plane.Normal);
