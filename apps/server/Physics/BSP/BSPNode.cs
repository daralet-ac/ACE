...
public class BSPNode : IEquatable<BSPNode>
{
    ...
    public BSPNode(
        DatLoader.Entity.BSPNode node,
        Dictionary<ushort, DatLoader.Entity.Polygon> polys,
        DatLoader.Entity.CVertexArray vertexArray
    )
    {
        if (node.Sphere != null)
        {
    ...
    public virtual void find_walkable(
        SpherePath path,
        Sphere validPos,
        ref Polygon polygon,
        Vector3 movement,
    ...
    public BoundingType sphere_intersects_cell_bsp(Sphere curSphere)
    {
        ...
        if (PosNode != null)
        {
            return PosNode.sphere_intersects_cell_bsp(curSphere) != BoundingType.Outside
                ? BoundingType.PartiallyInside
                : BoundingType.Outside;
        }
    ...
    public virtual bool sphere_intersects_solid_poly(
        Sphere checkPos,
        float radius,
    ...
    {
        if (
            Sphere != null && !Sphere.Equals(node.Sphere)
            || !SplittingPlane.is_equal(node.SplittingPlane)
            || Type != node.Type
            || Typename != null && !Typename.Equals(node.Typename)
            || NumPolys != node.NumPolys
        )
        {
            return false;
        }
