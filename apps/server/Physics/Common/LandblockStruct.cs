...
public class LandblockStruct
{
    ...
    static LandblockStruct()
    {
        ...
        LandUVs.AddRange(
            new List<Vector2>() { new Vector2(0, 1), new Vector2(1, 1), new Vector2(1, 0), new Vector2(0, 0) }
        );
    ...
    public void CalcWater()
    {
        ...
        WaterType = hasWater
            ? waterblock
                ? LandDefs.WaterType.EntirelyWater
                : LandDefs.WaterType.PartiallyWater
            : LandDefs.WaterType.NotWater;
    }
        ...
        for (var x = 0; x < SideCellCount; x++)
        {
            ...
            for (var y = 0; y < SideCellCount; y++)
            {
                ...
                for (var i = 0; i < LandDefs.VertexPerCell; i++)
                {
                    ...
                    {
                        var splitDir = (uint)(
                            ((int)lcoord.Y * LandDefs.VertexPerCell + idxJ) * magicA - magicB - 1369149221
                        );
                        ...
                        if (splitDir * 2.3283064e-10 < 0.5f)
                        {
                            ...
                            lcell.Polygons[polyIdx + 1] = AddPolygon(
                                firstVertex + 1,
                                nextVertIdx + 1,
                                vertIdx + 1,
                                nextVertIdx
    ...
    public void GetCellRotation(
        uint landblockID,
        uint x,
        uint y,
        ref bool singleTextureCell,
        ref uint surfNum,
        ref LandDefs.Rotation rotation
    )
    {
    ...
    public void Init()
    {
        ...
        for (uint i = 1; i <= 64; i++)
        {
            LandCells.TryAdd((int)i, new LandCell((i)));
        }
    }
    ...
    public void TransAdjust()
    {
        // refactor me..
        if (
            TransDir == LandDefs.Direction.North
            || TransDir == LandDefs.Direction.NorthWest
            || TransDir == LandDefs.Direction.NorthEast
        )
        {
        ...
        if (
            TransDir == LandDefs.Direction.West
            || TransDir == LandDefs.Direction.NorthWest
            || TransDir == LandDefs.Direction.SouthWest
        )
        {
            for (var i = 1; i < SidePolyCount; i += 2)
            {
        ...
        if (
            TransDir == LandDefs.Direction.South
            || TransDir == LandDefs.Direction.SouthWest
            || TransDir == LandDefs.Direction.SouthEast
        )
        {
            for (var i = 1; i < SidePolyCount; i += 2)
            {
        ...
        if (
            TransDir == LandDefs.Direction.East
            || TransDir == LandDefs.Direction.NorthEast
            || TransDir == LandDefs.Direction.SouthEast
        )
        {
