...
public class TextureMergeInfo
{
    ...
    public void Debug()
    {
        ...
        Console.WriteLine(
            $"TerrainOverlays: {String.Join(",", TerrainOverlays.Where(t => t != null).Select(t => t.TexGID.ToString("X8")))}"
        );
        Console.WriteLine(
            $"TerrainAlphaOverlays: {String.Join(",", TerrainAlphaOverlays.Where(t => t != null).Select(t => t.TexGID.ToString("X8")))}"
        );
