...
public class LandCell : SortCell
{
    ...
    public override TransitionState FindEnvCollisions(Transition transition)
    {
        ...
        if (
            get_block_water_type() == LandDefs.WaterType.EntirelyWater
            && !objInfo.State.HasFlag(ObjectInfoState.IsViewer)
            && !objInfo.Object.State.HasFlag(PhysicsState.Missile)
        )
        {
        ...
        return objInfo.ValidateWalkable(
            checkPos,
            walkable.Plane,
    ...
    public static void add_all_outside_cells(
        Position position,
        int numSphere,
        List<Sphere> spheres,
        CellArray cellArray
    ...
    /// </summary>
    public static void check_add_cell_boundary(
        CellArray cellArray,
        Vector2 point,
        Vector2 lcoord,
        float minRad,
