...
public class ObjCell : PartCell, IEquatable<ObjCell>
{
    ...
    public TransitionState FindObjCollisions(Transition transition)
    {
        ...
        try
        {
            ...
            for (var i = ShadowObjectList.Count - 1; i >= 0; i--)
            {
    ...
    public TransitionState check_entry_restrictions(Transition transition)
    {
        ...
        if (
            !transition.ObjectInfo.Object.IsPlayer
            || transition.CollisionInfo.VerifiedRestrictions
            || transition.SpherePath.BeginCell?.ID == ID
        )
        {
            return TransitionState.OK;
        }
    ...
    public static void find_cell_list(
        Position position,
        int numSphere,
        List<Sphere> sphere,
        CellArray cellArray,
        ref ObjCell currCell,
        SpherePath path
    )
    {
    ...
    public virtual void find_transit_cells(
        Position position,
        int numSphere,
        List<Sphere> sphere,
        CellArray cellArray,
