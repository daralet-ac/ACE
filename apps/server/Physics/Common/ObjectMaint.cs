...
public class ObjectMaint
{
    ...
    public List<KeyValuePair<uint, PhysicsObj>> GetKnownObjectsWhere(
        Func<KeyValuePair<uint, PhysicsObj>, bool> predicate
    )
    {
        rwLock.EnterReadLock();
        try
    ...
    public List<KeyValuePair<uint, PhysicsObj>> GetVisibleObjectsWhere(
        Func<KeyValuePair<uint, PhysicsObj>, bool> predicate
    )
    {
        rwLock.EnterReadLock();
    ...
    public List<PhysicsObj> GetVisibleObjects(ObjCell cell, VisibleObjectType type = VisibleObjectType.All)
    {
        ...
        try
        {
            ...
            return ApplyFilter(visibleObjs, type)
                .Where(i => i.ID != PhysicsObj.ID && (!(i.CurCell is EnvCell indoors) || indoors.SeenOutside))
                .ToList();
        }
        finally
        {
    ...
    private List<PhysicsObj> GetVisibleObjects(EnvCell cell, VisibleObjectType type)
    {
        ...
        if (cell.SeenOutside)
        {
            var outsideObjs = PhysicsObj
                .CurLandblock.GetServerObjects(true)
                .Where(i => !(i.CurCell is EnvCell indoors) || indoors.SeenOutside);
    ...
    private IEnumerable<PhysicsObj> ApplyFilter(List<PhysicsObj> objs, VisibleObjectType type)
    {
        ...
        else if (type == VisibleObjectType.AttackTargets)
        {
            ...
            {
                results = objs.Where(i =>
                    i.WeenieObj.IsMonster && i.WeenieObj.PlayerKillerStatus != PlayerKillerStatus.PK
            ...
            else if (PhysicsObj.WeenieObj.IsFactionMob)
            {
                results = objs.Where(i =>
                    i.IsPlayer
                    || i.WeenieObj.IsCombatPet
                    || i.WeenieObj.IsMonster && !i.WeenieObj.SameFaction(PhysicsObj)
                );
            }
            else
            {
                ...
                results = objs.Where(i =>
                    i.IsPlayer
                    || i.WeenieObj.IsCombatPet && PhysicsObj.WeenieObj.PlayerKillerStatus != PlayerKillerStatus.PK
                    || i.WeenieObj.IsFactionMob
                    || i.WeenieObj.PotentialFoe(PhysicsObj)
                );
            }
        }
        return results;
    ...
    public List<PhysicsObj> DestroyObjects()
    {
        ...
        try
        {
            ...
            var expiredObjs = DestructionQueue
                .Where(kvp => kvp.Value <= PhysicsTimer.CurrentTime)
                .Select(kvp => kvp.Key)
                .ToList();
    ...
    public List<KeyValuePair<uint, PhysicsObj>> GetKnownPlayersWhere(
        Func<KeyValuePair<uint, PhysicsObj>, bool> predicate
    )
    {
        rwLock.EnterReadLock();
    ...
    private bool AddKnownPlayer(PhysicsObj obj)
    {
        ...
        {
            Console.WriteLine(
                $"{PhysicsObj.Name}.ObjectMaint.AddKnownPlayer({obj.Name}): tried to add player for dat object"
            );
            return false;
        }
    ...
    private bool AddVisibleTarget(PhysicsObj obj, bool clamp = true, bool foeType = false)
    {
        if (PhysicsObj.WeenieObj.IsCombatPet)
        {
            ...
            {
                Console.WriteLine(
                    $"{PhysicsObj.Name}.ObjectMaint.AddVisibleTarget({obj.Name}): tried to add a non-monster"
                );
                return false;
            }
        }
        else if (PhysicsObj.WeenieObj.IsFactionMob)
        {
            ...
            if (
                !obj.IsPlayer
                && !obj.WeenieObj.IsCombatPet
                && (!obj.WeenieObj.IsMonster || PhysicsObj.WeenieObj.SameFaction(obj))
            )
            {
                Console.WriteLine(
                    $"{PhysicsObj.Name}.ObjectMaint.AddVisibleTarget({obj.Name}): tried to add a non-player / non-combat pet / non-opposing faction mob"
                );
                return false;
            }
        }
        else
        {
            ...
            if (
                obj.WeenieObj.FoeType != null
                && obj.WeenieObj.FoeType == PhysicsObj.WeenieObj.WorldObject?.CreatureType
                && (
                    PhysicsObj.WeenieObj.FoeType == null
                    || obj.WeenieObj.WorldObject != null
                        && PhysicsObj.WeenieObj.FoeType != obj.WeenieObj.WorldObject.CreatureType
                )
            )
            {
                obj.ObjMaint.AddVisibleTarget(PhysicsObj);
            ...
            {
                Console.WriteLine(
                    $"{PhysicsObj.Name}.ObjectMaint.AddVisibleTarget({obj.Name}): tried to add a non-player / non-combat pet"
                );
                return false;
            }
        }
        ...
        {
            Console.WriteLine(
                $"{PhysicsObj.Name}.ObjectMaint.AddVisibleTarget({obj.Name}): tried to add player for dat object"
            );
            return false;
        }
