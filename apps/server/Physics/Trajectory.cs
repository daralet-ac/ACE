...
public class Trajectory
{
    ...
    )
    {
        ...
        {
            var sqrt_D = Math.Sqrt(D);
            var u = Math.Pow(sqrt_D - q, 1.0 / 3.0);
            var v = -Math.Pow(sqrt_D + q, 1.0 / 3.0);
    ...
    public static float ballistic_range(float speed, float gravity, float initial_height)
    {
        ...
        var range =
            (speed * cos / gravity)
            * (speed * sin + Math.Sqrt(speed * speed * sin * sin + 2 * gravity * initial_height));
        return (float)range;
    }
    ...
    )
    {
        ...
        var diffXY = new Vector3(diff.X, diff.Y, 0);
        var groundDist = diffXY.Length();
        ...
        var root = speed4 - gravity * (gravity * x * x + 2 * z * speed2);
        ...
        var groundDir = Vector3.Normalize(diffXY);
        s0 = groundDir * (float)Math.Cos(lowAng) * proj_speed + Vector3.UnitZ * (float)Math.Sin(lowAng) * proj_speed;
        if (numSolutions > 1)
        {
            s1 =
                groundDir * (float)Math.Cos(highAng) * proj_speed
                + Vector3.UnitZ * (float)Math.Sin(highAng) * proj_speed;
        }
    ...
    )
    {
        ...
        var numTimes = SolveQuartic(c0, c1, c2, c3, c4, out times[0], out times[1], out times[2], out times[3]);
    ...
    )
    {
        ...
        var diff = targetPosition - projectilePosition;
        var diffXY = new Vector3(diff.X, diff.Y, 0f);
        var lateralDist = diffXY.Length();
        ...
        var a = projectilePosition.Z; // initial
        var c = targetPosition.Z; // final
    ...
    )
    {
        // Handling these cases is up to your project's coding standards
        Debug.Assert(
            proj_pos != target_pos && lateral_speed > 0 && max_height > proj_pos.Z,
        ...
        var time = lateralDist / lateral_speed;
        ...
        // end = z0 + vertical_speed*time + .5*gravity*time^s
        // Wolfram Alpha: solve b = a + .5*v*t + .5*g*(.5*t)^2, c = a + vt + .5*g*t^2 for g, v
        var a = proj_pos.Z; // initial
        var b = max_height; // peak
        var c = target_pos.Z; // final
    ...
    )
    {
        ...
        // Ground plane terms
        var targetVelXY = new Vector3(target_velocity.X, target_velocity.Y, 0f);
        var diffXY = target - proj_pos;
        diffXY.Z = 0;
        ...
        var c0 = Vector3.Dot(targetVelXY, targetVelXY) - lateral_speed * lateral_speed;
        var c1 = 2f * Vector3.Dot(diffXY, targetVelXY);
        var c2 = Vector3.Dot(diffXY, diffXY);
        double t0,
            t1;
        var n = SolveQuadric(c0, c1, c2, out t0, out t1);
