...
public class MotionTable
{
    ...
    public bool GetObjectSequence(
        uint motion,
        MotionState currState,
        Sequence sequence,
        float speedMod,
        ref uint numAnims,
        bool stopModifiers
    )
    {
        ...
        if ((motion & (uint)CommandMask.Style) != 0)
        {
            ...
            if (substate != 0)
            {
                ...
                if (cycles != null)
                {
                    ...
                    numAnims = (uint)(
                        (motionData != null ? motionData.Anims.Count : 0)
                        + (link != null ? link.Anims.Count : 0)
                        + (motionData_ != null ? motionData_.Anims.Count : 0)
                        + (cycles != null ? cycles.Anims.Count : 0)
                        - 1
                    );
        ...
        if ((motion & (uint)CommandMask.SubState) != 0)
        {
            ...
            if (motionData != null)
            {
                ...
                {
                    if (
                        motion == currState.Substate
                        && sequence.HasAnims()
                        && Math.Sign(speedMod) == Math.Sign(currState.SubstateMod)
                    )
                    {
                    ...
                    numAnims = (uint)(
                        (motionData == null ? 0 : motionData.Anims.Count)
                        + (link == null ? 0 : link.Anims.Count)
                ...
                else
                {
                    ...
                    if (motionData != null)
                    {
                        ...
                        if (link != null && Cycles.TryGetValue(cycleKey, out cycles))
                        {
                            ...
                            re_modify(sequence, currState);
                            numAnims = (uint)(
                                motionData.Anims.Count
                                + link.Anims.Count
                                + (motionData_ == null ? 0 : motionData.Anims.Count)
                            );
                            return true;
                        }
                    }
    ...
    public static MotionTable Get(uint motionTableID)
    {
        ...
        var motionTable = new MotionTable(
            DatManager.PortalDat.ReadFromDat<DatLoader.FileTypes.MotionTable>(motionTableID)
        );
        return motionTable;
    }
    ...
    public bool StopSequenceMotion(
        uint motion,
        float speed,
        MotionState currState,
        Sequence sequence,
    ...
    public MotionData get_link(uint style, uint substate, float substateSpeed, uint motion, float speed)
    {
        if (speed < 0.0f || substateSpeed < 0.0f)
        {
            ...
            if (
                StyleDefaults.TryGetValue(style, out var defaultMotion)
                && Links.TryGetValue((style << 16) | (substate & 0xFFFFFF), out var sublink)
            )
            {
    ...
    public static List<(float time, AttackHook attackHook)> GetAttackFrames(
        uint motionTableId,
        MotionStance stance,
    ...
    public static float GetAnimationLength(
        uint motionTableId,
        MotionStance stance,
        MotionCommand motion,
        float speed = 1.0f
    ...
    {
        if (RunSpeed.TryGetValue(motionTableID, out var runSpeed))
        {
            return runSpeed;
        }

        var runMotion = (uint)MotionCommand.RunForward;
    ...
    {
        if (TurnSpeed.TryGetValue(motionTableID, out var turnSpeed))
        {
            return turnSpeed;
        }

        var turnMotion = (uint)MotionCommand.TurnRight;
