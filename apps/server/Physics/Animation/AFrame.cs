...
public class AFrame : IEquatable<AFrame>
{
    ...
    {
        var originEpsilonEqual =
            Math.Abs(frame.Origin.X - Origin.X) <= PhysicsGlobals.EPSILON
            && Math.Abs(frame.Origin.Y - Origin.Y) <= PhysicsGlobals.EPSILON
            && Math.Abs(frame.Origin.Z - Origin.Z) <= PhysicsGlobals.EPSILON;

        if (!originEpsilonEqual)
        {
            return false;
        }

        var orientationEpsilonEqual =
            Math.Abs(frame.Orientation.X - frame.Orientation.X) <= PhysicsGlobals.EPSILON
