...
public class MotionInterp
{
    ...
    public void apply_raw_movement(bool cancelMoveTo, bool allowJump)
    {
        ...
        adjust_motion(
            ref InterpretedState.SideStepCommand,
            ref InterpretedState.SideStepSpeed,
            RawState.SideStepHoldKey
        );
    ...
    public int charge_jump()
    {
        ...
        if (
            forward == (uint)MotionCommand.Falling
            || forward >= (uint)MotionCommand.Crouch && forward < (uint)MotionCommand.Sleeping
        )
        {
            return 0x48;
        }
        else
        {
            if (
                ...
                && forward == (uint)MotionCommand.Ready
                && InterpretedState.SideStepCommand == 0
                && InterpretedState.TurnCommand == 0
            )
            {
    ...
    public bool contact_allows_move(uint motion)
    {
        ...
        if (
            motion == (uint)MotionCommand.Dead
            || motion == (uint)MotionCommand.Falling
    ...
    public WeenieError jump_charge_is_allowed()
    {
        ...
        if (
            forward == (uint)MotionCommand.Fallen
            || forward >= (uint)MotionCommand.Crouch && forward <= (uint)MotionCommand.Sleeping
        )
        {
    ...
    public WeenieError jump_is_allowed(float extent, int staminaCost)
    {
        ...
        )
        {
            ...
            if (jumpError == WeenieError.None)
            {
                ...
                if (
                    jumpError == WeenieError.None
                    && WeenieObj != null
                    && WeenieObj.JumpStaminaCost(extent, staminaCost) == 0
                )
                {
    ...
    {
        if (
            substate >= (uint)MotionCommand.Reload && substate <= (uint)MotionCommand.Pickup
