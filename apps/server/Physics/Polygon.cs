...
public class Polygon : IEquatable<Polygon>
{
    ...
    public void adjust_to_placement_poly(
        Sphere hitSphere,
        Sphere otherSphere,
        float radius,
        bool centerSolid,
    ...
    public bool point_in_poly2D(Vector3 point, Sidedness side)
    {
        ...
        for (var i = NumPoints - 1; i >= 0; i--)
        {
            ...
            var diffCross =
                (diff.Y * vertex.Origin.X) - (diff.X * vertex.Origin.Y) + (diff.X * point.Y) - (diff.Y * point.X);
    ...
    public bool Equals(Polygon p)
    {
        if (
            ...
            || Stippling != p.Stippling
            || SidesType != p.SidesType
            || PosSurface != p.PosSurface
            || NegSurface != p.NegSurface
        )
        {
            return false;
        }
