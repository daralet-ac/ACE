...
public class DerethDateTime
{
    ...
    /// </summary>
    private static DerethDateTime retailDayOne_Derethian_Lore = ConvertRealWorldToLoreDateTime(
        new DateTime(1999, 11, 2, 00, 00, 00)
    );

    /// <summary>
    /// <para>A <see cref="DerethDateTime"/> instance set to the Derethian Date, Portal Year and Time when the worlds closed.</para>
    /// </summary>
    private static DerethDateTime retailDayLast_Derethian = new DerethDateTime(
        206,
        Months.Solclaim,
        24,
        Hours.Gloaming
    ...
    /// </summary>
    private static DerethDateTime retailDayLast_Derethian_Lore = ConvertRealWorldToLoreDateTime(
        new DateTime(2017, 1, 31, 12, 00, 00)
    );
    ...
    /// </summary>
    public Hours HourName
    {
        get { return (Hours)Hour; }
    }
    ...
    public DerethDateTime(double ticks = 0)
    {
        if (ticks < MinValue | ticks > MaxValue)
        {
            throw new ArgumentOutOfRangeException(
                "ticks",
                "ticks is less than DerethDateTime.MinValue or greater than DerethDateTime.MaxValue"
            );
        }
    ...
    {
        var hoursInTicks = Ticks / hourTicks;
        var round = Math.Round(hoursInTicks * 4, MidpointRounding.ToEven) / 4;
        var round_multiplier = 100;
        var round_decimals = (int)((round - (int)round) * round_multiplier);
    ...
    public DerethDateTime(
        int year = 10,
        int month = (int)Months.Morningthaw,
        int day = 1,
        int hour = (int)Hours.Darktide
    )
    {
        if (year < 10 || year > 401)
        {
        ...
        if (month < (int)Months.Snowreap || month > (int)Months.Frostfell)
        {
            throw new ArgumentOutOfRangeException(
                "month",
                "month is less than " + Months.Snowreap + " or greater than " + Months.Frostfell
            );
        }
        ...
        if (hour < (int)Hours.Darktide || hour > (int)Hours.Gloaming_and_Half)
        {
            throw new ArgumentOutOfRangeException(
                "hour",
                "time is less than " + Hours.Darktide + " or greater than " + Hours.Gloaming_and_Half
            );
        }
    ...
    private double GetTicksFromDateTime(bool fixWintersebbYearRollUnder = false)
    {
        ...
        var monthsToAdd = (Month - 1) * monthTicks;

        var daysToAdd = (Day - 2) * dayTicks;

        var hoursToAdd = (Hour - 1) * hourTicks;
    ...
    public DerethDateTime AddTicks(double numOfTicksToAdd)
    {
        if ((Ticks + numOfTicksToAdd) < MinValue | (Ticks + numOfTicksToAdd) > MaxValue)
        {
            throw new ArgumentOutOfRangeException(
                "numOfTicksToAdd",
                "numOfTicksToAdd results in less than DerethDateTime.MinValue or greater than DerethDateTime.MaxValue"
            );
        }
    ...
    public DerethDateTime SubtractTicks(double numOfTicksToSubtract)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(
                "numOfTicksToSubtract",
                "numOfTicksToSubtract results in less than DerethDateTime.MinValue or greater than DerethDateTime.MaxValue"
            );
        }
    ...
    public DerethDateTime AddYears(int numOfYearsToAdd)
    {
        if ((Year + numOfYearsToAdd) < 10 | (Year + numOfYearsToAdd) > 401)
        {
            throw new ArgumentOutOfRangeException(
                "numOfYearsToAdd",
                "numOfYearsToAdd results in a portal year less than 10 or greater than 401"
            );
        }
    ...
    public DerethDateTime SubtractYears(int numOfYearsToSubtract)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(
                "numOfYearsToSubtract",
                "numOfYearsToSubtract results in a portal year less than 10 or greater than 401"
            );
        }
    ...
    public DerethDateTime AddMonths(int numOfMonthsToAdd)
    {
        ...
        if (newYear < 10 | newYear > 401)
        {
            throw new ArgumentOutOfRangeException(
                "numOfMonthsToAdd",
                "numOfMonthsToAdd results in a portal year less than 10 or greater than 401"
            );
        }
    ...
    {
        var opposite = numOfMonthsToSubtract * -1;
    ...
    public DerethDateTime AddDays(int numOfDaysToAdd)
    {
        ...
        if (newYear < 10 | newYear > 401)
        {
            throw new ArgumentOutOfRangeException(
                "numOfDaysToAdd",
                "numOfDaysToAdd results in a portal year less than 10 or greater than 401"
            );
        }
    ...
    {
        var opposite = numOfDaysToSubtract * -1;
    ...
    {
        var opposite = numOfHoursToSubtract * -1;
    ...
    public override string ToString()
    {
        return "Date: "
            ...
            + Year
            + " P.Y.  Time: "
            + Enum.GetName(typeof(Hours), Hour).Replace("_", "-");
    }
    ...
    {
        var convertedYear = 10;
        var convertedMonth = dateToBeConverted.Month - 3;
        var convertedDay = dateToBeConverted.Day;
        var convertedHour = 0;

        var yearsToAdd = dateToBeConverted.Year - retailDayOne_RealWorld.Year;
        ...
        {
            if (
                dateToBeConverted.Hour == (03 + 12)
                || (dateToBeConverted.Hour == (04 + 12) && dateToBeConverted.Minute <= 29)
            )
            {
        ...
        {
            if (
                dateToBeConverted.Hour == (06 + 12)
                || (dateToBeConverted.Hour == (07 + 12) && dateToBeConverted.Minute <= 29)
            )
            {
        ...
        {
            if (
                dateToBeConverted.Hour == (09 + 12)
                || (dateToBeConverted.Hour == (10 + 12) && dateToBeConverted.Minute <= 29)
            )
            {
    ...
    /// </summary>
    public static DerethDateTime UtcNowToGDLETime =>
        new DerethDateTime((DateTime.UtcNow - new DateTime(1999, 9, 1)).TotalSeconds);

    /// <summary>
    /// Converts the <see cref="DateTime.UtcNow"/> object to a new <see cref="DerethDateTime"/> object set to EMU Standard Sync Time.
    /// </summary>
    public static DerethDateTime UtcNowToEMUTime =>
        new DerethDateTime((DateTime.UtcNow.AddHours(-5) - retailDayLast_RealWorld).TotalSeconds); // -5 is Eastern Standard Time UTC Offset
}
