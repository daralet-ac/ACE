...
public class SpellBase : IUnpackable
{
    ...
    public void Unpack(BinaryReader reader)
    {
        ...
        var rawComps = new List<uint>();

        for (uint j = 0; j < 8; j++)
        {
            var comp = reader.ReadUInt32();
    ...
    private static List<uint> DecryptFormula(List<uint> rawComps, string name, string desc)
    {
        ...
        var nameHash = SpellTable.ComputeHash(name);
        var descHash = SpellTable.ComputeHash(desc);

        var key = (nameHash % 0x12107680) + (descHash % 0xBEADCF45);

        for (var i = 0; i < rawComps.Count; i++)
        {
