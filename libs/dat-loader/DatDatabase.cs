...
public class DatDatabase
{
    ...
    /// </summary>
    public T ReadFromDat<T>(uint fileId)
        where T : FileType, new()
    {
        // Check the FileCache so we don't need to hit the FileSystem repeatedly
        if (FileCache.TryGetValue(fileId, out var result))
        {
            return (T)result;
        }
    ...
    public DatReader GetReaderForFile(uint fileId)
    {
        ...
        {
            _log.Information(
                "Unable to find object_id {FileId:X8} in {DatDatabaseType}",
                fileId,
                Enum.GetName(typeof(DatDatabaseType), Header.DataSet)
            );
        }
