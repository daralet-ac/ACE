...
public class ClothingTable : FileType
{
    ...
    /// </summary>
    public Dictionary<uint, ClothingBaseEffect> ClothingBaseEffects { get; } =
        new Dictionary<uint, ClothingBaseEffect>();
    ...
    {
        if (ClothingSubPalEffects.TryGetValue(palEffectIdx, out var result))
        {
            return result.Icon;
        }
    ...
    public CoverageMask? GetVisualPriority(uint setupId = 0x02000001)
    {
        if (ClothingBaseEffects.ContainsKey(setupId))
        {
            ...
            foreach (var t in ClothingBaseEffects[setupId].CloObjectEffects)
            {
                switch (t.Index)
                {
