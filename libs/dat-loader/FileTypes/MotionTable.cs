...
public class MotionTable : FileType
{
    ...
    public Dictionary<uint, Dictionary<uint, MotionData>> Links { get; } =
        new Dictionary<uint, Dictionary<uint, MotionData>>();
    ...
    public override void Unpack(BinaryReader reader)
    {
        ...
        var numStyleDefaults = reader.ReadUInt32();
    ...
    private static readonly ConcurrentDictionary<
        AttackFrameParams,
        List<(float time, AttackHook attackHook)>
    > attackFrameCache = new ConcurrentDictionary<AttackFrameParams, List<(float time, AttackHook attackHook)>>();

    public List<(float time, AttackHook attackHook)> GetAttackFrames(
        uint motionTableId,
        MotionStance stance,
    ...
    public List<AnimData> GetAnimData(MotionStance stance, MotionCommand motion, MotionCommand currentMotion)
    {
        ...
        var motionHash = (uint)stance << 16 | (uint)currentMotion & 0xFFFFF;
    ...
    public ACE.Entity.Position GetAnimationFinalPositionFromStart(
        ACE.Entity.Position position,
        float objScale,
        MotionCommand motion
    )
    {
        ...
        var defaultMotion = GetDefaultMotion(defaultStyle);
    ...
    public ACE.Entity.Position GetAnimationFinalPositionFromStart(
        ACE.Entity.Position position,
        float objScale,
        MotionCommand currentMotionState,
        MotionStance style,
        MotionCommand motion
    )
    {
