    ...
    public static void Initialize(string datFileDirectory, bool keepOpen = false, bool loadCell = true)
    {
        ...
        if (loadCell)
        {
            try
            {
                ...
                _log.Information(
                    $"Successfully opened {datFile} file, containing {count} records, iteration {CellDat.Iteration}"
                );
                if (CellDat.Iteration != ITERATION_CELL)
                {
                    _log.Warning(
                        $"{datFile} iteration {CellDat.Iteration} does not match expected version of {ITERATION_CELL}."
                    );
                }
            }
            ...
            {
                _log.Error(
                    $"An exception occured while attempting to open {datFile} file!  This needs to be corrected in order for Landblocks to load!"
                );
        ...
        try
        {
            ...
            _log.Information(
                $"Successfully opened {datFile} file, containing {count} records, iteration {PortalDat.Iteration}"
            );
            if (PortalDat.Iteration != ITERATION_PORTAL)
            {
                _log.Warning(
                    $"{datFile} iteration {PortalDat.Iteration} does not match expected version of {ITERATION_PORTAL}."
                );
            }
        }
        catch (FileNotFoundException ex)
        ...
        if (File.Exists(datFile))
        {
            ...
            _log.Information(
                $"Successfully opened {datFile} file, containing {count} records, iteration {HighResDat.Iteration}"
            );
            if (HighResDat.Iteration != ITERATION_HIRES)
            {
                _log.Warning(
                    $"{datFile} iteration {HighResDat.Iteration} does not match expected iteration version of {ITERATION_HIRES}."
                );
            }
        }
        ...
        try
        {
            ...
            _log.Information(
                $"Successfully opened {datFile} file, containing {count} records, iteration {LanguageDat.Iteration}"
            );
            if (LanguageDat.Iteration != ITERATION_LANGUAGE)
            {
                _log.Warning(
                    $"{datFile} iteration {LanguageDat.Iteration} does not match expected version of {ITERATION_LANGUAGE}."
                );
            }
        }
        catch (FileNotFoundException ex)
