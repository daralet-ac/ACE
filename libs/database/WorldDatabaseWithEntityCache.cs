...
public class WorldDatabaseWithEntityCache : WorldDatabase
{
    ...
    private readonly ConcurrentDictionary<
        ...
        ,
        uint /* WCID */
    > weenieClassNameToClassIdCache = new ConcurrentDictionary<string, uint>();
    ...
    private readonly ConcurrentDictionary<WeenieType, List<ACE.Entity.Models.Weenie>> weenieCacheByType =
        new ConcurrentDictionary<WeenieType, List<ACE.Entity.Models.Weenie>>();

    private readonly Dictionary<
        uint /* Spell ID */
        ,
        ACE.Entity.Models.Weenie
    > scrollsBySpellID = new Dictionary<uint, ACE.Entity.Models.Weenie>();
    ...
    public List<ACE.Entity.Models.Weenie> GetRandomWeeniesOfType(int weenieTypeId, int count)
    {
        if (!weenieCacheByType.TryGetValue((WeenieType)weenieTypeId, out var weenies))
        {
            if (!weenieSpecificCachesPopulated)
            {
                ...
                {
                    var results = context.Weenie.AsNoTracking().Where(r => r.Type == weenieTypeId).ToList();
    ...
    public ACE.Entity.Models.Weenie GetScrollWeenie(uint spellID)
    {
        if (!scrollsBySpellID.TryGetValue(spellID, out var weenie))
        {
            if (!weenieSpecificCachesPopulated)
            {
                ...
                {
                    var query =
                        from weenieRecord in context.Weenie
                        join did in context.WeeniePropertiesDID on weenieRecord.ClassId equals did.ObjectId
                        where
                            weenieRecord.Type == (int)WeenieType.Scroll
                            && did.Type == (ushort)PropertyDataId.Spell
                            && did.Value == spellID
                        select weenieRecord;
    ...
    private readonly ConcurrentDictionary<string, uint> creatureWeenieNamesLowerInvariantCache =
    ...
    public bool IsCreatureNameInWorldDatabase(WorldDbContext context, string name)
    {
        var query =
            ...
            where
                ...
                && stringProperty.Type == (ushort)PropertyString.Name
                && stringProperty.Value == name
            select weenieRecord;

        var weenie = query.Include(r => r.WeeniePropertiesString).AsNoTracking().FirstOrDefault();
    ...
    private readonly Dictionary<
        ...
        Dictionary<
            ...
            CookBook
        >
    > cookbookCache = new Dictionary<uint, Dictionary<uint, CookBook>>();
    ...
    public override CookBook GetCookbook(WorldDbContext context, uint sourceWeenieClassId, uint targetWeenieClassId)
    {
        ...
        lock (cookbookCache)
        {
            ...
            else
            {
                cookbookCache.Add(
                    sourceWeenieClassId,
                    new Dictionary<uint, CookBook>() { { targetWeenieClassId, cookbook } }
                );
            }
        }
        ...
        lock (cookbookCache)
        {
            foreach (var cookbook in cookbooks)
            {
                ...
                else
                {
                    cookbookCache.Add(
                        cookbook.SourceWCID,
                        new Dictionary<uint, CookBook>() { { cookbook.TargetWCID, cookbook } }
                    );
                }
            }
        }
    ...
    private readonly ConcurrentDictionary<
        ...
        ,
        List<Encounter>
    > cachedEncounters = new ConcurrentDictionary<ushort, List<Encounter>>();
    ...
    public List<Encounter> GetCachedEncountersByLandblock(ushort landblock, out bool wasCached)
    {
        ...
        using (var context = new WorldDbContext())
        {
            ...
            var results = context.Encounter.AsNoTracking().Where(r => r.Landblock == landblock).ToList();
    ...
    private readonly ConcurrentDictionary<
        ...
        ,
        Event
    > cachedEvents = new ConcurrentDictionary<string, Event>();
    ...
    public Event GetCachedEvent(string name)
    {
        ...
        using (var context = new WorldDbContext())
        {
            var result = context.Event.AsNoTracking().FirstOrDefault(r => r.Name.ToLower() == nameToLower);
    ...
    private readonly ConcurrentDictionary<
        ...
        ,
        List<HousePortal>
    > cachedHousePortals = new ConcurrentDictionary<uint, List<HousePortal>>();
    ...
    public void CacheAllHousePortals()
    {
        using (var context = new WorldDbContext())
        {
            var results = context.HousePortal.AsNoTracking().AsEnumerable().GroupBy(r => r.HouseId);
    ...
    public List<HousePortal> GetCachedHousePortals(uint houseId)
    {
        ...
        using (var context = new WorldDbContext())
        {
            var results = context.HousePortal.AsNoTracking().Where(p => p.HouseId == houseId).ToList();
    ...
    private readonly ConcurrentDictionary<
        ...
        ,
        List<HousePortal>
    > cachedHousePortalsByLandblock = new ConcurrentDictionary<uint, List<HousePortal>>();
    ...
    public List<HousePortal> GetCachedHousePortalsByLandblock(uint landblockId)
    {
        ...
        using (var context = new WorldDbContext())
        {
            var results = context.HousePortal.AsNoTracking().Where(p => landblockId == p.ObjCellId >> 16).ToList();
    ...
    private readonly ConcurrentDictionary<
        ushort /* Landblock */
        ,
        List<LandblockInstance>
    > cachedLandblockInstances = new ConcurrentDictionary<ushort, List<LandblockInstance>>();
    ...
    public List<LandblockInstance> GetCachedInstancesByLandblock(WorldDbContext context, ushort landblock)
    {
        ...
        var results = context
            .LandblockInstance.Include(r => r.LandblockInstanceLink)
            .AsNoTracking()
    ...
    private readonly ConcurrentDictionary<
        ...
        ,
        uint /* House GUID */
    > cachedBasementHouseGuids = new ConcurrentDictionary<ushort, uint>();
    ...
    public uint GetCachedBasementHouseGuid(ushort landblock)
    {
        ...
        {
            var result = context
                .LandblockInstance.AsNoTracking()
                .Where(r =>
                    r.Landblock == landblock
    ...
    private readonly ConcurrentDictionary<string, PointsOfInterest> cachedPointsOfInterest =
        new ConcurrentDictionary<string, PointsOfInterest>();
    ...
    public void CacheAllPointsOfInterest()
    {
        using (var context = new WorldDbContext())
        {
            var results = context.PointsOfInterest.AsNoTracking();
    ...
    public PointsOfInterest GetCachedPointOfInterest(string name)
    {
        ...
        using (var context = new WorldDbContext())
        {
            var result = context.PointsOfInterest.AsNoTracking().FirstOrDefault(r => r.Name.ToLower() == nameToLower);
    ...
    private readonly ConcurrentDictionary<
        ...
        ,
        Spell
    > spellCache = new ConcurrentDictionary<uint, Spell>();
    ...
    public void CacheAllSpells()
    {
        using (var context = new WorldDbContext())
        {
            var results = context.Spell.AsNoTracking();
    ...
    public Spell GetCachedSpell(uint spellId)
    {
        ...
        using (var context = new WorldDbContext())
        {
            var result = context.Spell.AsNoTracking().FirstOrDefault(r => r.Id == spellId);
    ...
    private readonly ConcurrentDictionary<
        ...
        ,
        TreasureDeath
    > cachedDeathTreasure = new ConcurrentDictionary<uint, TreasureDeath>();
    ...
    public void CacheAllTreasuresDeath()
    {
        using (var context = new WorldDbContext())
        {
            var results = context.TreasureDeath.AsNoTracking();
    ...
    public TreasureDeath GetCachedDeathTreasure(uint dataId)
    {
        ...
        using (var context = new WorldDbContext())
        {
            var result = context.TreasureDeath.AsNoTracking().FirstOrDefault(r => r.TreasureType == dataId);
    ...
    private Dictionary<
        ...
        Dictionary<
            int /* Tier */
            ,
            List<TreasureMaterialBase>
        >
    > cachedTreasureMaterialBase;
    ...
    public List<TreasureMaterialBase> GetCachedTreasureMaterialBase(int materialCode, int tier)
    {
        ...
        if (
            cachedTreasureMaterialBase.TryGetValue(materialCode, out var tiers)
            && tiers.TryGetValue(tier, out var treasureMaterialBase)
        )
        {
    ...
    private Dictionary<
        ...
        Dictionary<
            int /* Color Code */
            ,
            List<TreasureMaterialColor>
        >
    > cachedTreasureMaterialColor;
    ...
    private void TreasureMaterialColor_Normalize(
        Dictionary<int, Dictionary<int, List<TreasureMaterialColor>>> materialColor
    )
    {
    ...
    public List<TreasureMaterialColor> GetCachedTreasureMaterialColors(int materialId, int tsysColorCode)
    {
        ...
        if (
            cachedTreasureMaterialColor.TryGetValue(materialId, out var colorCodes)
            && colorCodes.TryGetValue(tsysColorCode, out var result)
        )
        {
            return result;
        }
    ...
    private Dictionary<
        ...
        Dictionary<
            int /* Tier */
            ,
            List<TreasureMaterialGroups>
        >
    > cachedTreasureMaterialGroups;
    ...
    private void TreasureMaterialGroups_Normalize(
        Dictionary<int, Dictionary<int, List<TreasureMaterialGroups>>> materialGroups
    )
    {
    ...
    public List<TreasureMaterialGroups> GetCachedTreasureMaterialGroup(int materialGroup, int tier)
    {
        ...
        if (
            cachedTreasureMaterialGroups.TryGetValue(materialGroup, out var tiers)
            && tiers.TryGetValue(tier, out var treasureMaterialGroup)
        )
        {
    ...
    private readonly ConcurrentDictionary<
        uint /* Data ID */
        ,
        List<TreasureWielded>
    > cachedWieldedTreasure = new ConcurrentDictionary<uint, List<TreasureWielded>>();
    ...
    public void CacheAllTreasureWielded()
    {
        using (var context = new WorldDbContext())
        {
            var results = context.TreasureWielded.AsNoTracking().AsEnumerable().GroupBy(r => r.TreasureType);
    ...
    public List<TreasureWielded> GetCachedWieldedTreasure(uint dataId)
    {
        ...
        using (var context = new WorldDbContext())
        {
            var results = context.TreasureWielded.AsNoTracking().Where(r => r.TreasureType == dataId).ToList();
