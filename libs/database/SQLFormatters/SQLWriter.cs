...
{
    protected static readonly Regex IllegalInFileName = new Regex(
    ...
    protected string GetValuesForTreasureDID(List<TreasureWielded> treasureWieldedList)
    {
        ...
        foreach (var item in treasureWieldedList)
        {
            ...
            if (item.StackSize > 1 && item.StackSizeVariance > 0)
            {
                var minStack = Math.Max(
                    1,
                    (int)Math.Round(item.StackSize * (1.0f - item.StackSizeVariance), 0, MidpointRounding.AwayFromZero)
                );
            ...
            {
                treasureItem +=
                    $" | Palette: {Enum.GetName(typeof(PaletteTemplate), item.PaletteId)} ({item.PaletteId})";
            }
            ...
            {
                if (
                    (!nextItemIsStartOfSubSet && !item.ContinuesPreviousSet)
                    || (nextItemIsStartOfSubSet && item.ContinuesPreviousSet)
                    || (setTotalProbability >= 1.0f && !nextItemIsStartOfSubSet)
                )
                {
                    if (setTotalProbability > 0.0f && setTotalProbability < 1.0f)
                    {
                        ...
                        if (setProbabilityLeftover < 0.01f)
                        {
                            treasure +=
                                Environment.NewLine
                                + $"                                   | {Math.Ceiling(setProbabilityLeftover), 6:#.00}% chance of nothing from this set";
                        }
                        else
                        {
                            treasure +=
                                Environment.NewLine
                                + $"                                   | {(1.0f - setTotalProbability) * 100, 6:#.00}% chance of nothing from this set";
                        }
                    }
                    ...
                    if (depth > 0 && subSetTotalProbability[depth] > 0.0f && subSetTotalProbability[depth] < 1.0f)
                    {
                        ...
                        if (setProbabilityLeftover < 0.01f)
                        {
                            treasure +=
                                Environment.NewLine
                                + $"                                   |          {new string(' ', 1 * depth)} {Math.Ceiling(setProbabilityLeftover), 6:#.00}% chance of nothing from this subset";
                        }
                        else
                        {
                            treasure +=
                                Environment.NewLine
                                + $"                                   |          {new string(' ', 1 * depth)} {(1.0f - subSetTotalProbability[depth]) * 100, 6:#.00}% chance of nothing from this subset";
                        }
                    }
            ...
            if (!nextItemIsPartOfSubSet)
            {
                if ((setTotalProbability + item.Probability) > 1.0f)
                {
                    ...
                    {
                        treasureItem +=
                            $" | Chance adjusted down from {item.Probability * 100:#.00}% due to overage for this set";
                    }

                    treasure +=
                        Environment.NewLine
                        + $"                                   | {realProbability * 100, 6:#.00}% chance of {treasureItem}";
                }
                else
                {
                    treasure +=
                        Environment.NewLine
                        + $"                                   | {item.Probability * 100, 6:#.00}% chance of {treasureItem}";
                }
            ...
            else
            {
                treasure +=
                    Environment.NewLine
                    + $"                                   |          {new string(' ', 1 * depth)} {item.Probability * 100, 6:#.00}% chance of {treasureItem}";
    ...
    {
        var label = "UNKNOWN RANDOMLY GENERATED TREASURE";
        ...
        if (deathTreasureType.HasValue && wieldedTreasureType.HasValue)
        {
            ...
            {
                label =
                    $"RANDOMLY GENERATED TREASURE from Loot Tier {TreasureDeath[deathTreasureType.Value].Tier} from Death Treasure Table id: {deathTreasureType}";
            }
