...
public class LandblockInstanceWriter : SQLWriter
{
    ...
    {
        var fileName = (input.ObjCellId >> 16).ToString("X4");
    ...
    public void CreateSQLINSERTStatement(IList<LandblockInstance> input, StreamWriter writer)
    {
        ...
        foreach (var value in input)
        {
            ...
            var output =
                ...
                + $"); /* {label} */"
                + Environment.NewLine
                + $"/* @teleloc 0x{value.ObjCellId:X8} [{TrimNegativeZero(value.OriginX):F6} {TrimNegativeZero(value.OriginY):F6} {TrimNegativeZero(value.OriginZ):F6}] {TrimNegativeZero(value.AnglesW):F6} {TrimNegativeZero(value.AnglesX):F6} {TrimNegativeZero(value.AnglesY):F6} {TrimNegativeZero(value.AnglesZ):F6} */";
            ...
            if (value.LandblockInstanceLink != null && value.LandblockInstanceLink.Count > 0)
            {
                ...
                CreateSQLINSERTStatement(
                    value.LandblockInstanceLink.OrderBy(r => r.ChildGuid).ToList(),
                    instanceWcids,
                    writer
                );
            }
    ...
    private void CreateSQLINSERTStatement(
        IList<LandblockInstanceLink> input,
        Dictionary<uint, uint> instanceWcids,
        StreamWriter writer
    )
    {
        ...
        var lineGenerator = new Func<int, string>(i =>
        {
            ...
            if (
                WeenieNames != null
                && instanceWcids.TryGetValue(input[i].ChildGuid, out var wcid)
                && WeenieNames.TryGetValue(wcid, out var weenieName)
            )
            {
