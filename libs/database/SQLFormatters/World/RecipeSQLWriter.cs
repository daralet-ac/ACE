...
public class RecipeSQLWriter : SQLWriter
{
    ...
    public string GetDefaultFileName(Recipe input, IList<CookBook> cookBooks, bool descOnly = false)
    {
        ...
        if (cookBooks != null && cookBooks.Count > 0 && WeenieNames != null)
        {
            ...
            for (var i = 1; i < cookBooks.Count; i++)
            {
                if (
                    WeenieNames.TryGetValue(cookBooks[i].SourceWCID, out var sourceWeenieName)
                    && sourceWeenieName != alternateDescription
                )
                {
        ...
        var fileName = input.Id.ToString("00000");
    ...
    public void CreateSQLINSERTStatement(uint recipeId, IList<RecipeRequirementsInt> input, StreamWriter writer)
    {
        ...
        {
            var propertyValueDescription = GetValueEnumName((PropertyInt)input[i].Stat, input[i].Value);
    ...
    public void CreateSQLINSERTStatement(uint recipeId, IList<RecipeRequirementsDID> input, StreamWriter writer)
    {
        ...
        var lineGenerator = new Func<int, string>(i =>
            $"{recipeId}, {input[i].Index}, {input[i].Stat.ToString().PadLeft(3)}, {input[i].Value}, {input[i].Enum}, {GetSQLString(input[i].Message)}) /* {((RequirementType)input[i].Index).ToString()}.{Enum.GetName(typeof(PropertyDataId), input[i].Stat)} {((CompareType)input[i].Enum).ToString()} {input[i].Value} */"
        );
    ...
    public void CreateSQLINSERTStatement(uint recipeId, IList<RecipeRequirementsIID> input, StreamWriter writer)
    {
        ...
        var lineGenerator = new Func<int, string>(i =>
            $"{recipeId}, {input[i].Index}, {input[i].Stat.ToString().PadLeft(3)}, {input[i].Value}, {input[i].Enum}, {GetSQLString(input[i].Message)}) /* {((RequirementType)input[i].Index).ToString()}.{Enum.GetName(typeof(PropertyInstanceId), input[i].Stat)} {((CompareType)input[i].Enum).ToString()} {input[i].Value} */"
        );
    ...
    public void CreateSQLINSERTStatement(uint recipeId, IList<RecipeRequirementsFloat> input, StreamWriter writer)
    {
        ...
        var lineGenerator = new Func<int, string>(i =>
            $"{recipeId}, {input[i].Index}, {input[i].Stat.ToString().PadLeft(3)}, {input[i].Value}, {input[i].Enum}, {GetSQLString(input[i].Message)}) /* {((RequirementType)input[i].Index).ToString()}.{Enum.GetName(typeof(PropertyFloat), input[i].Stat)} {((CompareType)input[i].Enum).ToString()} {input[i].Value} */"
        );
    ...
    public void CreateSQLINSERTStatement(uint recipeId, IList<RecipeRequirementsString> input, StreamWriter writer)
    {
        ...
        var lineGenerator = new Func<int, string>(i =>
            $"{recipeId}, {input[i].Index}, {input[i].Stat.ToString().PadLeft(3)}, {GetSQLString(input[i].Value)}, {input[i].Enum}, {GetSQLString(input[i].Message)}) /* {((RequirementType)input[i].Index).ToString()}.{Enum.GetName(typeof(PropertyString), input[i].Stat)} {((CompareType)input[i].Enum).ToString()} {input[i].Value} */"
        );
    ...
    public void CreateSQLINSERTStatement(uint recipeId, IList<RecipeRequirementsBool> input, StreamWriter writer)
    {
        ...
        var lineGenerator = new Func<int, string>(i =>
            $"{recipeId}, {input[i].Index}, {input[i].Stat.ToString().PadLeft(3)}, {input[i].Value}, {input[i].Enum}, {GetSQLString(input[i].Message)}) /* {((RequirementType)input[i].Index).ToString()}.{Enum.GetName(typeof(PropertyBool), input[i].Stat)} {((CompareType)input[i].Enum).ToString()} {input[i].Value} */"
        );
    ...
    public void CreateSQLINSERTStatement(uint recipeId, IList<RecipeMod> input, StreamWriter writer)
    {
        foreach (var value in input)
        {
            ...
            var output =
                $"VALUES ({recipeId}, {value.ExecutesOnSuccess}, {value.Health}, {value.Stamina}, {value.Mana}, {value.Unknown7}, {(value.DataId > 0 ? $"0x{value.DataId:X8}" : $"{value.DataId}")}, {value.Unknown9}, {value.InstanceId});";
            ...
            if (
                (value.RecipeModsInt != null && value.RecipeModsInt.Count > 0)
