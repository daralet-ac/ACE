...
public static class ShardDatabaseOfflineTools
{
    ...
    private static List<Biota> GetInventoryBiotas(ShardDbContext context, uint parentId, bool includedNestedItems)
    {
        ...
        var results = context
            .BiotaPropertiesIID.Where(r => r.Type == (ushort)PropertyInstanceId.Container && r.Value == parentId)
            .Include(r => r.Object)
            .ToList();
    ...
    {
        return context
            .BiotaPropertiesIID.Where(r => r.Type == (ushort)PropertyInstanceId.Wielder && r.Value == parentId)
            .AsNoTracking()
            .Select(r => r.ObjectId)
            .ToList();
    }

    public static void PurgeCharacter(
        ShardDbContext context,
        uint characterId,
        out int charactersPurged,
        out int playerBiotasPurged,
        out int possessionsPurged,
        string reason = null
    )
    {
        charactersPurged = 0;
        ...
        {
            _log.Debug(
                "[DATABASE][PURGE] Character 0x{CharacterId:X8}:{CharacterName}, deleted on {CharacterDeletedAt}, and {NumPossessionsPurged} of their possessions has been purged. Reason: {Reason}.",
                characterId,
                character.Name,
                Time.GetDateTimeFromTimestamp(character.DeleteTime).ToLocalTime(),
                possessionsPurged,
                reason
            );
        }
        ...
        {
            _log.Debug(
                "[DATABASE][PURGE] Character 0x{CharacterId:X8}:{CharacterName}, deleted on {CharacterDeletedAt}, and {NumPossessionsPurged} of their possessions has been purged.",
                characterId,
                character.Name,
                Time.GetDateTimeFromTimestamp(character.DeleteTime).ToLocalTime(),
                possessionsPurged
        ...
        {
            _log.Debug(
                "[DATABASE][PURGE] Character 0x{CharacterId:X8}, and {NumPossessionsPurged} of their possessions has been purged. Reason: {Reason}.",
                characterId,
                possessionsPurged,
                reason
            );
        ...
        {
            _log.Debug(
                "[DATABASE][PURGE] Character 0x{CharacterId:X8}, and {NumPossessionsPurged} of their possessions has been purged.",
                characterId,
                possessionsPurged
            );
        }
    ...
    )
    {
        using (var context = new ShardDbContext())
        {
            PurgeCharacter(
                context,
                characterId,
                out charactersPurged,
                out playerBiotasPurged,
                out possessionsPurged,
                reason
            );
        }
    }

    public static void PurgeCharactersInParallel(
        ShardDbContext context,
        int daysLimiter,
        out int charactersPurged,
        out int playerBiotasPurged,
    ...
    )
    {
        ...
        var results = context
            .Character.Where(r =>
                (r.DeleteTime > 0 && r.DeleteTime < (ulong)deleteLimit) || (r.IsDeleted && r.DeleteTime == 0)
            )
            .AsNoTracking()
            .ToList();

        var charactersPurgedTotal = 0;
        var playerBiotasPurgedTotal = 0;
        var possessionsPurgedTotal = 0;

        Parallel.ForEach(
            results,
            ConfigManager.Config.Server.Threading.DatabaseParallelOptions,
            result =>
            {
                PurgeCharacter(
                    result.Id,
                    out var charactersPurgedResult,
                    out var playerBiotasPurgedResult,
                    out var possessionsPurgedResult
                );
    ...
    public static void PurgePlayer(
        ShardDbContext context,
        uint playerId,
        out int charactersPurged,
        out int playerBiotasPurged,
    ...
    {
        var biota = context.Biota.Include(r => r.BiotaPropertiesString).FirstOrDefault(r => r.Id == id);
    ...
    {
        var totalNumberOfBiotasPurged = 0;
        ...
        // Purge characters that do not have an associated biota
        {
            ...
            */
            ...
            playerBiotaIds = context
                .Biota.AsNoTracking()
                .Select(r => r.Id)
                .Where(id => id >= 0x50000000 && id <= 0x5FFFFFFF)
                .ToHashSet();
            ...
            Parallel.ForEach(
                ...
                result =>
                {
                    PurgeCharacter(
                        result,
                        out var charactersPurged,
                        out var playerBiotasPurged,
                        out var possessionPurged,
                        "No Player biota counterpart found"
                    );

                    if (charactersPurged != 1)
                    {
                        _log.Error(
                            "[DATABASE][PURGE] PurgeOrphanedBiotasInParallel failed to purge exactly 1 character. This should not happen!"
                        );
                    }
                    ...
                    {
                        _log.Error(
                            "[DATABASE][PURGE] PurgeOrphanedBiotasInParallel purged a player biota and character record. This should not happen!"
                        );
                    }
            ...
            Parallel.ForEach(
                ...
                result =>
                {
                    PurgePlayer(
                        ...
                        out var playerBiotasPurged,
                        out var possessionPurged,
                        "No Character record counterpart found"
                    );

                    if (charactersPurged != 0)
                    {
                        _log.Error(
                            "[DATABASE][PURGE] PurgeOrphanedBiotasInParallel purged a character record and a player biota. This should not happen!"
                        );
                    }
                    ...
                    {
                        _log.Error(
                            "[DATABASE][PURGE] PurgeOrphanedBiotasInParallel failed to purge exactly 1 player biota. This should not happen!"
                        );
                    }
        ...
        // Purge contained items that belong to a parent container that no longer exists
        {
            ...
            */
            ...
            containerPointers = context
                .BiotaPropertiesIID.AsNoTracking()
                .Where(r => r.Type == (ushort)PropertyInstanceId.Container)
        ...
        // Purge wielded items that belong to a parent container that no longer exists
        {
            ...
            */
            ...
            wielderPointers = context
                .BiotaPropertiesIID.AsNoTracking()
                .Where(r => r.Type == (ushort)PropertyInstanceId.Wielder)
                .ToDictionary(i => i.ObjectId, i => i);
        ...
        {
            var locationPointers = context
                .BiotaPropertiesPosition.AsNoTracking()
                .Where(i => i.PositionType == (ushort)PositionType.Location)
                .Select(i => i.ObjectId)
                .ToHashSet();
            ...
            foreach (var kvp in biotas)
            {
                ...
                if (
                    containerPointers.ContainsKey(kvp.Key)
                    || wielderPointers.ContainsKey(kvp.Key)
                    || locationPointers.Contains(kvp.Key)
                )
                {
                    continue;
                }
        ...
        {
            var monarchs = context
                .BiotaPropertiesIID.AsNoTracking()
                .Where(i => i.Type == (ushort)PropertyInstanceId.Monarch)
                .Select(i => i.Value)
                .ToHashSet();

            var query =
                from biota in context.Biota
                join monarch in context.BiotaPropertiesIID on biota.Id equals monarch.ObjectId
                where
                    biota.WeenieType == (int)WeenieType.Allegiance && monarch.Type == (ushort)PropertyInstanceId.Monarch
                select new { Id = biota.Id, MonarchId = monarch.Value };
    ...
    public static void FixAnimPartAndTextureMapFromPR2731(out int numberOfRecordsFixed)
    {
        ...
        using (var context = new ShardDbContext())
        {
            ...
            var animPartNullRecords = context
                .BiotaPropertiesAnimPart.Where(r => r.Order == null)
                .Select(r => r.ObjectId)
                .ToList();

            var animPartRecordsToRemove = context
                .BiotaPropertiesAnimPart.Where(r => animPartNullRecords.Contains(r.ObjectId) && r.Order != null)
                .ToList();
            ...
            var textureMapNullRecords = context
                .BiotaPropertiesTextureMap.Where(r => r.Order == null)
                .Select(r => r.ObjectId)
                .ToList();

            var textureMapRecordsToRemove = context
                .BiotaPropertiesTextureMap.Where(r => textureMapNullRecords.Contains(r.ObjectId) && r.Order != null)
                .ToList();
    ...
    public static void CheckForBiotaPropertiesPaletteOrderColumnInShard()
    {
        ...
        using (var context = new ShardDbContext())
        {
            ...
            {
                _log.Warning(
                    "order column in biota_properties_palette table in shard database is missing! Attempting to fix..."
                );
                try
                {
                    context.Database.ExecuteSqlRaw(
                        "ALTER TABLE `biota_properties_palette` ADD COLUMN `order` TINYINT(3) UNSIGNED NULL DEFAULT NULL AFTER `length`;"
                    );
                ...
                catch (Exception ex)
                {
                    _log.Fatal(
                        ex,
                        "Unable to restore order column in biota_properties_palette table in shard database"
                    );
    ...
    public static void PruneDeletedCharactersFromFriendLists(out int numberOfRecordsFixed)
    {
        ...
        using (var context = new ShardDbContext())
        {
            var validCharacterIds = context
                .Character.AsNoTracking()
                .Where(c => !c.IsDeleted && c.DeleteTime == 0)
                .Select(c => c.Id)
                .ToList();

            var friendIds = context.CharacterPropertiesFriendList.AsNoTracking().Select(c => c.FriendId).ToList();

            var invalidFriendIds = friendIds.Except(validCharacterIds).ToList();

            var invalidFriends = context.CharacterPropertiesFriendList.Where(c =>
                invalidFriendIds.Contains(c.FriendId)
            );
            //.ToList();
            ...
            {
                _log.Debug(
                    $"[PRUNE] Character 0x{invalidFriend.CharacterId:X8} had 0x{invalidFriend.FriendId:X8} for a friend, which is not found in database, and has been removed from their friends list."
                );
    ...
    public static void PruneDeletedObjectsFromShortcutBars(out int numberOfRecordsFixed)
    {
        ...
        using (var context = new ShardDbContext())
        {
            var validObjectIds = context.Biota.AsNoTracking().Select(b => b.Id).ToList();

            var allShortcutIds = context
                .CharacterPropertiesShortcutBar.AsNoTracking()
                .Select(s => s.ShortcutObjectId)
                .ToList();

            var invalidShortcutIds = allShortcutIds.Except(validObjectIds).ToList();

            var invalidShortcuts = context.CharacterPropertiesShortcutBar.Where(s =>
                invalidShortcutIds.Contains(s.ShortcutObjectId)
            );

            foreach (var invalidShortcut in invalidShortcuts)
            {
                _log.Debug(
                    $"[PRUNE] Character 0x{invalidShortcut.CharacterId:X8} had 0x{invalidShortcut.ShortcutObjectId:X8} as a shortcut (in position {invalidShortcut.ShortcutBarIndex}), which is not found in database, and has been removed from their shortcut bar."
                );
    ...
    public static void PruneDeletedCharactersFromSquelchLists(out int numberOfRecordsFixed)
    {
        ...
        using (var context = new ShardDbContext())
        {
            ...
            var squelchCharacterIds = context
                .CharacterPropertiesSquelch.AsNoTracking()
            ...
            var invalidSquelchCharacters = context.CharacterPropertiesSquelch.Where(s =>
                s.SquelchAccountId == 0 && invalidSquelchCharacterIds.Contains(s.SquelchCharacterId)
            );
            //.ToList();
            ...
            {
                _log.Debug(
                    $"[PRUNE] Character 0x{invalidSquelchCharacter.CharacterId:X8} had 0x{invalidSquelchCharacter.SquelchCharacterId:X8} squelched, which is not found in database, and has been removed from their squelch list."
                );
