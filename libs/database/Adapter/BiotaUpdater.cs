...
public static class BiotaUpdater
{
    ...
    )
    {
        ...
        {
            if (
                sourceBiota.PropertiesFloat == null
                || !sourceBiota.PropertiesFloat.ContainsKey((PropertyFloat)value.Type)
            )
            {
        ...
        {
            if (
                sourceBiota.PropertiesIID == null
                || !sourceBiota.PropertiesIID.ContainsKey((PropertyInstanceId)value.Type)
            )
            {
        ...
        {
            if (
                sourceBiota.PropertiesInt64 == null
                || !sourceBiota.PropertiesInt64.ContainsKey((PropertyInt64)value.Type)
            )
            {
        ...
        {
            if (
                sourceBiota.PropertiesString == null
                || !sourceBiota.PropertiesString.ContainsKey((PropertyString)value.Type)
            )
            {
        ...
        if (sourceBiota.PropertiesPosition != null)
        {
            ...
            {
                var existingValue = targetBiota.BiotaPropertiesPosition.FirstOrDefault(r =>
                    r.PositionType == (ushort)kvp.Key
                );
                ...
                if (
                    float.IsNaN(existingValue.AnglesX)
                    || float.IsNaN(existingValue.AnglesY)
                    || float.IsNaN(existingValue.AnglesZ)
                    || float.IsNaN(existingValue.AnglesW)
                )
                {
        ...
        {
            if (
                sourceBiota.PropertiesPosition == null
                || !sourceBiota.PropertiesPosition.ContainsKey((PositionType)value.PositionType)
            )
            {
        ...
        {
            for (var i = 0; i < sourceBiota.PropertiesAnimPart.Count; i++)
            {
                var value = sourceBiota.PropertiesAnimPart[i];

                var existingValue = targetBiota.BiotaPropertiesAnimPart.FirstOrDefault(r => r.Order == i);
        ...
        {
            if (
                sourceBiota.PropertiesAnimPart == null
                || value.Order == null
                || value.Order >= sourceBiota.PropertiesAnimPart.Count
            )
            {
        ...
        {
            for (var i = 0; i < sourceBiota.PropertiesPalette.Count; i++)
            {
                var value = sourceBiota.PropertiesPalette[i];

                var existingValue = targetBiota.BiotaPropertiesPalette.FirstOrDefault(r => r.Order == i);
        ...
        {
            if (
                sourceBiota.PropertiesPalette == null
                || value.Order == null
                || value.Order >= sourceBiota.PropertiesPalette.Count
            )
            {
        ...
        {
            for (var i = 0; i < sourceBiota.PropertiesTextureMap.Count; i++)
            {
                var value = sourceBiota.PropertiesTextureMap[i];

                var existingValue = targetBiota.BiotaPropertiesTextureMap.FirstOrDefault(r => r.Order == i);
        ...
        {
            if (
                sourceBiota.PropertiesTextureMap == null
                || value.Order == null
                || value.Order >= sourceBiota.PropertiesTextureMap.Count
            )
            {
        ...
        if (sourceBiota.PropertiesCreateList != null)
        {
            ...
            foreach (var value in sourceBiota.PropertiesCreateList)
            {
                ...
                var existingValue = targetBiota.BiotaPropertiesCreateList.FirstOrDefault(r =>
                    r.Id == value.DatabaseRecordId
                );
            ...
            foreach (var value in sourceBiota.PropertiesCreateList)
            {
                ...
                var existingValue = targetBiota.BiotaPropertiesCreateList.FirstOrDefault(r =>
                    !usedTargetCreateList.Contains(r)
                );
        ...
        if (sourceBiota.PropertiesEmote != null)
        {
            ...
            foreach (var value in sourceBiota.PropertiesEmote)
            {
                ...
                // Source record should already exist in the target
                var existingValue = targetBiota.BiotaPropertiesEmote.FirstOrDefault(r =>
                    r.Id == value.DatabaseRecordId
                );
            ...
            foreach (var value in sourceBiota.PropertiesEmote)
            {
                ...
                // For simplicity, just find the first unused target
                var existingValue = targetBiota.BiotaPropertiesEmote.FirstOrDefault(r => !emoteMap.Values.Contains(r));
        ...
        {
            for (var i = 0; i < kvp.Key.PropertiesEmoteAction.Count; i++)
            {
                var existingValue = kvp.Value.BiotaPropertiesEmoteAction.FirstOrDefault(r => r.Order == i);
        ...
        if (sourceBiota.PropertiesEventFilter != null)
        {
            ...
            {
                var existingValue = targetBiota.BiotaPropertiesEventFilter.FirstOrDefault(r => r.Event == value);
        ...
        {
            if (
                sourceBiota.PropertiesEventFilter == null
                || !sourceBiota.PropertiesEventFilter.Any(p => p == value.Event)
            )
            {
        ...
        if (sourceBiota.PropertiesGenerator != null)
        {
            ...
            foreach (var value in sourceBiota.PropertiesGenerator)
            {
                ...
                // Source record should already exist in the target
                var existingValue = targetBiota.BiotaPropertiesGenerator.FirstOrDefault(r =>
                    r.Id == value.DatabaseRecordId
                );
            ...
            foreach (var value in sourceBiota.PropertiesGenerator)
            {
                ...
                // For simplicity, just find the first unused target
                var existingValue = targetBiota.BiotaPropertiesGenerator.FirstOrDefault(r =>
                    !usedTargetGenerators.Contains(r)
                );
        ...
        if (sourceBiota.PropertiesAttribute != null)
        {
            ...
            {
                var existingValue = targetBiota.BiotaPropertiesAttribute.FirstOrDefault(r => r.Type == (ushort)kvp.Key);
        ...
        {
            if (
                sourceBiota.PropertiesAttribute == null
                || !sourceBiota.PropertiesAttribute.ContainsKey((PropertyAttribute)value.Type)
            )
            {
        ...
        if (sourceBiota.PropertiesAttribute2nd != null)
        {
            ...
            {
                var existingValue = targetBiota.BiotaPropertiesAttribute2nd.FirstOrDefault(r =>
                    r.Type == (ushort)kvp.Key
                );
        ...
        {
            if (
                sourceBiota.PropertiesAttribute2nd == null
                || !sourceBiota.PropertiesAttribute2nd.ContainsKey((PropertyAttribute2nd)value.Type)
            )
            {
        ...
        if (sourceBiota.PropertiesBodyPart != null)
        {
            ...
            {
                var existingValue = targetBiota.BiotaPropertiesBodyPart.FirstOrDefault(r => r.Key == (uint)kvp.Key);
        ...
        {
            if (
                sourceBiota.PropertiesBodyPart == null
                || !sourceBiota.PropertiesBodyPart.ContainsKey((CombatBodyPart)value.Key)
            )
            {
        ...
        if (sourceBiota.PropertiesSkill != null)
        {
            ...
            {
                var existingValue = targetBiota.BiotaPropertiesSkill.FirstOrDefault(r => r.Type == (ushort)kvp.Key);
        ...
        {
            for (var i = 0; i < sourceBiota.PropertiesBookPageData.Count; i++)
            {
                var value = sourceBiota.PropertiesBookPageData[i];

                var existingValue = targetBiota.BiotaPropertiesBookPageData.FirstOrDefault(r => r.PageId == i);
        ...
        if (sourceBiota.PropertiesAllegiance != null)
        {
            ...
            {
                var existingValue = targetBiota.BiotaPropertiesAllegiance.FirstOrDefault(r => r.CharacterId == kvp.Key);
        ...
        {
            if (
                sourceBiota.PropertiesAllegiance == null
                || !sourceBiota.PropertiesAllegiance.ContainsKey(value.CharacterId)
            )
            {
        ...
        if (sourceBiota.PropertiesEnchantmentRegistry != null)
        {
            ...
            {
                var existingValue = targetBiota.BiotaPropertiesEnchantmentRegistry.FirstOrDefault(r =>
                    r.SpellId == value.SpellId && r.LayerId == value.LayerId && r.CasterObjectId == value.CasterObjectId
                );
        ...
        {
            if (
                sourceBiota.PropertiesEnchantmentRegistry == null
                || !sourceBiota.PropertiesEnchantmentRegistry.Any(p =>
                    p.SpellId == value.SpellId && p.LayerId == value.LayerId && p.CasterObjectId == value.CasterObjectId
                )
            )
            {
        ...
        if (sourceBiota.HousePermissions != null)
        {
            ...
            {
                var existingValue = targetBiota.HousePermission.FirstOrDefault(r => r.PlayerGuid == kvp.Key);
