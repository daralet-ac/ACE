...
public class EmoteAction
{
    ...
    public bool? Display
    {
        get { return (!Display_Binder.HasValue) ? null : new bool?((Display_Binder.Value != 0) ? true : false); }
        set { Display_Binder = ((!value.HasValue) ? null : new byte?((byte)(value.Value ? 1 : 0))); }
    }
    ...
    {
        get { return PScript.HasValue ? new PhysicsScriptType?((PhysicsScriptType)PScript.Value) : null; }
    ...
    public static bool IsPropertyVisible(string propertyName, EmoteAction emote)
    {
        ...
        var list = property
            .GetCustomAttributes(typeof(EmoteTypeAttribute), inherit: false)
            .Cast<EmoteTypeAttribute>()
            .ToList();
