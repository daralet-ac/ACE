    ...
    partial class Player
    {
        ...
        public bool HandleActionRaiseVital(PropertyAttribute2nd vital, uint amount)
        {
            ...
            if (amount > AvailableExperience)
            {
                ...
                _log.Error($"{Name}.HandleActionRaiseVital({vital}, {amount}) - amount > AvailableExperience ({AvailableExperience})");
                return false;
            }
        ...
        private bool SpendVitalXp(CreatureVital creatureVital, uint amount, bool sendNetworkUpdate = true)
        {
            ...
            if (creatureVital.IsMaxRank)
            {
                _log.Error($"{Name}.SpendVitalXp({creatureVital.Vital}, {amount}) - player tried to raise vital beyond max rank");
                return false;
            }
            ...
            {
                _log.Error($"{Name}.SpendVitalXp({creatureVital.Vital}, {amount}) - player tried to raise vital beyond {amountToEnd} experience");
