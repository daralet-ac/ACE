...
namespace ACE.Server.Command.Handlers
{
    public static class AdminCommands
    {
        ...
        // allow_ip {ipv4}
        [CommandHandler("allow_ip", AccessLevel.Admin, CommandHandlerFlag.None, 1, "Temporarily add an IP to the allow list", "< ipv4 address >")]
        public static void HandleAllowIp(Session session, params string[] parameters)
        {
            ...
            {
                ChatPacket.SendServerMessage(session, $"Cannot add IP: `{ip}`. Please enter a valid IPv4 address.", ChatMessageType.Broadcast);
                return;
            }
            ...
            ChatPacket.SendServerMessage(session, $"Successfully added `{ip}` to the server allow list. **Reminder:** This is temporary and should be added to the config", ChatMessageType.Broadcast);
        }
